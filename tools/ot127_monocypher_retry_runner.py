#!/usr/bin/env python3
"""Fail-closed OT-127 two-node Monocypher corrective-retry coordinator.
...
import ot127_monocypher_retry_authority as authority_contract
...
CONTINUATION_PARENT_RAW_SHA256 = "247b0b80e64a3f6bf6654be279e90dcbd80a067c52ef861313a6f370c0355941"
CONTINUATION_PARENT = {
...
JOURNAL_PATH = ROOT / ".private" / "ot127-monocypher-corrective-retry-journal.json"
EXECUTION_RECEIPT_PATH = (
    ROOT / ".private" / "ot127-monocypher-corrective-retry-execution-receipt.json"
)
RECOVERY_RECEIPT_PATH = (
    ROOT / ".private" / "ot127-monocypher-corrective-retry-recovery-receipt.json"
)
...
FIRST_FRAME_GRACE_SECONDS = 10.0
CAPTURE_DEADLINE_SECONDS = 180.0
JOURNAL_SCHEMA = "OT127MCRJ0"
RECEIPT_SCHEMA = "OT127MCER0"
...
def _validate_continuation_parent() -> None:
    ...
    if (
        ...
        or value.get("schema") != "OTMCRAR0"
        or value.get("version") != 0
        or value.get("artifact_kind")
        != "monocypher_corrective_retry_execution_abort_receipt"
        or value.get("result")
        ...
        or value.get("all_touched_nodes_restored") is not True
        or value.get("all_devices_trail_application_verified") is not True
        or value.get("all_devices_runtime_reset_complete") is not True
        or value.get("two_usb_endpoints_returned") is not True
        or value.get("owner_observed_both_trail_displays_on") is not True
        or value.get("authority_raw_sha256")
        != authority_contract.CONSUMED_AUTHORITY_RAW_SHA256
        or type(value.get("abort")) is not dict
        or value["abort"].get("root_cause_confirmed") is not True
        or value["abort"].get("benchmark_result_admitted") is not False
...
def _receipt(
    ...
    return {
        ...
        "version": 1,
        "artifact_kind": "ot127_monocypher_corrective_retry_execution_receipt",
        "result": result,
...
def _verify_installed_application_preflight(
    config: RunConfig, transport: Transport, restore: Image,
) -> None:
    """Verify then runtime-reset both devices before creating the journal."""
    ...
    for private_port in config.private_ports:
        try:
            ...
            errors.append(exc)
    if errors:
        raise RunnerError("installed application preflight failed") from errors[0]


def execute(    config: RunConfig, transport: Transport,
    ...
    if config.recover:
        ...
        if JOURNAL_PATH.exists():
            raise RunnerError("authority already consumed")
        _verify_installed_application_preflight(config, transport, restore)
...
class EsptoolSerialTransport:
    ...
    def write_application(self, private_port: str, offset: int, image: Image) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="ot127-monocypher-app-") as directory:
    ...
    def verify_application(self, private_port: str, offset: int, image: Image) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="ot127-monocypher-readback-") as directory:
        ...
        try:
            ...
            for cycle in range(CAPTURE_CYCLE_ATTEMPTS):
                ...
                if overall_deadline is None:
                    overall_deadline = opened_at + CAPTURE_DEADLINE_SECONDS
                elif overall_deadline - opened_at < FIRST_FRAME_GRACE_SECONDS:
                    raise RunnerError("serial capture incomplete")
                first_frame_grace_deadline = (
                    opened_at + FIRST_FRAME_GRACE_SECONDS
                )
                empty_reads = 0
                ...
                while (
                    ...
                    if not line:
                        ...
                        if (
                            ...
                            and time.monotonic()
                            >= first_frame_grace_deadline
                            and cycle + 1 < CAPTURE_CYCLE_ATTEMPTS
                        ):
                            retry_fresh_cycle = True
                            break
                        continue
