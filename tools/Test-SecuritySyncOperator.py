"""Complete affected OT-201 host matrix; synthetic devices and local files only."""
import argparse
...
SUITES = [
    'security_policy_sync_bundle_tests.py',
    'security_policy_sync_restore_observation_tests.py',
    'security_policy_sync_execution_tests.py',
    'security_policy_sync_runtime_bundle_tests.py',
    'security_policy_sync_operator_tests.py',
    'security_policy_sync_operator_integration_tests.py',
    'security_policy_sync_isolated_tests.py',
...
def main():
    ...
    output = ROOT/'.private/ot201-operator/matrix'/stamp
    ...
    try:
        ...
        sources = [*bundle.SOURCE_PATHS, *('tests/host/'+name for name in SUITES),
                   'tools/Test-SecuritySyncOperator.py', bundle.REPORT]
        ...
        if args.candidate is not None:
            for label, command in commands:
                if label == 'security_policy_sync_operator_integration_tests':
