param([Parameter(Mandatory=$true)][ValidatePattern('^ot24[012]-enrolled-[a-z0-9][a-z0-9-]{0,48}$')][string]$BuildName)
$ErrorActionPreference = 'Stop'
$taskRoot = Split-Path -Parent $PSScriptRoot
$taskBuild = Join-Path $taskRoot "build\$BuildName"
$taskTarget = 'heltec_v4_enrolled_eval'
$taskEvidenceFolder = if ($BuildName.StartsWith('ot242-')) { '.private\ot242-tx-completion' } elseif ($BuildName.StartsWith('ot241-')) { '.private\ot241-diagnostics' } else { '.private\ot240-integration' }
$taskEvidence = Join-Path $taskRoot $taskEvidenceFolder
