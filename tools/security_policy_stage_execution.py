"""Distinct OT196 stage-trial execution; no CLI, grants issued, or import-time I/O.
...
import security_policy_stage_observation as observation_seam
...
SCHEMAS={'execute':'OT196-STAGE-EXECUTE-GRANT-1','recover':'OT196-STAGE-RECOVER-GRANT-1'}
...
def used_path(root,attempt):
    ...
    return private_path(root,f'ot196-stage-grant-{attempt}.used')
...
def observation_path(root,role,attempt,suffix):
    ...
    return private_path(root,f'ot196-stage-observation-{role}-{attempt}.{suffix}')
...
"""
    ...
    need(intent['schema']=='OT196-STAGE-OBSERVATION-INTENT-1' and intent['role']==role and intent['attempt']==grant.attempt
    ...
    if consume:exclusive(claimed,{'schema':'OT196-STAGE-OBSERVATION-CLAIM-1','intent_sha256':purpose['sha256']})
    return intent
...
class _ObservationBackend:
    ...
    def read(self,role,offset,size):
        ...
        intent={'schema':'OT196-STAGE-OBSERVATION-INTENT-1','role':role,'attempt':self.journal.owner['attempt'],
...
def restore_stage(root,backend,journal,row,data):
    ...
    try:
        ...
        capture=private_path(root,f"ot195-stage-{row['role']}-{journal.owner['attempt']}.bin")
        raw=_bounded(capture,NVS_SPAN);need(len(raw)==NVS_SPAN and sha(raw)==proxy.raw_sha,'observation_custody_invalid')
        receipt={'schema':'OT196-STAGE-OBSERVATION-CUSTODY-1','role':row['role'],'attempt':journal.owner['attempt'],
