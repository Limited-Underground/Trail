"""OT-210 synchronizing-input backup custody. Import is inert; device paths need authority.
...
import security_policy_invitation_operator as operator
...
def request(ctx, path, sha):
    ...
    operator.need(set(req) == {'schema','runtime_sha256','operation','request_path',
        ...
        and req['schema'] == 'OT210-INVITATION-BACKUP-OPERATOR-1' and req['runtime_sha256'] == ctx['sha']
...
def make_transport(ctx, path, sha):
    ...
    class BackupRomTransport(hardware.RomTransport):
        ...
        def command(self, route, operation, *, reset=False):
            ...
            child = {'schema':'OT210-INVITATION-BACKUP-ROM-1','runtime_sha256':ctx['sha'],
                     'operator_request':{'path':str(path),'sha256':sha},'argv':argv}
            raw = json.dumps(child,sort_keys=True,separators=(',',':')).encode()
            with tempfile.TemporaryDirectory(prefix='ot210-invitation-backup-rom-',dir=self.private_root) as directory:
                ...
                result = subprocess.run(operator.child_args(ctx['manifest'],ctx['path'],ctx['sha'],
                    'InvitationBackupRom',child_path,operator.digest(raw)),stdin=subprocess.DEVNULL,
...
def _handoff(root,ctx,req,data):
    ...
    readback=importlib.import_module('security_policy_invitation_readback')
...
def run_rom(ctx,path,sha):
    ...
    operator.need(set(child) == {'schema','runtime_sha256','operator_request','argv'}
                  and child['schema'] == 'OT210-INVITATION-BACKUP-ROM-1' and child['runtime_sha256'] == ctx['sha'])
    ...
    execution.exclusive(execution.private_path(root,'ot210-invitation-backup-child-'+claim+'.used'),{'request_sha256':sha})
    if argv[11:] == ['run']:
        ...
        execution.exclusive(execution.private_path(root,'ot210-invitation-backup-reset-'+marker+'.used'),{'grant':grant['raw_sha256']})
