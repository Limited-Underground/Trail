"""Enrolled-status evaluation custody successor; import is inert, no grant issuer.
...
SPANS = base.SPANS
ACTIVE = 'enrolled-confirmation-active.lock'
ROLES = ('A', 'B')
ACTIONS = ['capture_both', 'sequential_candidate_write', 'candidate_boot',
           'enrolled_status_bridge', 'restore_both', 'original_reset']
RECOVERY_ACTIONS = ['restore_both', 'original_reset']
OUTCOMES = ('status_exchanged', 'refused', 'cancelled', 'unavailable', 'timeout')
PAIR_NAMESPACES = {b'ot230_boot', b'ot230_role', b'ot230_tx', b'ot230_rx', b'ot240_eval'}
...
def validate_request(request):
    ...
    need(request['schema'] == 'OT-ENROLLED-REQUEST-1' and request['case'] in ('enrolled-status-and-restore', 'enrolled-status-radio-and-restore'), 'request_invalid')
...
def actions_for(request, operation):
    ...
    need(request['case'] in ('enrolled-status-and-restore', 'enrolled-status-radio-and-restore'), 'request_invalid')
    return [action if action != 'enrolled_status_bridge' or request['case'] == 'enrolled-status-and-restore'
            else 'bounded_radio_enrolled_status_bridge' for action in ACTIONS]
...
def validate_grant(request, raw, pin, operation, utc):
    ...
    need(g['schema'] == 'OT-ENROLLED-GRANT-1' and type(g['attempt']) is str and common.HEX32.fullmatch(g['attempt']), 'grant_invalid')
...
class Journal:
    def __init__(self, root, attempt):
        ...
        self.file = path(root, 'enrolled-confirmation-' + attempt + '.json')
        self.pending = path(root, 'enrolled-confirmation-' + attempt + '.pending')
    ...
    def original_path(self, role, name):
        return path(self.root, f'enrolled-confirmation-{self.attempt}-{role}-{name}.bin')
...
def execute(root, request, grant_raw, grant_sha, candidates, backends, bridge, *, utc=time.time):
    ...
    common.exclusive(path(root, 'enrolled-confirmation-used-' + g['attempt']), {'grant_sha256': grant_sha})
    journal = Journal(root, g['attempt'])
    state = {'schema': 'OT-ENROLLED-JOURNAL-1', 'attempt': g['attempt'], 'request': request,
...
def recover(root, request, grant_raw, grant_sha, backends, passive_idle, *, utc=time.time):
    ...
    need(state['schema'] == 'OT-ENROLLED-JOURNAL-1' and state['attempt'] == g['origin_attempt'] and state['request'] == request and state['closed'] is False, 'journal_invalid')
    common.exclusive(path(root, 'enrolled-confirmation-used-' + g['attempt']), {'grant_sha256': grant_sha})
    if journal.pending.exists():
        os.replace(journal.pending, path(root, 'enrolled-confirmation-' + g['attempt'] + '.interrupted'))
