param([Parameter(Mandatory=$true)][ValidatePattern('^ot200-sync-(?:(?:user|r2)-)?[ab]$')][string]$BuildName)
$ErrorActionPreference = 'Stop'
$taskRoot = Split-Path -Parent $PSScriptRoot
$taskBuild = Join-Path $taskRoot "build\$BuildName"
$taskEvidence = Join-Path $taskRoot '.private\ot200-integration'
...
& $taskPython -B "$env:IDF_PATH\tools\idf.py" -C "$taskRoot\firmware\targets\heltec_v4_security_sync_diag" -B $taskBuild -D "SDKCONFIG=$taskBuild\sdkconfig" -D "CMAKE_MAKE_PROGRAM=$($taskNinja.Replace('\','/'))" -D 'CCACHE_ENABLE=OFF' build *> $taskLog
