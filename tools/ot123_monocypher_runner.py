#!/usr/bin/env python3
"""Fail-closed OT-123 two-node Monocypher comparison coordinator.
...
CONTINUATION_PARENT_PATH = (
    ROOT / "tests" / "benchmarks" / "crypto"
    / "OT-122-OT005-LIBSODIUM-NOISE-RESOURCE-EXECUTION-RECEIPT-V0.json"
)
CONTINUATION_PARENT_RAW_SHA256 = "2b023c640bfbec8ad6eb5d1d63d65e8f1ad75dcfe593566aba6b3d468355a178"
CONTINUATION_PARENT = {
    "path": "tests/benchmarks/crypto/OT-122-OT005-LIBSODIUM-NOISE-RESOURCE-EXECUTION-RECEIPT-V0.json",
    "raw_sha256": CONTINUATION_PARENT_RAW_SHA256,
}
JOURNAL_PATH = ROOT / ".private" / "ot123-monocypher-recovery-journal.json"
...
JOURNAL_SCHEMA = "OT123MCRJ0"
RECEIPT_SCHEMA = "OT123MCER0"
...
def _validate_continuation_parent() -> None:
    ...
    if (
        ...
        or value.get("schema") != "OT121LPER1"
        or value.get("version") != 1
        or value.get("artifact_kind") != "ot121_local_primitive_execution_receipt"
        or value.get("result") != "two_node_local_primitives_passed_and_restored"
        or value.get("restoration_complete") is not True
        or value.get("node_count") != 2
        or value.get("authority_raw_sha256") != AUTHORITY_RAW_SHA256
...
def _receipt(
    ...
    return {
        ...
        "version": 1,
        "artifact_kind": "ot123_monocypher_comparison_execution_receipt",
        "result": result,
        ...
        "claims": {
            "monocypher_comparison_primitives_executed": result == "two_node_monocypher_comparison_passed_and_restored",
...
def _preflight_paths(config: RunConfig) -> None:
    if (
        ...
        or JOURNAL_PATH.resolve() != (
            ROOT / ".private" / "ot123-monocypher-recovery-journal.json"
        ).resolve()
...
def execute(
    ...
    if primary_error is None and restored and len(captures) == 2:
        ...
        persist()
        receipt = _receipt(
            journal, "two_node_monocypher_comparison_passed_and_restored", True,
    ...
    persist()
    receipt = _receipt(
        journal, "monocypher_comparison_execution_aborted", restored,
        captures, capture_hashes,
    )
    _write_new_receipt(config.receipt_path, receipt)
...
class EsptoolSerialTransport:
    ...
    def write_application(self, private_port: str, offset: int, image: Image) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="ot123-monocypher-app-") as directory:
    ...
    def verify_application(self, private_port: str, offset: int, image: Image) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="ot123-monocypher-readback-") as directory:
