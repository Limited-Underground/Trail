#!/usr/bin/env python3
"""Strict host-only parser for OT-149 mbedTLS/PSA comparison frames."""
...
def _decode(raw: bytes) -> list[dict[str, Any]]:
    ...
    if raw.startswith(b"\xef\xbb\xbf") or b"\x00" in raw or b"\r" in raw:
        raise FrameError("capture encoding or framing mismatch")
...
def parse_capture_bytes(raw: bytes) -> dict[str, Any]:
    """Validate one complete mbedTLS/PSA local capture and return safe facts."""
    records = _decode(raw)
    cursor = 0
