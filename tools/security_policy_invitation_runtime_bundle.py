"""Build and verify a private isolated OT-210 runtime; never launch it or access devices."""
...
SCHEMA = "OT210-INVITATION-RUNTIME-1"
PTH = b"Lib\nDLLs\npackages\npolicy\n"
ESPTOOL_CONFIG = b"[esptool]\n"
POLICY = tuple("security_policy_" + name + ".py" for name in
               ('bundle','hardware','endpoint','execution','capture','operator','backup','backup_operator','diagnostics','runtime_bundle','input_readback','input_timing','invitation_readback','invitation_observation','invitation_bundle','invitation_execution','invitation_operator','invitation_backup_operator','invitation_runtime_bundle','invitation_restore_observation','receipt_boundary')) + ("Invoke-SecurityPolicyOperator.ps1", "Invoke-SecurityPolicyInvitationOperator.ps1")
