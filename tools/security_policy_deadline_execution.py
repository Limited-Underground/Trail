"""Distinct OT212 receipt-boundary trial composition; no CLI or import-time I/O.
...
import security_policy_deadline_restore_observation as observation_seam
...
SCHEMAS={'execute':'OT212-DEADLINE-EXECUTE-GRANT-1','recover':'OT212-DEADLINE-RECOVER-GRANT-1'}
...
def used_path(root,attempt):
    ...
    return private_path(root,f'ot212-deadline-grant-{attempt}.used')
...
def observation_path(root,role,attempt,suffix):
    ...
    return private_path(root,f'ot212-deadline-observation-{role}-{attempt}.{suffix}')
...
"""
    ...
    need(intent['schema']=='OT212-DEADLINE-OBSERVATION-INTENT-1' and intent['role']==role and intent['attempt']==grant.attempt
    ...
    if consume:exclusive(claimed,{'schema':'OT212-DEADLINE-OBSERVATION-CLAIM-1','intent_sha256':purpose['sha256']})
    return intent
...
class _ObservationBackend:
    ...
    def read(self,role,offset,size):
        ...
        intent={'schema':'OT212-DEADLINE-OBSERVATION-INTENT-1','role':role,'attempt':self.journal.owner['attempt'],
...
def restore_stage(root,backend,journal,row,data):
    ...
    try:
        ...
        need(claim=={'schema':'OT212-DEADLINE-OBSERVATION-CLAIM-1','intent_sha256':proxy.intent_sha},
             'observation_claim_missing')
        capture=private_path(root,f"ot212-deadline-{row['role']}-{journal.owner['attempt']}.bin")
        raw=_bounded(capture,NVS_SPAN);need(len(raw)==NVS_SPAN and sha(raw)==proxy.raw_sha,'observation_custody_invalid')
        receipt={'schema':'OT212-DEADLINE-OBSERVATION-CUSTODY-1','role':row['role'],'attempt':journal.owner['attempt'],
