"""Distinct OT201 synchronization trial composition; no CLI or import-time I/O.
...
import security_policy_sync_restore_observation as observation_seam
...
SCHEMAS={'execute':'OT201-SYNC-EXECUTE-GRANT-1','recover':'OT201-SYNC-RECOVER-GRANT-1'}
...
def used_path(root,attempt):
    ...
    return private_path(root,f'ot201-sync-grant-{attempt}.used')
...
def observation_path(root,role,attempt,suffix):
    ...
    return private_path(root,f'ot201-sync-observation-{role}-{attempt}.{suffix}')
...
"""
    ...
    need(intent['schema']=='OT201-SYNC-OBSERVATION-INTENT-1' and intent['role']==role and intent['attempt']==grant.attempt
    ...
    if consume:exclusive(claimed,{'schema':'OT201-SYNC-OBSERVATION-CLAIM-1','intent_sha256':purpose['sha256']})
    return intent
...
class _ObservationBackend:
    ...
    def read(self,role,offset,size):
        ...
        intent={'schema':'OT201-SYNC-OBSERVATION-INTENT-1','role':role,'attempt':self.journal.owner['attempt'],
...
def restore_stage(root,backend,journal,row,data):
    ...
    try:
        ...
        need(claim=={'schema':'OT201-SYNC-OBSERVATION-CLAIM-1','intent_sha256':proxy.intent_sha},
             'observation_claim_missing')
        capture=private_path(root,f"ot201-sync-{row['role']}-{journal.owner['attempt']}.bin")
        raw=_bounded(capture,NVS_SPAN);need(len(raw)==NVS_SPAN and sha(raw)==proxy.raw_sha,'observation_custody_invalid')
        receipt={'schema':'OT201-SYNC-OBSERVATION-CUSTODY-1','role':row['role'],'attempt':journal.owner['attempt'],
