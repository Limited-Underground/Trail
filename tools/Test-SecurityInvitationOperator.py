"""Complete affected OT-210 host matrix; synthetic devices and local files only."""
import argparse
...
SUITES = [
    'security_policy_invitation_bundle_tests.py',
    'security_policy_invitation_restore_observation_tests.py',
    'security_policy_invitation_execution_tests.py',
    'security_policy_invitation_runtime_bundle_tests.py',
    'security_policy_invitation_operator_tests.py',
    'security_policy_invitation_operator_integration_tests.py',
    'security_policy_invitation_isolated_tests.py',
...
def main():
    ...
    output = ROOT/'.private/ot210-operator/matrix'/stamp
    ...
    try:
        ...
        sources = [*bundle.SOURCE_PATHS, *('tests/host/'+name for name in SUITES),
                   'tools/Test-SecurityInvitationOperator.py', bundle.REPORT]
        ...
        if args.candidate is not None:
            for label, command in commands:
                if label == 'security_policy_invitation_operator_integration_tests':
