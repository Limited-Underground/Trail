"""OT-201 synchronizing-input backup custody. Import is inert; device paths need authority.
...
import security_policy_sync_operator as operator
...
def request(ctx, path, sha):
    ...
    operator.need(set(req) == {'schema','runtime_sha256','operation','request_path',
        ...
        and req['schema'] == 'OT201-SYNC-BACKUP-OPERATOR-1' and req['runtime_sha256'] == ctx['sha']
...
def make_transport(ctx, path, sha):
    ...
    class BackupRomTransport(hardware.RomTransport):
        ...
        def command(self, route, operation, *, reset=False):
            ...
            child = {'schema':'OT201-SYNC-BACKUP-ROM-1','runtime_sha256':ctx['sha'],
                     'operator_request':{'path':str(path),'sha256':sha},'argv':argv}
            raw = json.dumps(child,sort_keys=True,separators=(',',':')).encode()
            with tempfile.TemporaryDirectory(prefix='ot201-sync-backup-rom-',dir=self.private_root) as directory:
                ...
                result = subprocess.run(operator.child_args(ctx['manifest'],ctx['path'],ctx['sha'],
                    'SyncBackupRom',child_path,operator.digest(raw)),stdin=subprocess.DEVNULL,
...
def run_rom(ctx,path,sha):
    ...
    operator.need(set(child) == {'schema','runtime_sha256','operator_request','argv'}
                  and child['schema'] == 'OT201-SYNC-BACKUP-ROM-1' and child['runtime_sha256'] == ctx['sha'])
    ...
    execution.exclusive(execution.private_path(root,'ot201-sync-backup-child-'+claim+'.used'),{'request_sha256':sha})
    if argv[11:] == ['run']:
        ...
        execution.exclusive(execution.private_path(root,'ot201-sync-backup-reset-'+marker+'.used'),{'grant':grant['raw_sha256']})
