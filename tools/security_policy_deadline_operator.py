"""OT-212 isolated synchronizing-input operator. Import is inert; physical modes need authority.
...
PTH = b'Lib\nDLLs\npackages\npolicy\n'
CONFIG = b'[esptool]\n'
POLICY = ('bundle', 'hardware', 'endpoint', 'execution', 'capture', 'operator', 'backup', 'backup_operator', 'diagnostics', 'runtime_bundle', 'input_readback', 'input_timing', 'invitation_readback', 'invitation_observation', 'deadline_bundle', 'deadline_execution', 'deadline_operator', 'deadline_backup_operator', 'deadline_runtime_bundle', 'deadline_restore_observation', 'receipt_boundary', 'deadline_endpoint', 'deadline_boundary')
...
def verify_manifest(path, expected_sha):
    try:
        ...
        need(set(obj) == {'schema', 'root', 'worktree', 'files', 'versions', 'powershell'}
             and obj['schema'] == 'OT212-DEADLINE-RUNTIME-1')
        ...
        for name in ('python.exe', 'python314.dll', 'esptool.cfg', 'policy/security_policy_deadline_operator.py',
                     'policy/Invoke-SecurityPolicyDeadlineOperator.ps1','policy/Invoke-SecurityPolicyOperator.ps1', *('policy/security_policy_'+x+'.py' for x in POLICY)):
            need(name in files)
        return obj
    except Exception:
        raise OperatorError('operator_refused') from None
...
def child_args(manifest, manifest_path, manifest_sha, mode, request_path=None, request_sha=None):
    need(mode in ('Probe', 'Version', 'DeadlineOperator', 'DeadlineRom', 'DeadlineBackup', 'DeadlineBackupRom'))
    args = [manifest['powershell']['path'], '-NoProfile', '-NonInteractive', '-File',
            str(Path(manifest['root']) / 'policy' / 'Invoke-SecurityPolicyDeadlineOperator.ps1'),
            '-Manifest', str(manifest_path), '-ManifestSha256', manifest_sha, '-Mode', mode]
    if mode in ('DeadlineOperator', 'DeadlineRom', 'DeadlineBackup', 'DeadlineBackupRom'):
...
def operator_request(ctx, path, sha):
    ...
    need(set(req) == {'schema', 'runtime_sha256', 'operation', 'package_path', 'package_sha256',
         ...
         and req['schema'] == 'OT212-DEADLINE-OPERATOR-1' and req['runtime_sha256'] == ctx['sha']
...
def make_transport(ctx, request_path, request_sha):
    ...
    class VerifiedRomTransport(hardware.RomTransport):
        ...
        def command(self, route, operation, *, reset=False):
            ...
            req = {'schema': 'OT212-DEADLINE-ROM-1', 'runtime_sha256': ctx['sha'], 'operator_request':
            ...
            with tempfile.TemporaryDirectory(prefix='ot212-deadline-rom-', dir=self.private_root) as directory:
                path = Path(directory) / 'request.json'
                path.write_bytes(raw)
                result = subprocess.run(child_args(ctx['manifest'], ctx['path'], ctx['sha'], 'DeadlineRom', path, digest(raw)),
                                        stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
...
def make_backend(ctx, bindings, transport):
    module=importlib.import_module('security_policy_deadline_boundary')
...
def run_operator(ctx, request_path, request_sha):
    ...
    bundle = importlib.import_module('security_policy_deadline_bundle')
    execution = importlib.import_module('security_policy_deadline_execution')
    hardware = importlib.import_module('security_policy_hardware')
...
def run_rom(ctx, path, sha):
    ...
    need(set(req) == {'schema', 'runtime_sha256', 'operator_request', 'argv', 'purpose'}
         and req['schema'] == 'OT212-DEADLINE-ROM-1' and req['runtime_sha256'] == ctx['sha'])
    ...
    execution.exclusive(execution.private_path(root,'ot212-deadline-child-'+claim+'.used'),{'request_sha256':sha})
    if argv[11] in ('write-flash','run'):
        ...
        execution.exclusive(execution.private_path(root,'ot212-deadline-mutation-'+marker+'.used'),{'grant':grant.raw_sha256})
    tool = importlib.import_module('esptool')
    audit_loaded_modules(ctx)
    tool.main(argv)
...
def main(argv=None):
    ...
    parser.add_argument('--mode', choices=('probe','version','deadlineoperator','deadlinerom','deadlinebackup','deadlinebackuprom'), required=True)
    ...
    try:
        ...
        admit_origins(ctx)
        if args.mode in ('deadlinebackup','deadlinebackuprom'):
            need('backup' in runtime_policy(ctx))
            controller = importlib.import_module('security_policy_deadline_backup_operator')
            audit_loaded_modules(ctx)
            if args.mode == 'deadlinebackuprom':
