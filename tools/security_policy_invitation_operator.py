"""OT-210 isolated synchronizing-input operator. Import is inert; physical modes need authority.
...
PTH = b'Lib\nDLLs\npackages\npolicy\n'
CONFIG = b'[esptool]\n'
POLICY = ('bundle','hardware','endpoint','execution','capture','operator','backup','backup_operator','diagnostics','runtime_bundle','input_readback','input_timing','invitation_readback','invitation_observation','invitation_bundle','invitation_execution','invitation_operator','invitation_backup_operator','invitation_runtime_bundle','invitation_restore_observation','receipt_boundary')
...
def verify_manifest(path, expected_sha):
    try:
        ...
        need(set(obj) == {'schema', 'root', 'worktree', 'files', 'versions', 'powershell'}
             and obj['schema'] == 'OT210-INVITATION-RUNTIME-1')
        ...
        for name in ('python.exe', 'python314.dll', 'esptool.cfg', 'policy/security_policy_invitation_operator.py',
                     'policy/Invoke-SecurityPolicyInvitationOperator.ps1','policy/Invoke-SecurityPolicyOperator.ps1', *('policy/security_policy_'+x+'.py' for x in POLICY)):
            need(name in files)
        return obj
    except Exception:
        raise OperatorError('operator_refused') from None
...
def child_args(manifest, manifest_path, manifest_sha, mode, request_path=None, request_sha=None):
    need(mode in ('Probe', 'Version', 'InvitationOperator', 'InvitationRom', 'InvitationBackup', 'InvitationBackupRom'))
    args = [manifest['powershell']['path'], '-NoProfile', '-NonInteractive', '-File',
            str(Path(manifest['root']) / 'policy' / 'Invoke-SecurityPolicyInvitationOperator.ps1'),
            '-Manifest', str(manifest_path), '-ManifestSha256', manifest_sha, '-Mode', mode]
    if mode in ('InvitationOperator', 'InvitationRom', 'InvitationBackup', 'InvitationBackupRom'):
...
def operator_request(ctx, path, sha):
    ...
    need(set(req) == {'schema', 'runtime_sha256', 'operation', 'package_path', 'package_sha256',
         ...
         and req['schema'] == 'OT210-INVITATION-OPERATOR-1' and req['runtime_sha256'] == ctx['sha']
...
def make_transport(ctx, request_path, request_sha):
    ...
    class VerifiedRomTransport(hardware.RomTransport):
        ...
        def command(self, route, operation, *, reset=False):
            ...
            req = {'schema': 'OT210-INVITATION-ROM-1', 'runtime_sha256': ctx['sha'], 'operator_request':
            ...
            with tempfile.TemporaryDirectory(prefix='ot210-invitation-rom-', dir=self.private_root) as directory:
                path = Path(directory) / 'request.json'
                path.write_bytes(raw)
                result = subprocess.run(child_args(ctx['manifest'], ctx['path'], ctx['sha'], 'InvitationRom', path, digest(raw)),
                                        stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
...
def make_backend(ctx, bindings, transport):
    module=importlib.import_module('security_policy_receipt_boundary')
...
def with_diagnostics(ctx, backend, result):
    ...
    try:
        ...
        pass
    return {**result, 'receipt_diagnostics': report, 'receipt_boundary': importlib.import_module('security_policy_receipt_boundary').summary(backend), 'input_timing': importlib.import_module('security_policy_input_timing').summary(getattr(backend,'input_timing',None))}
...
def run_operator(ctx, request_path, request_sha):
    ...
    bundle = importlib.import_module('security_policy_invitation_bundle')
    execution = importlib.import_module('security_policy_invitation_execution')
    hardware = importlib.import_module('security_policy_hardware')
...
def run_rom(ctx, path, sha):
    ...
    need(set(req) == {'schema', 'runtime_sha256', 'operator_request', 'argv', 'purpose'}
         and req['schema'] == 'OT210-INVITATION-ROM-1' and req['runtime_sha256'] == ctx['sha'])
    ...
    if purpose is not None:
        ...
        execution.admit_observation_child(root,package_sha,grant,journal,row,purpose,
            runtime_sha256=ctx['sha'],expected_image=bundle.image_binding(package),
            consume=argv[11]=='read-flash')
    ...
    execution.exclusive(execution.private_path(root,'ot210-invitation-child-'+claim+'.used'),{'request_sha256':sha})
    if argv[11] in ('write-flash','run'):
        ...
        execution.exclusive(execution.private_path(root,'ot210-invitation-mutation-'+marker+'.used'),{'grant':grant.raw_sha256})
    tool = importlib.import_module('esptool')
    audit_loaded_modules(ctx)
    tool.main(argv)
...
def main(argv=None):
    ...
    parser.add_argument('--mode', choices=('probe','version','invitationoperator','invitationrom','invitationbackup','invitationbackuprom'), required=True)
    ...
    try:
        ...
        admit_origins(ctx)
        if args.mode in ('invitationbackup','invitationbackuprom'):
            need('backup' in runtime_policy(ctx))
            controller = importlib.import_module('security_policy_invitation_backup_operator')
            audit_loaded_modules(ctx)
            if args.mode == 'invitationbackuprom':
