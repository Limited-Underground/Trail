#!/usr/bin/env python3
"""OT-129 bounded retrying START/READY capture transport.
...
def _printable_preamble(line: bytes) -> bool:
    return bool(line) and line.endswith(b"\n") and all(
        value == 0x0A or 0x20 <= value <= 0x7E for value in line
    )
        ...
        while True:
            ...
            while True:
                ...
                if phase == "ready":
                    ...
                    if (
                        not _printable_preamble(line)
                        or counters.preamble_lines_ignored > MAX_PREAMBLE_LINES
