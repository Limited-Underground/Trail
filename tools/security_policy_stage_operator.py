"""OT-196 isolated stage operator entry point. Import is inert; physical modes need authority.
...
POLICY = ('bundle','hardware','endpoint','execution','capture','operator','backup',
          'backup_operator','diagnostics','runtime_bundle','stage_bundle','stage_execution',
          'stage_operator','stage_backup_operator','stage_runtime_bundle','stage_readback','stage_observation')
...
def verify_manifest(path, expected_sha):
    try:
        ...
        need(set(obj) == {'schema', 'root', 'worktree', 'files', 'versions', 'powershell'}
             and obj['schema'] == 'OT196-STAGE-RUNTIME-1')
        ...
        for name in ('python.exe', 'python314.dll', 'esptool.cfg', 'policy/security_policy_stage_operator.py',
                     'policy/Invoke-SecurityPolicyStageOperator.ps1','policy/Invoke-SecurityPolicyOperator.ps1', *('policy/security_policy_'+x+'.py' for x in POLICY)):
            need(name in files)
        return obj
    except Exception:
        raise OperatorError('operator_refused') from None
...
def child_args(manifest, manifest_path, manifest_sha, mode, request_path=None, request_sha=None):
    need(mode in ('Probe', 'Version', 'StageOperator', 'StageRom', 'StageBackup', 'StageBackupRom'))
    args = [manifest['powershell']['path'], '-NoProfile', '-NonInteractive', '-File',
            str(Path(manifest['root']) / 'policy' / 'Invoke-SecurityPolicyStageOperator.ps1'),
            '-Manifest', str(manifest_path), '-ManifestSha256', manifest_sha, '-Mode', mode]
    if mode in ('StageOperator', 'StageRom', 'StageBackup', 'StageBackupRom'):
...
def operator_request(ctx, path, sha):
    ...
    need(set(req) == {'schema', 'runtime_sha256', 'operation', 'package_path', 'package_sha256',
         ...
         and req['schema'] == 'OT196-STAGE-OPERATOR-1' and req['runtime_sha256'] == ctx['sha']
...
def make_transport(ctx, request_path, request_sha):
    ...
    class VerifiedRomTransport(hardware.RomTransport):
        ...
        def command(self, route, operation, *, reset=False):
            ...
            req = {'schema': 'OT196-STAGE-ROM-1', 'runtime_sha256': ctx['sha'], 'operator_request':
            ...
            with tempfile.TemporaryDirectory(prefix='ot196-stage-rom-', dir=self.private_root) as directory:
                path = Path(directory) / 'request.json'
                path.write_bytes(raw)
                result = subprocess.run(child_args(ctx['manifest'], ctx['path'], ctx['sha'], 'StageRom', path, digest(raw)),
                                        stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
...
def run_operator(ctx, request_path, request_sha):
    ...
    bundle = importlib.import_module('security_policy_stage_bundle')
    execution = importlib.import_module('security_policy_stage_execution')
    hardware = importlib.import_module('security_policy_hardware')
...
def run_rom(ctx, path, sha):
    ...
    need(set(req) == {'schema', 'runtime_sha256', 'operator_request', 'argv', 'purpose'}
         and req['schema'] == 'OT196-STAGE-ROM-1' and req['runtime_sha256'] == ctx['sha'])
    ...
    execution.exclusive(execution.private_path(root,'ot196-stage-child-'+claim+'.used'),{'request_sha256':sha})
    if argv[11] in ('write-flash','run'):
        ...
        execution.exclusive(execution.private_path(root,'ot196-stage-mutation-'+marker+'.used'),{'grant':grant.raw_sha256})
    tool = importlib.import_module('esptool')
    audit_loaded_modules(ctx)
    tool.main(argv)
...
def main(argv=None):
    ...
    parser.add_argument('--mode', choices=('probe','version','stageoperator','stagerom','stagebackup','stagebackuprom'), required=True)
    ...
    try:
        ...
        admit_origins(ctx)
        if args.mode in ('stagebackup','stagebackuprom'):
            need('backup' in runtime_policy(ctx))
            controller = importlib.import_module('security_policy_stage_backup_operator')
            audit_loaded_modules(ctx)
            if args.mode == 'stagebackuprom':
