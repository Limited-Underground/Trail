"""Distinct OT210 receipt-boundary trial composition; no CLI or import-time I/O.
...
import security_policy_invitation_restore_observation as observation_seam
...
ACTIONS=['rom_preflight','diagnostic_application_write','diagnostic_candidate_reset',
         'diagnostic_nvs_init','diagnostic_stage_writes','diagnostic_input_detail_write',
         'invitation_boot_record_writes','invitation_role_record_writes','serial_command',
...
SCHEMAS={'execute':'OT210-INVITATION-EXECUTE-GRANT-1','recover':'OT210-INVITATION-RECOVER-GRANT-1'}
...
def used_path(root,attempt):
    ...
    return private_path(root,f'ot210-invitation-grant-{attempt}.used')
...
def material(root, package, backend, recovery):
    ...
    image = bundle.image_binding(result['package'])
    readback.assert_image_binding(image, image)
    need(image.contract == readback.SYNC_CONTRACT, 'image_binding_invalid')
    if not recovery:
        need(type(result['candidate']) is bytes and sha(result['candidate']) == image.sha256,
             'candidate_changed')
...
def image_descriptor(image):
    """Private custody metadata only; the bundle establishes the expected image."""
    readback.assert_image_binding(image, image)
    need(image.contract == readback.SYNC_CONTRACT, 'image_binding_invalid')
    return {'contract': image.contract, 'target': image.target, 'sha256': image.sha256}
...
def observation_path(root,role,attempt,suffix):
    ...
    return private_path(root,f'ot210-invitation-observation-{role}-{attempt}.{suffix}')

def admit_observation_child(root,package_sha,grant,journal,row,purpose,*,runtime_sha256,expected_image,consume=True):
...
"""
    ...
    expected_descriptor = image_descriptor(expected_image)
    ...
    intent=decode(raw)
    need(type(intent) is dict and set(intent)=={'schema','role','attempt','package_sha256','runtime_sha256','original_nvs_sha256','journal_seq','journal_sha256','image_binding'},'observation_invalid')
    need(intent['schema']=='OT210-INVITATION-OBSERVATION-INTENT-1' and intent['role']==role and intent['attempt']==grant.attempt
         and intent['package_sha256']==package_sha==grant.package_sha256 and intent['runtime_sha256']==runtime_sha256
         and intent['original_nvs_sha256']==sha(row['nvs'])
         and intent['image_binding']==expected_descriptor,'observation_invalid')
    ...
    if consume:exclusive(claimed,{'schema':'OT210-INVITATION-OBSERVATION-CLAIM-1','intent_sha256':purpose['sha256']})
    return intent
...
class _ObservationBackend:
    def __init__(self,backend,root,journal,row,data):
        ...
        self.image = bundle.image_binding(data['package'])
    ...
    def read(self,role,offset,size):
        ...
        intent={'schema':'OT210-INVITATION-OBSERVATION-INTENT-1','role':role,'attempt':self.journal.owner['attempt'],
                ...
                'image_binding':image_descriptor(self.image)}
...
def restore_stage(root,backend,journal,row,data):
    ...
    if not any(e.get('role')==row['role'] and e['event']=='candidate_boot_intent' for e in journal.events):
        ...
        return observation_seam._unavailable(row['role'], 'candidate_not_booted')
    image = bundle.image_binding(data['package'])
    observer=observation_seam.Observation(row['nvs'],row['role'],expected_image=image)
    proxy=_ObservationBackend(backend,root,journal,row,data)
    result=observation_seam.restore_with_observation(root,proxy,journal,row,observer,observed_image=image)
    ...
    try:
        ...
        claim=decode(_bounded(observation_path(root,row['role'],journal.owner['attempt'],'claimed.json'),1024))
        need(claim=={'schema':'OT210-INVITATION-OBSERVATION-CLAIM-1','intent_sha256':proxy.intent_sha},
             'observation_claim_missing')
        capture=private_path(root,f"ot210-invitation-{row['role']}-{journal.owner['attempt']}.bin")
        raw=_bounded(capture,NVS_SPAN);need(len(raw)==NVS_SPAN and sha(raw)==proxy.raw_sha,'observation_custody_invalid')
        receipt={'schema':'OT210-INVITATION-OBSERVATION-CUSTODY-1','role':row['role'],'attempt':journal.owner['attempt'],
                 ...
                 'image_binding':image_descriptor(image),
        ...
        result=observation_seam._unavailable(row['role'],'custody_unavailable')
    return result
