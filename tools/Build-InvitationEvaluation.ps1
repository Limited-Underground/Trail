param([Parameter(Mandatory=$true)][ValidatePattern('^ot208-invitation-[a-z0-9][a-z0-9-]{0,48}$')][string]$BuildName)
$ErrorActionPreference = 'Stop'
$taskRoot = Split-Path -Parent $PSScriptRoot
$taskBuild = Join-Path $taskRoot "build\$BuildName"
$taskEvidence = Join-Path $taskRoot '.private\ot208-target'
...
& $taskPython -B "$PSScriptRoot\invitation_build_process.py" $taskLog "$env:IDF_PATH\tools\idf.py" -C "$taskRoot\firmware\targets\heltec_v4_invitation_eval" -B $taskBuild -D "SDKCONFIG=$taskBuild\sdkconfig" -D "CMAKE_MAKE_PROGRAM=$($taskNinja.Replace('\','/'))" -D 'CCACHE_ENABLE=OFF' build
