"""OT-203 synchronizing-input backup custody. Import is inert; device paths need authority.
...
import security_policy_receipt_operator as operator
...
def request(ctx, path, sha):
    ...
    operator.need(set(req) == {'schema','runtime_sha256','operation','request_path',
        ...
        and req['schema'] == 'OT203-RECEIPT-BACKUP-OPERATOR-1' and req['runtime_sha256'] == ctx['sha']
...
def make_transport(ctx, path, sha):
    ...
    class BackupRomTransport(hardware.RomTransport):
        ...
        def command(self, route, operation, *, reset=False):
            ...
            child = {'schema':'OT203-RECEIPT-BACKUP-ROM-1','runtime_sha256':ctx['sha'],
                     'operator_request':{'path':str(path),'sha256':sha},'argv':argv}
            raw = json.dumps(child,sort_keys=True,separators=(',',':')).encode()
            with tempfile.TemporaryDirectory(prefix='ot203-receipt-backup-rom-',dir=self.private_root) as directory:
                ...
                result = subprocess.run(operator.child_args(ctx['manifest'],ctx['path'],ctx['sha'],
                    'ReceiptBackupRom',child_path,operator.digest(raw)),stdin=subprocess.DEVNULL,
...
def run_rom(ctx,path,sha):
    ...
    operator.need(set(child) == {'schema','runtime_sha256','operator_request','argv'}
                  and child['schema'] == 'OT203-RECEIPT-BACKUP-ROM-1' and child['runtime_sha256'] == ctx['sha'])
    ...
    execution.exclusive(execution.private_path(root,'ot203-receipt-backup-child-'+claim+'.used'),{'request_sha256':sha})
    if argv[11:] == ['run']:
        ...
        execution.exclusive(execution.private_path(root,'ot203-receipt-backup-reset-'+marker+'.used'),{'grant':grant['raw_sha256']})
