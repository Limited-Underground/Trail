"""Distinct OT203 receipt-boundary trial composition; no CLI or import-time I/O.
...
import security_policy_receipt_restore_observation as observation_seam
...
ACTIONS=['rom_preflight','diagnostic_application_write','diagnostic_candidate_reset',
         'diagnostic_nvs_init','diagnostic_stage_writes','diagnostic_input_detail_write','serial_command',
...
SCHEMAS={'execute':'OT203-RECEIPT-EXECUTE-GRANT-1','recover':'OT203-RECEIPT-RECOVER-GRANT-1'}
...
def used_path(root,attempt):
    ...
    return private_path(root,f'ot203-receipt-grant-{attempt}.used')
...
def observation_path(root,role,attempt,suffix):
    ...
    return private_path(root,f'ot203-receipt-observation-{role}-{attempt}.{suffix}')
...
"""
    ...
    need(intent['schema']=='OT203-RECEIPT-OBSERVATION-INTENT-1' and intent['role']==role and intent['attempt']==grant.attempt
    ...
    if consume:exclusive(claimed,{'schema':'OT203-RECEIPT-OBSERVATION-CLAIM-1','intent_sha256':purpose['sha256']})
    return intent
...
class _ObservationBackend:
    ...
    def read(self,role,offset,size):
        ...
        intent={'schema':'OT203-RECEIPT-OBSERVATION-INTENT-1','role':role,'attempt':self.journal.owner['attempt'],
...
def restore_stage(root,backend,journal,row,data):
    ...
    try:
        ...
        need(claim=={'schema':'OT203-RECEIPT-OBSERVATION-CLAIM-1','intent_sha256':proxy.intent_sha},
             'observation_claim_missing')
        capture=private_path(root,f"ot203-receipt-{row['role']}-{journal.owner['attempt']}.bin")
        raw=_bounded(capture,NVS_SPAN);need(len(raw)==NVS_SPAN and sha(raw)==proxy.raw_sha,'observation_custody_invalid')
        receipt={'schema':'OT203-RECEIPT-OBSERVATION-CUSTODY-1','role':row['role'],'attempt':journal.owner['attempt'],
