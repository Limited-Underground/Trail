"""OT-201 isolated synchronizing-input operator. Import is inert; physical modes need authority.
...
PTH = b'Lib\nDLLs\npackages\npolicy\n'
CONFIG = b'[esptool]\n'
POLICY = ('bundle','hardware','endpoint','execution','capture','operator','backup','backup_operator','diagnostics','runtime_bundle','input_readback','input_timing','sync_readback','sync_observation','sync_bundle','sync_execution','sync_operator','sync_backup_operator','sync_runtime_bundle','sync_restore_observation')
...
def verify_manifest(path, expected_sha):
    try:
        ...
        need(set(obj) == {'schema', 'root', 'worktree', 'files', 'versions', 'powershell'}
             and obj['schema'] == 'OT201-SYNC-RUNTIME-1')
        ...
        for name in ('python.exe', 'python314.dll', 'esptool.cfg', 'policy/security_policy_sync_operator.py',
                     'policy/Invoke-SecurityPolicySyncOperator.ps1','policy/Invoke-SecurityPolicyOperator.ps1', *('policy/security_policy_'+x+'.py' for x in POLICY)):
            need(name in files)
        return obj
    except Exception:
        raise OperatorError('operator_refused') from None
...
def child_args(manifest, manifest_path, manifest_sha, mode, request_path=None, request_sha=None):
    need(mode in ('Probe', 'Version', 'SyncOperator', 'SyncRom', 'SyncBackup', 'SyncBackupRom'))
    args = [manifest['powershell']['path'], '-NoProfile', '-NonInteractive', '-File',
            str(Path(manifest['root']) / 'policy' / 'Invoke-SecurityPolicySyncOperator.ps1'),
            '-Manifest', str(manifest_path), '-ManifestSha256', manifest_sha, '-Mode', mode]
    if mode in ('SyncOperator', 'SyncRom', 'SyncBackup', 'SyncBackupRom'):
...
def operator_request(ctx, path, sha):
    ...
    need(set(req) == {'schema', 'runtime_sha256', 'operation', 'package_path', 'package_sha256',
         ...
         and req['schema'] == 'OT201-SYNC-OPERATOR-1' and req['runtime_sha256'] == ctx['sha']
...
def make_transport(ctx, request_path, request_sha):
    ...
    class VerifiedRomTransport(hardware.RomTransport):
        ...
        def command(self, route, operation, *, reset=False):
            ...
            req = {'schema': 'OT201-SYNC-ROM-1', 'runtime_sha256': ctx['sha'], 'operator_request':
            ...
            with tempfile.TemporaryDirectory(prefix='ot201-sync-rom-', dir=self.private_root) as directory:
                path = Path(directory) / 'request.json'
                path.write_bytes(raw)
                result = subprocess.run(child_args(ctx['manifest'], ctx['path'], ctx['sha'], 'SyncRom', path, digest(raw)),
                                        stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
...
def run_operator(ctx, request_path, request_sha):
    ...
    bundle = importlib.import_module('security_policy_sync_bundle')
    execution = importlib.import_module('security_policy_sync_execution')
    hardware = importlib.import_module('security_policy_hardware')
...
def run_rom(ctx, path, sha):
    ...
    need(set(req) == {'schema', 'runtime_sha256', 'operator_request', 'argv', 'purpose'}
         and req['schema'] == 'OT201-SYNC-ROM-1' and req['runtime_sha256'] == ctx['sha'])
    ...
    execution.exclusive(execution.private_path(root,'ot201-sync-child-'+claim+'.used'),{'request_sha256':sha})
    if argv[11] in ('write-flash','run'):
        ...
        execution.exclusive(execution.private_path(root,'ot201-sync-mutation-'+marker+'.used'),{'grant':grant.raw_sha256})
    tool = importlib.import_module('esptool')
    audit_loaded_modules(ctx)
    tool.main(argv)
...
def main(argv=None):
    ...
    parser.add_argument('--mode', choices=('probe','version','syncoperator','syncrom','syncbackup','syncbackuprom'), required=True)
    ...
    try:
        ...
        admit_origins(ctx)
        if args.mode in ('syncbackup','syncbackuprom'):
            need('backup' in runtime_policy(ctx))
            controller = importlib.import_module('security_policy_sync_backup_operator')
            audit_loaded_modules(ctx)
            if args.mode == 'syncbackuprom':
