...
$builds = @(
    ...
    @{
        ...
        Sources = @(
            (Join-Path $projectRoot 'firmware\components\companion\src\companion_protocol.cpp'),
            (Join-Path $projectRoot 'firmware\components\companion\src\companion_semantics.cpp'),
            (Join-Path $projectRoot 'tests\host\companion_semantics_tests.cpp')
        )
    },
    @{
    ...
    @{
        Name = 'companion protected-root inventory reader route'
        Output = Join-Path $buildDirectory 'companion_protected_root_inventory_reader_route_tests.exe'
        Sources = @(
            (Join-Path $projectRoot 'firmware\components\companion\src\companion_protected_root_inventory_reader_route.cpp'),
            (Join-Path $projectRoot 'tests\host\companion_protected_root_inventory_reader_route_tests.cpp')
        )
    },
    @{
    ...
    @{
        Name = 'companion SECURE_VERSION rollback-floor viability'
        Output = Join-Path $buildDirectory 'companion_rollback_floor_secure_version_viability_tests.exe'
        Sources = @(
            (Join-Path $projectRoot 'firmware\components\companion\src\companion_rollback_floor_secure_version_viability.cpp'),
            (Join-Path $projectRoot 'tests\host\companion_rollback_floor_secure_version_viability_tests.cpp')
        )
    },
    @{
    ...
    @{
        ...
        Sources = @(
            ...
            (Join-Path $projectRoot 'firmware\components\companion\src\companion_authorization_wire.cpp'),
            (Join-Path $projectRoot 'firmware\components\companion\src\companion_gatt_authorization.cpp'),
            (Join-Path $projectRoot 'tests\host\companion_gatt_authorization_tests.cpp')
        )
    },
    @{
    ...
    @{
        Name = 'revision-bound local breadcrumb archive consent'
        Output = Join-Path $buildDirectory 'breadcrumb_archive_consent_tests.exe'
        Sources = @(
            (Join-Path $projectRoot 'firmware\components\location\src\position_codec.cpp'),
            (Join-Path $projectRoot 'firmware\components\location\src\position_broadcast_scheduler.cpp'),
            (Join-Path $projectRoot 'firmware\components\location\src\breadcrumb_archive.cpp'),
            ...
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_snapshot_adapter.cpp'),
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_runtime_owner.cpp'),
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_consent.cpp'),
            (Join-Path $projectRoot 'tests\host\breadcrumb_archive_consent_tests.cpp')
        )
    },
    @{
        Name = 'complete local breadcrumb archive workflow coordinator'
        Output = Join-Path $buildDirectory 'breadcrumb_archive_workflow_coordinator_tests.exe'
        Sources = @(
            (Join-Path $projectRoot 'firmware\components\location\src\position_codec.cpp'),
            (Join-Path $projectRoot 'firmware\components\location\src\position_broadcast_scheduler.cpp'),
            (Join-Path $projectRoot 'firmware\components\location\src\breadcrumb_archive.cpp'),
            ...
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_runtime_owner.cpp'),
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_consent.cpp'),
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_workflow_coordinator.cpp'),
            (Join-Path $projectRoot 'tests\host\breadcrumb_archive_workflow_coordinator_tests.cpp')
        )
    },
    @{
        Name = 'durable breadcrumb archive workflow bootstrap'
        Output = Join-Path $buildDirectory 'breadcrumb_archive_workflow_bootstrap_tests.exe'
        Sources = @(
            (Join-Path $projectRoot 'firmware\components\location\src\position_codec.cpp'),
            (Join-Path $projectRoot 'firmware\components\location\src\position_broadcast_scheduler.cpp'),
            (Join-Path $projectRoot 'firmware\components\location\src\breadcrumb_archive.cpp'),
            ...
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_consent.cpp'),
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_workflow_coordinator.cpp'),
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_workflow_bootstrap.cpp'),
            ...
            (Join-Path $projectRoot 'tests\host\breadcrumb_archive_workflow_bootstrap_tests.cpp')
        )
    },
    @{
        Name = 'exact-revision breadcrumb archive navigation handoff'
        Output = Join-Path $buildDirectory 'breadcrumb_archive_navigation_coordinator_tests.exe'
        Sources = @(
            (Join-Path $projectRoot 'firmware\components\location\src\position_codec.cpp'),
            (Join-Path $projectRoot 'firmware\components\location\src\position_broadcast_scheduler.cpp'),
            (Join-Path $projectRoot 'firmware\components\location\src\breadcrumb_archive.cpp'),
            ...
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_workflow_coordinator.cpp'),
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_workflow_bootstrap.cpp'),
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_navigation_coordinator.cpp'),
            (Join-Path $projectRoot 'firmware\components\persistence\src\breadcrumb_archive_session_lease_store.cpp'),
            (Join-Path $projectRoot 'firmware\components\persistence\test_support\memory_persistent_storage.cpp'),
            (Join-Path $projectRoot 'tests\host\breadcrumb_archive_navigation_coordinator_tests.cpp')
        )
    },
    @{
        Name = 'optional breadcrumb archive parent page'
        Output = Join-Path $buildDirectory 'breadcrumb_archive_parent_page_coordinator_tests.exe'
        Sources = @(
            (Join-Path $projectRoot 'firmware\components\location\src\position_codec.cpp'),
            (Join-Path $projectRoot 'firmware\components\location\src\position_broadcast_scheduler.cpp'),
            (Join-Path $projectRoot 'firmware\components\location\src\breadcrumb_archive.cpp'),
            ...
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_workflow_bootstrap.cpp'),
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_navigation_coordinator.cpp'),
            (Join-Path $projectRoot 'firmware\components\integration\src\breadcrumb_archive_parent_page_coordinator.cpp'),
            (Join-Path $projectRoot 'firmware\components\persistence\src\breadcrumb_archive_session_lease_store.cpp'),
            (Join-Path $projectRoot 'firmware\components\persistence\test_support\memory_persistent_storage.cpp'),
            (Join-Path $projectRoot 'tests\host\breadcrumb_archive_parent_page_coordinator_tests.cpp')
        )
    },
    @{
    ...
    @{
        ...
        Sources = @(
            ...
            (Join-Path $projectRoot 'tests\host\quick_status_menu_coordinator_tests.cpp')
        )
    },
    @{
...
& $python.Source (Join-Path $projectRoot 'tests\host\protected_root_secure_version_floor_plan_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw 'Heltec V4 SECURE_VERSION rollback-floor viability plan tests failed.'
}
...
& $python.Source (Join-Path $projectRoot 'tests\host\crypto_candidate_source_lock_tests.py')
if ($LASTEXITCODE -ne 0) {
...
& $python.Source (Join-Path $projectRoot 'tests\host\crypto_candidate_acquisition_inspection_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw ('OTCAI0 candidate acquisition-inspection tests failed with exit code {0}.' -f $LASTEXITCODE)
}

& $python.Source (Join-Path $projectRoot 'tests\host\crypto_libsodium_managed_import_tests.py')
if ($LASTEXITCODE -ne 0) {
...
& $python.Source (Join-Path $projectRoot 'tests\host\crypto_monocypher_source_lock_admission_tests.py')
if ($LASTEXITCODE -ne 0) {
...
& $python.Source (Join-Path $projectRoot 'tests\host\crypto_mbedtls_api_config_admission_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw ('OTCAPIA0 mbedTLS/PSA API/config admission tests failed with exit code {0}.' -f $LASTEXITCODE)
}
...
& $python.Source (Join-Path $projectRoot 'tests\host\ot125_monocypher_retry_authority_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw 'OT-125 Monocypher retry authority host tests failed.'
}

& $python.Source (Join-Path $projectRoot 'tests\host\ot125_monocypher_retry_runner_tests.py')
if ($LASTEXITCODE -ne 0) {
...
& $python.Source (Join-Path $projectRoot 'tests\host\ot144_monocypher_coordinator_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw 'OT-144 Monocypher restoration-safe coordinator host tests failed.'
}
& $python.Source (Join-Path $projectRoot 'tests\host\ot144_monocypher_execution_authority_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw 'OT-144 Monocypher executable bundle/authority host tests failed.'
}
& $python.Source (Join-Path $projectRoot 'tests\host\ot144_monocypher_hardware_adapter_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw 'OT-144 Monocypher hardware adapter host tests failed.'
}
& $python.Source (Join-Path $projectRoot 'tests\host\ot144_monocypher_binding_consistency_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw 'OT-144 Monocypher binding-consistency host tests failed.'
}
...
& $python.Source (Join-Path $projectRoot 'tests\host\ot131_monocypher_hardware_adapter_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw 'OT-131 Monocypher hardware adapter host tests failed.'
}
& $python.Source (Join-Path $projectRoot 'tests\host\ot131_monocypher_abort_record_tests.py')
if ($LASTEXITCODE -ne 0) {
...
& $python.Source (Join-Path $projectRoot 'tests\host\ot136_monocypher_coordinator_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw 'OT-136 Monocypher restoration-safe coordinator host tests failed.'
}
& $python.Source (Join-Path $projectRoot 'tests\host\ot136_monocypher_execution_authority_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw 'OT-136 Monocypher executable bundle/authority host tests failed.'
}
& $python.Source (Join-Path $projectRoot 'tests\host\ot136_monocypher_hardware_adapter_tests.py')
if ($LASTEXITCODE -ne 0) {
...
& $python.Source (Join-Path $projectRoot 'tests\host\ot146_monocypher_success_record_tests.py')
if ($LASTEXITCODE -ne 0) {
...
foreach ($readinessTest in @('noise_xk_ready_firmware_source_tests.py', 'noise_xk_solicited_endpoint_tests.py', 'noise_xk_solicited_runner_tests.py')) {
    & $python.Source (Join-Path $projectRoot ('tests\host\' + $readinessTest))
    if ($LASTEXITCODE -ne 0) {
        throw ('Solicited readiness validation failed: {0} (exit {1}).' -f $readinessTest, $LASTEXITCODE)
    }
}
...
foreach ($executionTest in @('noise_xk_solicited_coordinator_tests.py', 'noise_xk_solicited_backend_tests.py', 'noise_xk_solicited_execution_tests.py', 'noise_xk_solicited_bundle_tests.py', 'noise_xk_retry_diagnostics_tests.py', 'noise_xk_diagnostic_execution_tests.py', 'noise_xk_startup_execution_tests.py')) {
...
& $python.Source (Join-Path $projectRoot 'tests\host\ot157_noise_xk_radio_coordinator_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw ('OT-157 reset-aware Noise XK coordinator tests failed with exit code {0}.' -f $LASTEXITCODE)
}

& $python.Source (Join-Path $projectRoot 'tests\host\ot157_noise_xk_radio_hardware_adapter_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw ('OT-157 reset-aware Noise XK hardware-adapter tests failed with exit code {0}.' -f $LASTEXITCODE)
}

& $python.Source (Join-Path $projectRoot 'tests\host\ot157_noise_xk_radio_bundle_tests.py')
if ($LASTEXITCODE -ne 0) {
...
& $python.Source (Join-Path $projectRoot 'tests\host\ot159_noise_xk_radio_preconsumption_blockage_tests.py')
if ($LASTEXITCODE -ne 0) {
...
& $python.Source (Join-Path $projectRoot 'tests\host\ot161_noise_xk_radio_bundle_tests.py')
if ($LASTEXITCODE -ne 0) {
...
& $python.Source (Join-Path $projectRoot 'tests\host\ot163_noise_xk_radio_abort_record_tests.py')
if ($LASTEXITCODE -ne 0) {
    throw ('OT163NXAR0 Noise XK radio abort-record tests failed with exit code {0}.' -f $LASTEXITCODE)
}
...
& $python.Source (Join-Path $projectRoot 'tests\host\security_policy_runtime_bundle_tests.py')
if ($LASTEXITCODE -ne 0) { throw 'security_policy_runtime_bundle_tests.py failed.' }
...
& $python.Source (Join-Path $projectRoot 'tests\host\security_policy_replay_tests.py')
if ($LASTEXITCODE -ne 0) { throw 'security_policy_replay_tests.py failed.' }
...
& $python.Source (Join-Path $projectRoot 'tests\host\security_policy_capture_tests.py')
if ($LASTEXITCODE -ne 0) { throw 'security_policy_capture_tests.py failed.' }
