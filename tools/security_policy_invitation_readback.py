"""Pure, explicitly image-bound OT-208 invitation-target NVS projection.

The frozen OT-198 parser owns page, entry, CRC and blob structural admission.
Fresh-original admission additionally excludes recognizable diagnostic, invitation
and evaluation namespace identities. It cannot establish never-used storage,
physical freshness or rollback resistance; fully purged records leave no evidence.
The OT-200 fixed record codec is retained unchanged. External image metadata must
come from separately admitted candidate/capture custody, never this NVS record.
Neither CRC nor stage values authenticate the device or prove host completion.
"""
import binascii
...
CONTRACT_TARGETS = {
    ...
    SYNC_CONTRACT: "heltec_v4_invitation_eval",
}
INPUT_KEY = b"input"
INPUT_MAGIC = 0xA20
...
# Physical namespace names only: logical ot_state/ot_counter aliases and raw
# substrings in unrelated payloads do not identify the target's retained state.
FRESH_NAMESPACES = frozenset((
    b"ot198diag", b"ot208_boot", b"ot208_ia", b"ot208_ib",
    b"ot187_ta", b"ot187_tb", b"ot187_ra", b"ot187_rb",
))


def assert_fresh(raw):
    """Admit supported originals with no recognizable target namespace identity.

    Even an empty namespace or one containing only deleted keys is retained
    state. The unchanged parser rejects deleted namespace identities, malformed
    history and unresolved blobs before this exact namespace-name check.
    """
    try:
        namespaces, _ = v1._parse(raw, True)
        _need(FRESH_NAMESPACES.isdisjoint(namespaces.values()))
