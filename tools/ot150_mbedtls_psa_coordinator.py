#!/usr/bin/env python3
"""Restoration-safe OT-150 two-node mbedTLS/PSA execution coordinator.
...
JOURNAL_PATH = PRIVATE_ROOT / "ot150-mbedtls-psa-execution-journal.json"
EXECUTION_RECEIPT_PATH = PRIVATE_ROOT / "ot150-mbedtls-psa-execution-receipt.json"
RECOVERY_RECEIPT_PATH = PRIVATE_ROOT / "ot150-mbedtls-psa-recovery-receipt.json"
...
BAUD = 115_200
JOURNAL_SCHEMA = "OT150MCJ0"
RECEIPT_SCHEMA = "OT150MCR0"
HASH64 = re.compile(r"^[0-9a-f]{64}$")
SAFE_IMAGE_NAME = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}\.bin$")

_PROTOCOL_PATH = ROOT / "tools" / "ot150_mbedtls_psa_protocol_runner.py"
_PROTOCOL_SPEC = importlib.util.spec_from_file_location(
    "_ot150_protocol", _PROTOCOL_PATH
)
...
def _binding_valid(value: object) -> bool:
    return (
        ...
        and value.baud == BAUD
        and value.protocol_start == protocol.START.decode("ascii")
        and value.protocol_ready == protocol.READY.decode("ascii")
        and value.expected_frame_count
        == protocol.frame_contract.EXPECTED_FRAME_COUNT
        == 1_015
    )
...
def _private_paths_valid() -> bool:
    expected = {
        JOURNAL_PATH: "ot150-mbedtls-psa-execution-journal.json",
        EXECUTION_RECEIPT_PATH: "ot150-mbedtls-psa-execution-receipt.json",
        RECOVERY_RECEIPT_PATH: "ot150-mbedtls-psa-recovery-receipt.json",
    }
...
def execute(
    ...
    receipt = _receipt(
        journal,
        "two_node_mbedtls_psa_passed_and_restored" if success
        else "mbedtls_psa_execution_aborted",
        restored,
        captures,
        diagnostics,
        failure,
