...
param(
    ...
    [ValidateSet('Probe','Version','StageOperator','StageRom','StageBackup','StageBackupRom')][string]$Mode = 'Probe',
    [string]$Request,
...
try {
    ...
    if ($data.schema -cne 'OT196-STAGE-RUNTIME-1' -or (($data.Keys | Sort-Object) -join ',') -cne 'files,powershell,root,schema,versions,worktree') { throw 'refused' }
    ...
    foreach ($required in @('python.exe','python314.dll','python314._pth','esptool.cfg','policy/Invoke-SecurityPolicyStageOperator.ps1','policy/Invoke-SecurityPolicyOperator.ps1','policy/security_policy_bundle.py','policy/security_policy_hardware.py','policy/security_policy_endpoint.py','policy/security_policy_execution.py','policy/security_policy_capture.py','policy/security_policy_operator.py','policy/security_policy_backup.py','policy/security_policy_backup_operator.py','policy/security_policy_diagnostics.py','policy/security_policy_runtime_bundle.py','policy/security_policy_stage_bundle.py','policy/security_policy_stage_execution.py','policy/security_policy_stage_operator.py','policy/security_policy_stage_backup_operator.py','policy/security_policy_stage_runtime_bundle.py','policy/security_policy_stage_readback.py','policy/security_policy_stage_observation.py')) {
    ...
    if ((Get-FileHash -LiteralPath $PSCommandPath -Algorithm SHA256).Hash.ToLowerInvariant() -cne $data.files['policy/Invoke-SecurityPolicyStageOperator.ps1'].sha256) { throw 'refused' }
    ...
    $worker = Join-Path $root 'policy/security_policy_stage_operator.py'
    $arguments = @('-I','-S','-B',$worker,'--manifest',$Manifest,'--sha256',$ManifestSha256,'--mode',$Mode.ToLowerInvariant())
    if ($Mode -in @('StageBackup','StageBackupRom')) {
        foreach ($name in @('policy/security_policy_backup.py','policy/security_policy_stage_backup_operator.py')) {
