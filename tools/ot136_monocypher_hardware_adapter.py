#!/usr/bin/env python3
"""Privacy-safe hardware adapter for the OT-136-authorized OT-136 coordinator.
...
COORDINATOR_PATH = ROOT / "tools" / "ot136_monocypher_coordinator.py"
AUTHORITY_TOOL_PATH = ROOT / "tools" / "ot136_monocypher_execution_authority.py"
...
coordinator = _load_module("_ot136_coordinator", COORDINATOR_PATH)
authority_contract = _load_module("_ot136_execution_authority", AUTHORITY_TOOL_PATH)
...
class ExecutionAuthorityGate:
    """Validate the immutable OT-136 authority before coordinator device I/O."""
...
class EsptoolSerialBackend:
    ...
    def write_application(self, private_endpoint: object, offset: int, image: object) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="ot136-monocypher-app-") as directory:
    ...
    def verify_application(self, private_endpoint: object, offset: int, image: object) -> None:
        ...
        if (
            ...
            raise AdapterError("application_image_invalid")
        with tempfile.TemporaryDirectory(prefix="ot136-monocypher-readback-") as directory:
    ...
    *,
    ...
    try:
        ...
        if args.preflight_only:
            ...
            print("OK: OT-136 preflight reset completed on both nodes")
        elif args.recover:
            coordinator.recover(config, backend, gate)
            print("OK: OT-136 recovery restored all touched nodes")
        else:
            coordinator.execute(config, backend, gate)
            print("OK: OT-136 two-node attempt completed and restored")
        return 0
    except BaseException:
        print("ERROR: OT-136 hardware operation failed", file=sys.stderr)
        return 2
