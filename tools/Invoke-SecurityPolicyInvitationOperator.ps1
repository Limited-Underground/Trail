...
param(
    ...
    [ValidateSet('Probe','Version','InvitationOperator','InvitationRom','InvitationBackup','InvitationBackupRom')][string]$Mode = 'Probe',
    [string]$Request,
...
try {
    ...
    if ($data.schema -cne 'OT210-INVITATION-RUNTIME-1' -or (($data.Keys | Sort-Object) -join ',') -cne 'files,powershell,root,schema,versions,worktree') { throw 'refused' }
    ...
    if ((Get-FileHash -LiteralPath $PSCommandPath -Algorithm SHA256).Hash.ToLowerInvariant() -cne $data.files['policy/Invoke-SecurityPolicyInvitationOperator.ps1'].sha256) { throw 'refused' }
    ...
    $worker = Join-Path $root 'policy/security_policy_invitation_operator.py'
    $arguments = @('-I','-S','-B',$worker,'--manifest',$Manifest,'--sha256',$ManifestSha256,'--mode',$Mode.ToLowerInvariant())
    if ($Mode -in @('InvitationBackup','InvitationBackupRom')) {
        foreach ($name in @('policy/security_policy_backup.py','policy/security_policy_invitation_backup_operator.py')) {
