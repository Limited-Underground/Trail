...
// Encodes stage_record_v1.json. Host contract checks bind these values.
...
enum class Error:std::uint8_t { none=0,console_install=1,input_refused=2,session_refused=3,entropy_start=4,sodium_init=5,evaluation_failed=6,entropy_stop=7,receipt_send=8 };
inline bool valid(Stage stage,Error error)noexcept {
 ...
 if(e==0)return true;
 return (s==2&&e==1)||(s==4&&(e==2||e==3))||(s==6&&e>=4&&e<=7)||(s==8&&e==8);
}
inline std::uint64_t encode(Stage stage,Error error)noexcept {
 if(!valid(stage,error))return 0;
 std::array<std::uint8_t,6> bytes{0x95,0xd1,1,1,static_cast<std::uint8_t>(stage),static_cast<std::uint8_t>(error)};
