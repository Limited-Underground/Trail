#pragma once
// OT229 durable v2 invitation admission around the independent policy session.
// One serialized runtime owner; authority and isolated storage outlive this
...
#include "opentrail/independent_policy_session.hpp"

namespace opentrail::security_evaluation {
class IndependentAuthorizedPolicySession final {
public:
 IndependentAuthorizedPolicySession(security::SecureRandomSource& random,
                         persistence::PersistentStorage& tx,
                         persistence::PersistentStorage& rx)
  : session_(random,tx,rx) {}
 IndependentAuthorizedPolicySession(const IndependentAuthorizedPolicySession&)=delete;
 IndependentAuthorizedPolicySession& operator=(const IndependentAuthorizedPolicySession&)=delete;

 bool generate_identity(){
  if(closed_||!session_.generate_identity())return fail();
  identity_=true;return true;
 }
 bool bind_authority(IndependentRoleInvitationAuthority& authority){
 ...
 bool begin(const IndependentInvitation& invitation,std::uint64_t now){
  // Freeze the exact packet across durable admission and core gate validation.
  ...
  if(!authority_->consume(candidate,now))return fail();
  if(!session_.begin(role==InvitationRole::initiator?OT_NOISE_XK_INITIATOR:OT_NOISE_XK_RESPONDER,
                     candidate,authority_->trusted_signer(),authority_->peer_a(),
...
private:
 ...
 bool abort_core(){
  // Preserve the frozen core abort ordering: the invalid-role begin transition
  ...
  const IndependentInvitation invitation{};const InvitationKey key{};const InvitationToken boot{};
 ...
 IndependentRoleInvitationAuthority* authority_{nullptr};IndependentPolicySession session_;
