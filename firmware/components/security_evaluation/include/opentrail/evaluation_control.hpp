...
enum class State { waiting, ready, refused };
enum class Result { pass, refused, entropy_contained, nvs_unavailable };
// Single-owner, nonblocking parser. A ready transition consumes the sole request.
class Control {
public:
 explicit Control(std::uint64_t start) noexcept : start_(start),last_(start) {}
 State poll(std::uint64_t now) noexcept {
  if(state_==State::waiting && (now<last_ || now-start_>=(size_?5000000:60000000))) state_=State::refused;
  last_=now; return state_;
 }
 State feed(char byte,std::uint64_t now) noexcept {
  if(state_!=State::waiting){state_=State::refused;return state_;}
  if(poll(now)!=State::waiting)return state_;
  if(size_==sizeof(input_)-1){state_=State::refused;return state_;}
  if(size_==0)start_=now;
  input_[size_++]=byte;
  if(byte!='\n')return state_;
  constexpr char prefix[]="RUN SEC_EVAL1 ot187-policy-v0 ";
  constexpr std::size_t n=sizeof(prefix)-1;
  if(size_!=n+33 || std::memcmp(input_,prefix,n)!=0){state_=State::refused;return state_;}
  for(std::size_t i=0;i<32;++i){char c=input_[n+i];if(!((c>='0'&&c<='9')||(c>='a'&&c<='f'))){state_=State::refused;return state_;}}
...
private:
 std::uint64_t start_,last_;State state_{State::waiting};char input_[96]{};char challenge_[33]{};std::size_t size_{0};bool emitted_{false};
};
}
