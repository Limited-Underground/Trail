#pragma once
// OT239 public signed evidence storage. Dedicated storage owns all five domains.
// Caller authenticates evidence; no key or traffic-resume authority lives here.
// Serialized ownership, exact snapshots and transition intent reject torn rotation.
// Checksums detect corruption, not adversarial whole-storage rollback.
#include "opentrail/independent_invitation.hpp"

namespace opentrail::security_evaluation {
class EnrollmentEvidenceStore final {
public:
    explicit EnrollmentEvidenceStore(persistence::PersistentStorage& storage) : storage_(storage) {}
    EnrollmentEvidenceStore(const EnrollmentEvidenceStore&) = delete;
    EnrollmentEvidenceStore& operator=(const EnrollmentEvidenceStore&) = delete;
    bool initialize() {
        return operation([&] {
            ...
            for (std::size_t slot=0; slot<2; ++slot) {
                ...
                if (!decode(retained_[1-slot], transitioning, prior) || prior.reset ||
                    ...
                    (!current.reset && equal(current.invitation, prior.invitation))) continue;
                record_=current; current_slot_=slot; return true;
            }
            return refuse();
        });
    ...
    bool read(IndependentInvitation& output) const {
        IndependentInvitation candidate{};
        const bool ok=operation([&] {
            if (!initialized_) return false;
            if (!exact()) return refuse();
            if (empty_ || record_.reset) return false;
            candidate=record_.invitation; return true;
        });
        if (ok) output=candidate;
        return ok;
    }
    bool replace(const IndependentInvitation& invitation) {
        const auto candidate=invitation;
        return operation([&] {
            if (!initialized_ || !exact() || (!empty_ && record_.reset)) return refuse();
            if (!empty_ && equal(candidate, record_.invitation)) return true;
...
private:
    ...
    struct Record { IndependentInvitation invitation{}; std::uint64_t generation{0}; bool reset{false}; };
    static constexpr std::uint32_t committed=0x2391CA17U, transitioning=0x2391CA16U;
    ...
    static bool equal(const IndependentInvitation& a,const IndependentInvitation& b) {
    ...
    static Bank encode(const Record& record,std::uint32_t marker) {
        ...
        std::memcpy(header.data(),"OTEE",4); header[4]=1; header[5]=record.reset ? 2 : 1;
        ...
        if (!record.reset) {
            std::memcpy(hashed.data()+16,record.invitation.payload.data(),188);
            std::memcpy(hashed.data()+204,record.invitation.signature.data(),64);
        }
    ...
    static bool decode(const Bank& bytes,std::uint32_t marker,Record& output) {
        ...
        if (!value.reset) {
            ...
            std::memcpy(value.invitation.payload.data(),body.data(),188);
            std::memcpy(value.invitation.signature.data(),body.data()+188,64);
        }
