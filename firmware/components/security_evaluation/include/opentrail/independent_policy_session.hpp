#pragma once
// OT229 evaluation session with the version-2 independent boot/time gate.
// The Noise, directional KDF, counter, replay, confirmation and wipe operations
// retain the frozen session contract. This is not a product crypto selection or
// same-key resume implementation. One serialized owner initializes the library
// and supplies local trusted entropy/time and isolated TX/RX storage.
#include "opentrail/evaluation_policy_session.hpp" // Shared EvaluationRecord only.
#include "opentrail/independent_invitation.hpp"

namespace opentrail::security_evaluation {
class IndependentPolicySession final {
public:
 IndependentPolicySession(security::SecureRandomSource& random,persistence::PersistentStorage& tx_storage,
 ...
 ~IndependentPolicySession(){wipe();}
 IndependentPolicySession(const IndependentPolicySession&)=delete;IndependentPolicySession& operator=(const IndependentPolicySession&)=delete;
 ...
 bool begin(ot_noise_xk_role role,const IndependentInvitation& invite,const InvitationKey& trusted_signer,
  ...
  if(phase_!=Phase::identity||(role!=OT_NOISE_XK_INITIATOR&&role!=OT_NOISE_XK_RESPONDER)||
     local_!=(role==OT_NOISE_XK_INITIATOR?a:b)||!gate_.open(invite,role==OT_NOISE_XK_INITIATOR?InvitationRole::initiator:InvitationRole::responder,
                 trusted_signer,a,b,boot,now))return fail();
...
private:
 ...
 Phase phase_{Phase::empty};IndependentInvitationGate gate_{};std::uint64_t last_now_{0};ot_noise_xk_keypair identity_{};ot_noise_xk_state state_{};
