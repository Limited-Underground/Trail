...
bool ot_console_begin_session() noexcept {
    ...
    receipt_enabled.store(true); // One-way, idempotent; never resets a fault.
    return ot_console_healthy();
}

void ot_receipt_log(char level, const char* tag, const char* format, ...) noexcept {
    if (!ot_console_healthy()) { phase.store(fault); return; }
    // Startup may precede host serial open. Only a validated READY enables output.
    if (!receipt_enabled.load()) return;
    const std::uint64_t started = now(nullptr);
    if (receipt_active.test_and_set(std::memory_order_acquire)) { fail(); return; }
    struct Release { ~Release() { receipt_active.clear(std::memory_order_release); } } release;
    if (started > std::numeric_limits<std::uint64_t>::max() - receipt_budget_us) { fail(); return; }
    const auto deadline = started + receipt_budget_us;
    receipt_deadline = deadline;
    receipt_last_admission = started;
    BoundedReceiptFormatter formatter;
    va_list args;
    va_start(args, format);
    const auto formatted = formatter.formatv({nullptr, now}, deadline, level,
                                              started / 1000, tag, format, args);
    va_end(args);
    const auto current = now(nullptr);
    if (formatted.status != ReceiptFormatStatus::complete || current < started
        || current >= deadline || !ot_console_healthy()) { fail(); return; }
    receipt_last_admission = current;
    const auto result = writer.write(reinterpret_cast<const std::uint8_t*>(formatter.data()),
                                    formatted.size, deadline - current);
    const auto ended = now(nullptr);
    if (result.status != ConsoleWriteStatus::accepted || ended < current
        || ended >= deadline || !ot_console_healthy()) fail();
}

// Receipt output bypasses FILE entirely. Unknown stdio cannot compete with it.
// No writer or adapter is touched before constructors. SDK logs are compiled out.
extern "C" ssize_t _write_r(struct _reent* r, int fd, const void*, size_t size) {
