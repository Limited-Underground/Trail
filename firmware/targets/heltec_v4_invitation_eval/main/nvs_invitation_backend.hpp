...
class NvsInvitationBackend final : public persistence::PersistentKvBackend {
public:
    enum class Store { boot, invite_a, invite_b };
    using Error = persistence::PersistentKvBackendError;
    explicit NvsInvitationBackend(Store store) {
        const char* name = store == Store::boot ? "ot208_boot" :
                           store == Store::invite_a ? "ot208_ia" :
                           store == Store::invite_b ? "ot208_ib" : nullptr;
        if (name != nullptr) good_ = nvs_open(name, NVS_READWRITE, &handle_) == ESP_OK;
    }
    ~NvsInvitationBackend() { if (handle_ != 0) nvs_close(handle_); }
    NvsInvitationBackend(const NvsInvitationBackend&) = delete;
    NvsInvitationBackend& operator=(const NvsInvitationBackend&) = delete;
