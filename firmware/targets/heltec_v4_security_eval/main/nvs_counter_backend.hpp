...
// exclusively owned handle to nvs/oteval_a or nvs/oteval_b. Never erases NVS.
class NvsCounterBackend final : public persistence::PersistentKvBackend {
public:
 enum class Role { a, b };
 using Error=persistence::PersistentKvBackendError;
 explicit NvsCounterBackend(Role role) {
  const char* name=role==Role::a?"oteval_a":role==Role::b?"oteval_b":nullptr;
  if(name!=nullptr) good_=nvs_open(name,NVS_READWRITE,&handle_)==ESP_OK;
 }
 ~NvsCounterBackend(){if(handle_!=0)nvs_close(handle_);}
 NvsCounterBackend(const NvsCounterBackend&)=delete;
 NvsCounterBackend& operator=(const NvsCounterBackend&)=delete;
