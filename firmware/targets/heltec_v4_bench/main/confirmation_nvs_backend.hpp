...
class ConfirmationNvsBackend final : public persistence::PersistentKvBackend {
public:
    enum class Store { boot, invite_a, invite_b, tx_a, tx_b, rx_a, rx_b };
    using Error = persistence::PersistentKvBackendError;
    explicit ConfirmationNvsBackend(Store store) {
        const char* name = store == Store::boot ? "ot216_boot" :
                           store == Store::invite_a ? "ot216_ia" :
                           store == Store::invite_b ? "ot216_ib" :
                           store == Store::tx_a ? "ot216_ta" :
                           store == Store::tx_b ? "ot216_tb" :
                           store == Store::rx_a ? "ot216_ra" :
                           store == Store::rx_b ? "ot216_rb" : nullptr;
        if (name != nullptr) good_ = nvs_open(name, NVS_READWRITE, &handle_) == ESP_OK;
    }
    ~ConfirmationNvsBackend() { if (handle_ != 0) nvs_close(handle_); }
    ConfirmationNvsBackend(const ConfirmationNvsBackend&) = delete;
    ConfirmationNvsBackend& operator=(const ConfirmationNvsBackend&) = delete;
