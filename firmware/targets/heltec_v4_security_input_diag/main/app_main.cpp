...
extern "C" void app_main(){
 ...
 DiagnosticControl control(now_us());
 const bool received=receive_diagnostic_control(control,ot_policy_read,now_us,[]{vTaskDelay(1);},ot_console_healthy);
 const bool session=received&&ot_console_begin_session();
 const auto input_error=!received?static_cast<Error>(control.reason()):!session?Error::session_refused:Error::none;
