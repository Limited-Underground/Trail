...
- Budget and measure each task stack separately. Application-task headroom does
  not establish NimBLE-task headroom. A stack-overflow hook identifies the failing
  task; a corrupted backtrace does not establish the full overflowing call chain.
...
| **OT-224: protected BLE candidate reboot** | Actual serial evidence named `ot_ble_host` in a stack-overflow report immediately before software CPU reset; the exact ELF resolves the FreeRTOS overflow hook. The candidate allocated 4096 bytes to this task. Its main-task headroom was unrelated, and the corrupted backtrace leaves the full overflowing call chain unknown. | Retain fixed task/panic classes and symbolizable instruction addresses, validate the actual SDK emitter and collector together, and measure the failing task across protected Ready and the requested operation. Stack sizing remains a candidate correction until physical acceptance. | [OT-224 evidence](testing/OT-224-CRASH-EVIDENCE-2026-09-14.md), `tests/host/ble_startup_diagnostics_tests.py`, `tests/host/companion_host_stack_observer_tests.py` |
