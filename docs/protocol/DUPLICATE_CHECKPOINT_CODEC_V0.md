...
Status: deterministic host-tested serialization boundary, 2026-08-09. The
separate context-bound `ODS0/v1` two-slot store now composes this record, but no
target adapter,
authenticated record, secure anti-rollback mechanism, or physical power-loss
result exists.
...
It contains no raw key, PIN, transport address, location, or free text. The
host-tested `DuplicateCheckpointStore` supplies two-slot generation, recovery,
and readback policy around this payload. A target adapter must still define
privacy, retention, wear, physical atomicity, rollback, and erase behavior.
...
The full 24-executable OpenTrail host matrix passes after composition with the
two-slot store. The codec and existing duplicate-window suites each pass 100
