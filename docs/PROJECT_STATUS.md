...
OT-134 permanently corrects deterministic Windows Host validation checkout drift without changing accepted bytes or hashes. The last green was OT-092 run 210; OT-093 run 211 was the first red with the older OTCBL0 signature; exact OTCAI failure began at OT-098 run 217. OT-098 exists, is 5,454 bytes versus its 131,072-byte limit, and matches its expected SHA-256; unpinned CRLF conversion alone caused the mismatch. Run 262 proved OTCAI 4/4 and OT-099 5/5 green before exposing the same issue at OT-100. Deterministic LF, CRLF, and exact mixed-byte policy now covers every known raw-bound input; the new audit checks 69 files, focused OT-100 passes 6/6, and the complete Windows host matrix passes locally. No firmware, hardware, phone, product capability, V1 percentage, or website status changes. See [Decision 0073](decisions/0073-freeze-windows-raw-evidence-checkout-bytes.md) and [OT-134 evidence](../tests/hardware/OT-134-2026-08-24.md).
