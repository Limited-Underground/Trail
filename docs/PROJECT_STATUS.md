...
OT-126 records the OT-125 corrective retry as aborted before its first accepted Monocypher frame. The only benchmark-touched node was restored, readback-verified, and reset to exact Trail; the other node was never rewritten. A final non-writing hard reset returned both devices to normal runtime, both USB endpoints returned, and the owner observed both Trail displays on. The accepted OT-125 runner exposed an inadequate pre-frame timing boundary and failed to reset an untouched preflight node on abort; Decision 0064 is consumed and nonreusable. OT-127 now grants exactly one successor application-only attempt with a fixed 180-second deadline, 10-second first-frame grace, no timing CLI override, and all-device preflight reset before journal/write. No Monocypher frame, timing, primitive, or benchmark result is yet admitted. V1 remains exact 43.75%/displayed 44%. See [Decision 0065](decisions/0065-monocypher-corrective-retry-execution-abort.md), [OT-126 evidence](../tests/hardware/OT-126-2026-08-24.md), [Decision 0066](decisions/0066-one-time-monocypher-second-corrective-retry-authority.md), and [OT-127 evidence](../tests/hardware/OT-127-2026-08-24.md).
