...
OT-134 permanently corrects the deterministic Windows Host validation failure without changing any accepted artifact or pinned digest. The last green was OT-092 run 210. OT-093 run 211 began the red streak with the older OTCBL0 raw-manifest signature; the exact OTCAI `contract digest mismatch` first appeared when OT-098 introduced that validator in run 217. The OT-098 artifact is present, 5,454 bytes, below the 131,072-byte limit, and its authoritative SHA-256 matches the expected value; only unpinned Windows CRLF conversion changed the checkout bytes. Exact `.gitattributes` policies now preserve all six raw-bound OT-098/OT-099 files, focused tests pass 4/4 and 5/5, and the complete Windows host matrix passes. No firmware, hardware, phone, product capability, V1 percentage, or website status changes. See [Decision 0073](decisions/0073-freeze-windows-raw-evidence-checkout-bytes.md) and [OT-134 evidence](../tests/hardware/OT-134-2026-08-24.md).
