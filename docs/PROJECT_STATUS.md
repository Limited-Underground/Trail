...
OT-136 accepts the immutable OT-135 executable successor and a fresh non-reusable authority without executing it. The bundle binds the exact byte-bounded runner, unchanged strict 1,014-frame parser/schema, fresh restoration-safe coordinator/private state, concrete endpoint-bound adapter, exact benchmark application, and exact Trail restoration application. The authority permits exactly one two-node application-only attempt from this workspace, is consumed on success or abort, and grants no continuation; this guarantee is workspace-local operational state, not a global, hardware-backed, signed, or copy-proof mechanism. Coordinator, authority, and adapter suites pass 11/11, 10/10, and 14/14; OT-135 and frozen OT-132 pass 14/14 each, frozen OT-133 passes 11/11 + 10/10 + 14/14 + 7/7, and the raw-byte audit passes 81/81. No hardware, flash, benchmark, radio, result, selection, Phase 2 completion, score, or website status changes. V1 remains exact 43.75%/displayed 44%. The next permitted hardware step is only the single authorized attempt with exact Trail restoration of every touched node. See [Decision 0075](decisions/0075-freeze-ot136-immutable-successor-binding-and-authority.md), [OT-136 evidence](../tests/hardware/OT-136-2026-08-25.md), the [preparation](../tests/benchmarks/crypto/OT-136-OT005-MONOCYPHER-EXECUTABLE-BUNDLE-PREPARATION-V0.json), and the [authority](../tests/benchmarks/crypto/OT-136-OT005-MONOCYPHER-ONE-ATTEMPT-AUTHORITY-V0.json).
