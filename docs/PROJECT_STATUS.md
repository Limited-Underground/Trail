...
OT-152 freezes the host-only Noise XK radio-cost measurement contract around the exact accepted benchmark-only 48/48/64-byte messages and OT-114 US915 profile. One successful handshake is 160 payload bytes in three direct frames with 313,088 microseconds theoretical airtime. A later execution must reverse roles and exercise one baseline plus one bounded whole-handshake restart per direction, with temporary-secret wipe and stale-attempt rejection. These are prepared reference values only: OT-152 authorizes no device, build, flash, radio, key/entropy, or benchmark operation and admits no `measured_airtime_us`, candidate/suite/Packet-v1 selection, Phase 2 completion, readiness, or score. OT-153 must freeze an exact executable/restoration bundle and separately accept fresh non-reusable two-node radio authority before hardware. V1 remains exact 43.75%/displayed 44%, and no website update is required. See [Decision 0088](decisions/0088-prepare-noise-xk-radio-cost-measurement.md), [OT-152 evidence](../tests/hardware/OT-152-2026-08-27.md), and the [canonical preparation](../tests/benchmarks/crypto/OT-152-OT005-LIBSODIUM-NOISE-XK-RADIO-COST-PREPARATION-V0.json).
