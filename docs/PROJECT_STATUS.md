...
| --- | --- | --- |
| Two Heltec V4 LoRa-capable boards | Both V4.2 bench nodes now run the identical corrected experimental OpenTrail `heltec_v4_radio_diag` image. OT-112 proves receive-only boot plus controlled valid structured frames A→B and B→A at 17 and exactly 163 total wire bytes. OT-DEV-001 retains its earlier startup OLED/BLE evidence; OT-DEV-002 previously ran MeshCore USB Companion `v1.16.0-07a3ca9`, which remains historical evidence. | This closes only the bounded OT-112 diagnostic. Protected storage, GATT authorization, production UI/GNSS binding, stress, measured radio ceiling/rejection, latency, RSSI/SNR recording, restart persistence, exact RF output/EIRP, field range/endurance, regulatory acceptance, compatibility, and support remain open. See `tests/hardware/OT-112-2026-08-21.md` and `hardware/INVENTORY.md`. |
