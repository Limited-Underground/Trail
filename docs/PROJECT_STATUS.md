...
OT-154 now accepts the fresh explicitly owner-approved `OT154NXRA0` authority for the unchanged OT-153 executable/restoration bundle. The 3,330-byte canonical authority binds the exact benchmark and 500,944-byte Trail applications, strict runner, simultaneous two-node coordinator, concrete adapter, anonymous roles, private one-use state, and success-or-abort consumption. It permits exactly one later two-node application-only attempt from this workspace, is non-reusable, grants no continuation, and remains unexecuted. OT-155 is the sole permitted execution step. No device, endpoint, reset, flash, radio, phone, private journal, terminal receipt, result, selection, Phase 2 completion, readiness, support, release, regulatory, or score claim changes. V1 remains exact 43.75%/displayed 44%, and no website update is required. See [Decision 0090](decisions/0090-accept-ot154-noise-xk-radio-one-attempt-authority.md), [OT-154 evidence](../tests/hardware/OT-154-2026-08-27.md), and the [canonical authority](../tests/benchmarks/crypto/OT-154-OT005-LIBSODIUM-NOISE-XK-RADIO-ONE-ATTEMPT-AUTHORITY-V0.json).
