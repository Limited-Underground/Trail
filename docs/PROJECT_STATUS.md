...
OT-124 records the subsequent hardware attempt as aborted after Node A benchmark write/readback and a bounded serial-capture timeout. Node A was restored, readback-verified, and reset to exact Trail; Node B remained untouched after both installed-Trail preflight readbacks. No Monocypher frame, timing, primitive, or benchmark result is admitted, and Decision 0059 is consumed and nonreusable. OT-125 separately grants exactly one corrected application-only retry and binds the fresh runner that hard-resets before opening a new Windows serial handle, never clears that handle, and permits one bounded pre-frame fresh cycle. Hardware execution remains pending both USB endpoints and visible two-display reset preflight. V1 remains exact 43.75%/displayed 44%. See [Decision 0063](decisions/0063-monocypher-comparison-execution-abort.md), [OT-124 evidence](../tests/hardware/OT-124-2026-08-24.md), [Decision 0064](decisions/0064-one-time-monocypher-corrective-retry-authority.md), and [OT-125 evidence](../tests/hardware/OT-125-2026-08-24.md).
