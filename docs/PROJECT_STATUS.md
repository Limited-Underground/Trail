...
OT-156 now accepts the separate host-only reset-aware successor for the consumed Noise XK path. Both restart acknowledgements precede either fresh endpoint open, each old handle and queued receipt is discarded, initial/restart opens are bounded, and both complete post-reboot contracts precede every radio verb. Sixteen privacy-safe stages replace raw exception detail; the fixed-token public result and complete command stream remain exact to the frozen OT-153 happy path. The adversarial successor suites pass 17/17, the frozen regression chain passes 52/52, and the complete Windows Host matrix passes locally with exit 0. This corrects the host lifecycle contract but does not prove the physical OT-155 root cause. OT-157 must freeze a fresh executable/restoration bundle around new coordinator and adapter bindings; fresh authority remains a later separate gate. No device, firmware, radio, result, selection, Phase 2 completion, readiness, support, release, regulatory, or score claim changes. V1 remains exact 43.75%/displayed 44%, and no website update is required. See [Decision 0092](decisions/0092-accept-ot156-reset-aware-noise-xk-radio-host-correction.md) and [OT-156 evidence](../tests/hardware/OT-156-2026-08-27.md).
