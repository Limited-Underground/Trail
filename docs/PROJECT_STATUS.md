...
OT-153 now freezes the exact host-only executable/restoration bundle required by OT-152: reproducible Heltec V4.2 ESP32-S3 firmware, the strict Noise XK radio runner, simultaneous two-node restoration-safe coordinator, concrete fail-closed adapter, and exact 500,944-byte Trail restoration application. The bundle preserves the raw 48/48/64-byte `OTNXK0/v0` messages, both role directions, one baseline and one bounded whole-handshake restart per direction, strict receipt/counter validation, and exact restoration of every touched node. OT-153 creates no authority and performs no device, phone, flash, reset, serial, radio, cryptographic, or physical-measurement operation. OT-154 must separately accept fresh explicit non-reusable one-attempt owner authority before either node may be touched. No candidate/library/suite/handshake/KDF/Packet-v1 selection, Phase 2 completion, readiness, support, release, regulatory, or score claim changes. V1 remains exact 43.75%/displayed 44%, and no website update is required. See [Decision 0089](decisions/0089-freeze-noise-xk-radio-executable-bundle.md), [OT-153 evidence](../tests/hardware/OT-153-2026-08-27.md), and the [canonical preparation](../tests/benchmarks/crypto/OT-153-OT005-LIBSODIUM-NOISE-XK-RADIO-EXECUTABLE-BUNDLE-PREPARATION-V0.json).
