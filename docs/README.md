...
| Review identity or security | [Secure-LoRa lifecycle/admission contract](security/SECURE_LORA_KEY_TRANSPORT_V0.md), [Decision 0035](decisions/0035-host-tested-secure-lora-key-transport-contract.md), [threat model](security/THREAT_MODEL_V0.md), [group lifecycle](security/GROUP_LIFECYCLE_V0.md), and [crypto candidate review](security/CRYPTO_CANDIDATE_REVIEW_2026-08-10.md) |
...
4. Complete the exact OT-005 target benchmark and accept the suite/library, handshake/KDF, and packet-v1 wire selection.
5. Then implement and physically accept the frozen pairing/replacement and selected secure-LoRa paths only under separate authority.
6. Run one coherent two-phone/two-Heltec V1 acceptance only after its exact candidates and release artifact are frozen.
7. Defer four-supported-node interoperability and every relay claim to V1.5.
