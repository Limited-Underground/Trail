...
3. Next freeze and host-test the separate secure-LoRa key-provisioning and transport contract.
4. Then implement and physically accept the frozen pairing/replacement and secure-LoRa paths only under separate authority.
5. Run one coherent two-phone/two-Heltec V1 acceptance only after its exact candidates and release artifact are frozen.
6. Defer four-supported-node interoperability and every relay claim to V1.5.
