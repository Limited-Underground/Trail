...
The evaluation handshake endpoint isolates each peer's session, durable authority and local confirmation from transport. Its bounded host value frames are not a selected radio wire protocol. The additive [version-2 invitation contract](testing/OT-228-INDEPENDENT-INVITATIONS-2026-09-14.md) supports independent local boot/time authority; the [successor endpoint binding](testing/OT-229-INDEPENDENT-ENDPOINT-INTEGRATION-2026-09-14.md) is host-validated. An [experimental transport adapter](testing/OT-233-INDEPENDENT-TRANSPORT-2026-09-14.md) composes these endpoints with the existing radio interface and packet codec; trusted provisioning and target wiring remain separate boundaries. See [endpoint boundaries](testing/OT-227-INDEPENDENT-HANDSHAKE-ENDPOINTS-2026-09-14.md).
...
OT-130 freezes that successor into one immutable execution boundary. The exact firmware source inputs and matching two-build six-file tuple, OT-129 transport, unchanged strict parser/schema, single restoration-safe coordinator, benchmark application, and exact Trail restoration application are digest-bound together. The coordinator requires exactly two distinct endpoints, verifies and resets both installed Trail applications before journal or write, permits only application-offset writes, admits capture only after benchmark readback, and restores/readbacks/resets every touched node. Its recovery-only path does not require the benchmark artifact and remains retryable until restoration succeeds. A separate non-reusable authority grants exactly one two-node application-only attempt and is consumed on success or abort; it is accepted but not executed by OT-130. Device access, flash, benchmark evidence, radio, selection, and continuing authority remain outside this host-only checkpoint.

OT-131 supplies the missing concrete adapter while leaving the OT-130 coordinator unchanged. One endpoint-bound backend owns application write/readback, reset, presence, and serial-open operations. A non-consuming preflight validates the live authority/artifacts, verifies exact Trail on both nodes, and resets both before separate visual acknowledgement; execution then requires that acknowledgement. The single attempt reached Node A benchmark readback but failed closed at `capture_failed` / `preamble_invalid`; Node A restored/readback/reset exactly to Trail, Node B was never benchmark-flashed, and the owner confirmed both Trail logos. The consumed authority grants no retry. Closed counters classify the boundary without retaining the preamble, raw capture, endpoint, path, identifier, or backend text.

OT-132 adds a separate host-only successor runner rather than rewriting the OT-129 transport bound into OT-130/OT-131 history. Complete pre-READY records are opaque only within the unchanged eight-record and 512-byte limits. Only exact complete READY transitions to capture; a frame prefix before READY, duplicate READY, any non-frame record after READY, count/timing overflow, or a mismatching partial READY still fails closed. The same privacy-safe counters and unchanged strict 1,014-frame parser remain the admission boundary. No hardware, flash, execution authority, radio, or result is added; the next device gate is a new immutable executable binding and fresh explicit one-attempt authority.

OT-133 leaves OT-129 through OT-132 immutable and binds the OT-132 runner into a true successor coordinator with fresh private journal/receipt state, a concrete endpoint-bound adapter, the unchanged strict parser/schema, exact benchmark application, and exact Trail restoration application. Its fresh authority permits exactly one two-node application-only attempt and is consumed on success or abort. The executed attempt reached Node A benchmark readback, then failed closed when nine complete pre-READY records in one 512-byte read exceeded the frozen eight-record cap before any frame. Node A restored/readback/reset exactly to Trail, Node B remained untouched on Trail, and both logos were confirmed. The authority grants no retry, radio, selection, or result admission; the next gate is host-only correction/adversarial testing followed by a new immutable successor and fresh authority before any later device access.

OT-135 leaves OT-129 through OT-133 immutable and replaces only the independent complete-record ceiling in a fresh host-only runner. Opaque complete pre-READY records are limited by the unchanged cumulative 512-byte budget and control deadline; because every complete record consumes at least one newline byte, those limits already provide a finite record bound. Exact READY, fragmented READY at the full budget, frame-before-READY rejection, duplicate READY, post-READY frame-only behavior, privacy-safe diagnostics, endpoint lifecycle, and the strict 1,014-frame parser remain unchanged. The successor has no hardware or authority surface; a later executable binding and fresh non-reusable authority remain mandatory before device access.

OT-136 leaves OT-129 through OT-135 immutable and binds the exact OT-135 runner into a fresh executable boundary with new private journal/receipt state, one concrete endpoint-bound adapter, the unchanged strict parser/schema, the exact benchmark application, and the exact Trail restoration application. A fresh authority permits exactly one two-node application-only attempt from this workspace, is consumed on success or abort, is non-reusable, and was unexecuted at that checkpoint. The one-use guarantee is workspace-local operational state rather than a global, hardware-backed, signed, or copy-proof mechanism. Both installed Trail applications must pass exact readback/reset before journal or write; every touched node must restore/readback/reset exactly to Trail. No hardware, flash, benchmark, radio, result, selection, or score was added at the OT-136 checkpoint.

OT-137 records the single OT-136-authorized attempt. Node A passed benchmark readback, then the OT-135 runner failed closed as `capture_failed` / `preamble_invalid`: two 512-byte reads observed 1,024 bytes; 11 complete opaque pre-READY records were counted within the accepted complete-record allowance, and remaining partial pre-READY data crossed the cumulative 512-byte bound before any frame. Node A restored/readback/reset exactly to Trail, Node B remained untouched on Trail, and both logos were confirmed. The authority is consumed and non-reusable. No result, radio, selection, Phase 2 completion, support, regulatory, production, end-to-end, or score claim is added. No further Monocypher hardware attempt is authorized; the next gate is host-only partial-data/live boot-control investigation before any new immutable successor and fresh authority.

OT-138 host-only investigation reproduces the complete public OT-137 counter shape with fabricated data and classifies the transport conflict without reconstructing the unretained physical bytes. The frozen runner sends START before its first read and charges all subsequently returned complete and partial pre-READY bytes to one 512-byte budget; two immediately queued reads can therefore cross the budget before an empty-read sleep or the 250 ms START retry. The accepted build configuration uses USB Serial/JTAG for the startup console with INFO logging, while the application suppresses runtime logs only later in app_main before installing the direct USB driver and waiting for START. The selected successor direction is a separately pinned console-isolated quiet target with bootloader/default logs disabled and direct USB Serial/JTAG retained. It must reproduce identical BIN/ELF/map/sdkconfig tuples from two fresh builds. Frozen runners and all 512-byte/time, exact READY/frame, privacy, and real-parser boundaries remain unchanged; OT-138 creates no executable binding or authority.

OT-139 implements and accepts that separate quiet target without modifying frozen protocol sources. The target compiles the OT-129 application and START/READY control sources by reference. Its generated configuration resolves primary and secondary ESP-IDF consoles to none, bootloader/default/maximum application logging to none, direct USB Serial/JTAG enabled, and reproducible application builds enabled. It deliberately retains normal ESP32-S3 ROM logging behavior; no eFuse/GPIO suppression is used, and silence before application start is not claimed. Two fresh cache-disabled builds under the exact pinned ESP-IDF commit reproduce the same application BIN/ELF/map, bootloader BIN, partition-table BIN, and sdkconfig tuple. The target remains host-only and unbound: a separate immutable executable/restoration/coordinator binding plus fresh explicit one-attempt authority is required before device access.

OT-140 leaves OT-139 and every frozen predecessor immutable and binds the exact six-file quiet-target tuple, pinned ESP-IDF/source evidence, OT-135 runner, strict parser/schema, fresh restoration-safe coordinator/private namespace, concrete endpoint-bound adapter, and exact Trail restoration application. Only the 149,920-byte application BIN is writable at `0x10000`; the other five build artifacts are provenance-only. A fresh explicitly approved authority permits one two-node application-only attempt from this workspace, is consumed on success or abort, is non-reusable, and remains unexecuted. Both installed Trail images must pass exact readback/reset before journal or write, and every touched node must restore/readback/reset exactly. Normal ROM logging remains enabled and endpoint silence is not claimed. No hardware, result, selection, radio, Phase 2 completion, or score is added at this checkpoint.

OT-142 closes the resulting diagnostic loop without changing any frozen target. The OT-140 authority was consumed on a fail-closed attempt, every touched node restored exactly, and no benchmark result was admitted. The deterministic failure was a malformed RFC 8032 seed in the benchmark-only primitive fixture; the real pinned Monocypher implementation produces the correct vector. A separate successor corrects only the seed, adds a real-library regression gate, reproduces two matching fresh build tuples, and passes the complete Windows host matrix. No device access is authorized until a new immutable binding and fresh explicit one-attempt authority are accepted.

OT-143 leaves OT-142 and every frozen predecessor immutable and binds the exact corrected six-file tuple plus its canonical source/build evidence, the frozen OT-135 byte-bounded runner, strict OT-123 parser/schema, fresh restoration-safe coordinator/private namespace, concrete endpoint-bound adapter, and exact Trail restoration application. Only the 149,824-byte application BIN may be written at `0x10000`; the ELF, linker map, bootloader, partition table, and generated sdkconfig remain provenance-only. The fresh explicitly approved workspace-local authority permits one two-node application-only attempt, is consumed on success or abort, is non-reusable, and remains unexecuted. Both installed Trail images must pass exact readback/reset before journal or write, and every touched node must restore/readback/reset exactly. Normal ROM logging remains enabled and endpoint silence is not claimed. No hardware, flash, benchmark, radio, result, selection, Phase 2 completion, support, regulatory, production, score, or website status claim is added at this checkpoint.
...
OT-148 performs that independent host-only review and refuses the premature completion/selection step. The exact OT-116 plan still requires a two-node mbedTLS/PSA comparison result, matched linked-flash/static-RAM admission under signed/zero-capable rules, Noise XK handshake radio-cost evidence, reconciliation of all eight named gates, private raw-trace custody verification, and a separate Phase 3 admission. Libsodium 1.0.22 is the only candidate with admitted 8/8 coverage and structural selection eligibility and is therefore the evidence-backed recommendation only. No library, suite, handshake/KDF, or Packet-v1 wire format is selected, and OT-148 adds no hardware or radio authority.

OT-149 supplies only the next host boundary. A separate mbedTLS/PSA candidate and no-candidate control share the same common harness, partition layout, admitted Heltec/reproducibility/crypto configuration lineage, and narrow quiet-console successor delta; only the candidate adapter/linkage differs. The candidate exposes exactly X25519, SHA-256, HKDF-SHA256, and ChaCha20-Poly1305 encrypt/decrypt, while Ed25519 and Noise XK remain unavailable. Exact fixed vectors and fail-closed negative cases gate timing, cleanup failure reaches the terminal result, and the START/READY transport precedes exactly 1,015 strict result frames. A separate OTMRAC1/OTMRAR1 resource successor computes linked flash from ESP-IDF size JSON total size and static RAM from exact DIRAM data/bss/noinit plus all tdata/tbss parts; deltas remain signed and may be positive, zero, or negative. OT-149 creates no executable authority or result admission.

OT-150 is the deterministic host-only executable/resource successor. Candidate A/B and no-candidate control A/B are four fresh, initially absent, cache-disabled ESP-IDF v6.0.2 builds with fixed `PROJECT_VER=ot150-mbedtls-psa-v0`; each side reproduces exactly after 1,199/1,199 steps and zero compiler warnings. The canonical preparation binds each side's strict six-role executable tuple—application BIN, application ELF, linker map, bootloader BIN, partition-table BIN, and generated sdkconfig—while retaining the candidate/control JSON2 reports as matched-resource evidence. Candidate/control linked flash is 245,466/145,834 bytes (`+99,632`) and static RAM is 50,187/49,035 bytes (`+1,152`). Only the exact candidate application may become writable at `0x10000` under later fresh explicit authority; every other artifact is non-writable, and restoration must use the exact OT-147 Trail application (500,944 bytes, SHA-256 `f2a58414f82eed585ba90bf7671b06c8ebfaa82e8b23f6ac2093de95143b4e0e`). OT-150 itself performs no hardware or radio action and grants no selection, Phase 2 completion, readiness, or score credit.

OT-151 closes the later OT-150-authorized attempt as a consumed, exactly restored abort and preserves every frozen OT-150 byte. The five complete post-READY frame records and exact 1,468-byte total map to header, initialization pass, primitive-vector failure, runtime resources, and terminal failure; the frozen runner's success-only 1,015-frame trigger explains the otherwise unnecessary 180-second wait. Frozen-ELF inspection proves generic mbedTLS X25519 peer validation returned `PSA_ERROR_INVALID_ARGUMENT` for the all-zero peer, after which the pinned TF-PSA wrapper randomized the output buffer and set its reported length to 32. The benchmark fixture's additional `output_length == 0` requirement was therefore nonportable and deterministically failed. A separate corrected target removes only that assertion while retaining status rejection and zeroization. A separate failure parser/runner accepts no result from early termination; it merely recognizes exact canonical failure shapes immediately, while the successful transcript remains fixed at 1,015 frames with the same strict parser and admission boundary. OT-151 grants no authority; any future device attempt requires a newly frozen executable bundle and fresh explicit non-reusable one-attempt authority.
