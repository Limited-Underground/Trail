...
host-mediated Heltec/MeshCore delivery evidence, 2026-08-09. This is not
transport authentication, on-device OpenTrail firmware, a retry policy, or
...
- replace the passed host-mediated two-Heltec/SenseCAP adapter proof with an
  authenticated on-device producer/consumer binding and direct radio target;
- test failure/restart/revoke/field cases. The SenseCAP forwarded the physical
  frames but did not itself supply the application acknowledgement.
