...
OT-123 prepares the five-of-eight Monocypher comparison. OT-124, OT-126, and OT-128 record three subsequent attempts as aborted before an admitted Monocypher result, with every benchmark-touched node exactly restored; Decisions 0059, 0064, and 0066 are consumed. After OT-128, a non-writing reset succeeded on both devices and both USB endpoints returned. The private receipt does not classify the capture failure, so no physical root cause is admitted. Source inspection requires a host/device start-ready handshake, accumulation across partial reads, verified endpoint lifecycle, and privacy-safe failure categories before any new authority. No Monocypher hardware attempt is currently authorized. Entropy,
