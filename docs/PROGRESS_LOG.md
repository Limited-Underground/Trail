...
One freshly authorized two-Heltec attempt completed the three-message radio handshake, full transcript comparison and separate physical-button confirmation on both devices. Both diagnostics were healthy. A reported TX attempted/completed/RX/errors2/2/1/0 and local elapsed778ms; B reported1/1/2/0 and1143ms; both confirmed radio stop. These are local handshake durations, not packet latency. The bench separation was unmeasured; no range or reliability claim follows.
...
Started the additive Heltec radio target and actual channel/driver composition. The Python radio bridge arms B then A, enforces the exact profile, bounded counters/deadlines and confirmed stop, while retaining USB provisioning, full transcript comparison, physical local confirmation and both CLOSED replies. Distinct grant actions separate RF from nonradio trials; restore-only recovery cannot arm RF. Python 57 focused tests pass; actual target/driver startup validation, final matrix and fresh builds remain pending. No physical action, V1 or website credit. See [scope and mandatory preflight](testing/OT-234-PAIR-RADIO-TARGET-2026-09-14.md).

Final software validation passes 987 C++ groups, 26 scalar controls and 60 Python tests, including two generator tests/eight compiled cases, plus actual two-process interoperability; all 969 source pins match. Two fresh radio builds match seven raw artifact pairs; the existing USB regression build passes. Exact private physical proposal preparation is complete without enumeration, hardware access or a grant. Physical approval, RF/headroom acceptance and source publication remain pending. The reused HAL aborts on SDK GPIO/SPI errors, so those paths require external restoration rather than universal in-process CLOSED0 reporting. No V1 or website credit. See the [sanitized proof](../tests/benchmarks/crypto/OT-234-PAIR-RADIO-TARGET-HOST-2026-09-14.json).

### OT-233 Integrate independent endpoints with bounded transport

Started host-only composition of actual independent endpoints with RadioTransport and the experimental probe codec. Frames remain at most 154 bytes without fragmentation; routing IDs do not confer authority. Additive endpoint polling bounds silent loss through existing trusted role/Ready/time guards. Duplicate/order/malformed/short-send/error paths fail closed without retry or membership/traffic APIs. 39 focused actual transport groups and independent review pass. Final validation passes 896 C++ groups, 26 scalar controls, 47 Python tests and actual two-process interoperability; 954 source pins reverify. Both fresh existing-target regression builds match all seven raw artifact pairs. The regression image is distinct from the OT-232 physically accepted image; raw-byte and diff checks pass. The transport cannot cancel already queued public handshake frames; no drain is claimed. Target wiring and physical radio are later gates. No V1 or website change. See [scope and preflight](testing/OT-233-INDEPENDENT-TRANSPORT-2026-09-14.md).

### OT-232 Bound pair startup framing

Prepared the next bounded correction after both OT-231 roles reported stage 19/error 15 before HELLO. The proposal quarantines at most 4096 startup bytes, recognizes DIAG/HELLO only before transition, and sends one guarded LF per fresh host lease; strict post-HELLO behavior remains unchanged. Software application follows verified OT-231 closure. Final validation passes 857 C++ groups including 30 actual-startup cases, 26 scalar controls, 47 Python tests and actual two-process crypto interoperability, with matching source pins. Both fresh builds and isolated preparation pass without hardware access, enumeration or grant issuance. Independent preparation audit passes; independent raw-byte comparison confirms all seven artifact pairs match. Software preparation is complete; physical execution remains pending fresh exact-image authorization. No physical resolution, new hardware authority, V1 or website credit. See [scope](testing/OT-232-PAIR-STARTUP-FRAMING-2026-09-14.md).

Later, one newly authorized OT-232 attempt returned healthy diagnostics from both roles, reached matching full transcripts and reported local confirmation B then A. The user confirmed matching OLED codes and about one-second BOOT holds/releases on both devices. Both session CLOSE replies and passive closure completed; the journal records local_confirmed. Both originals passed controller readback/restoration and reset; custody closed, exit 0, local_confirmed, one candidate attempt and zero recoveries. Independent evidence audit passes all ten capture hashes, authority/source pins and closed custody; original-screen user confirmation remains pending. The earlier startup refusal did not recur in this single trial. This establishes USB evaluation handshake/local decisions, not radio, phone, product membership or V1 acceptance.

### OT-231 Expose bounded pair startup diagnostics

Started the bounded diagnostic correction after OT-230 refused on both devices before comparison. Fixed stage/error OLED and DIAG evidence, host collection from both roles before provisioning, and SDK-fault tests of actual `app_main.cpp` address the missing startup observation boundary. The fresh 852-group compiled matrix includes 25 actual-startup cases and passes with 26 scalar controls. Final 45 Python tests and actual two-process crypto interoperability pass; a final Python-only cleanup guard was validated without changing compiled matrix inputs. Both fresh builds match all seven artifact pairs, and isolated controller preparation passes without hardware access, enumeration or grant issuance. Software preparation is complete; physical execution remains pending fresh exact-image authorization. The physical root cause is unknown, historical executed evidence is preserved, and no additional hardware authority, V1 credit or website change is created. See [scope and mandatory preflight](testing/OT-231-PAIR-STARTUP-DIAGNOSTICS-2026-09-14.md).

Later, one OT-231 physical diagnostic attempt reported stage 19/error 15/cleanup 1 from both roles, matching user-observed OLED S19 E15. The first board was observed before bridge open while the second was being written; the second board was later reported to match. This identifies invalid USB input in the normal application loop before HELLO, with earlier checked initialization complete; exact byte/emitter unknown. Original restoration and independent audit pass all ten span hashes; both original resets are verified and custody is closed. The controller exited 0 with restored=true, observation trial_failed, one candidate attempt and zero recovery attempts. Executed evidence is preserved; no retry or physical correction is claimed.

### OT-230 Prepare the two-device USB confirmation milestone

Added a target with one independent endpoint per device, trusted USB evaluation provisioning, exact three-message relay, OLED transcript comparison and local GPIO0 confirmation. Initial validation passes 48 C++ groups, 9 bridge tests with separate-process real-crypto interoperability and 17 custody coordinator tests. Two fresh builds of the 459,296-byte candidate match all seven artifact pairs; final admission/matrix and physical trial/restoration remain pending. This entry records preparation only: no hardware execution, LoRa/phone acceptance, product membership, V1 credit or website change. OT-229 and OT-230 remain one combined publication-pending batch. See [evidence and planned local actions](testing/OT-230-TWO-DEVICE-BENCH-2026-09-14.md).

Later on 2026-09-14, final admission and the fresh 827-group matrix, 26 scalar controls and 40 Python tests passed. One physical trial then failed before comparison: both actual OLEDs showed `ROLE ? / REFUSED`; the bridge failed and no human confirmation was requested or reached. `ROLE ?` identifies no specific startup or INIT boundary, and the cause remains unknown. The grant is consumed; no retry occurred. Both original applications/full NVS and protected spans passed readback verification, both original resets are verified, and custody is closed. The controller exited 0 with observation `trial_failed`, one candidate attempt and zero recovery attempts; original phone Ready/running UI was not independently observed. The host proof missed actual target startup integration. The next correction is fixed stage/error DIAG and actual `app_main.cpp` SDK-fault tests before another flash. No V1 or website credit; combined publication remains pending.

### OT-229 Integrate independent invitations into endpoints

Bound v2 admission through additive real session, local confirmation owner and endpoint layers. Independent boot generations and clocks now complete endpoint handshake and separate local confirmations. 50 new groups and the fresh 779-group matrix plus 26 controls pass; frozen predecessors remain unchanged. No hardware, V1 credit or website change. See [evidence](testing/OT-229-INDEPENDENT-ENDPOINT-INTEGRATION-2026-09-14.md).
...
- Focused OT-157 suites pass 23/23, OT-156 suites pass 17/17, the frozen OT-153 chain passes 75/75, the authoritative raw-byte audit passes, and the complete Windows Host matrix passes locally with exit 0.
...
- The first correction run 262 passed OTCAI 4/4 and OT-099 5/5 before exposing the same issue at OT-100. A new OTRBC0 audit now checks 69 frozen inputs and their existence, length, digest, EOL, BOM, final newline, and Git policy; focused OT-100 passes 6/6. Run 263 then exposed the separate depth-one checkout omission at the later OT-093 historical gate; complete-history checkout and an early regression now preserve the required OT-092/OT-093 objects, and the complete Windows matrix passes locally. No hardware, phone, firmware, accepted artifact, product capability, score, or website status changes. See [Decision 0073](decisions/0073-freeze-windows-raw-evidence-checkout-bytes.md) and [OT-134 evidence](../tests/hardware/OT-134-2026-08-24.md).
