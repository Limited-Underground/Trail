...
| Latest increment | OT-063 adds a target-linked read-only protected-storage admission probe. Six strict host groups, the deterministic boot self-check at 100/100, five target-only static groups, and two reproducible pinned ESP-IDF v6.0.2 builds pass. The current configuration still fails first at `nvs_encryption_not_configured` before any partition, eFuse, or HMAC read; the 440,240-byte build is not flashed and GATT remains denied. No key, NVS, bond, device, Android, or V1 state changed |
| Proven so far | A 123-executable C++ host matrix, 59 C# loader scenarios, five target-only Heltec admission groups, one exact-profile ESP-IDF target, and the separate six-group OT-063 read-only probe remain accepted. OT-DEV-001 is experimentally flashed with the prior OT-061 image; OT-063 is build-linked and not flashed. Android passes 136 JVM tests across thirteen suites, clean lint, debug assembly, and manifest inspection; its 12,236,702-byte APK has SHA-256 `0E3A9C91E4AB68F0D6C45FB1D5A613CED7EE33154155AB2D0E76CE453F52918E` and owner-accepted physical API 33 launch/reopen evidence. There is still no admitted protected target storage/key/bond backend, GATT exchange, successful app/device authorization or Ready state, real coordinate source, LoRa/GNSS/display runtime, release signing/store package, power/endurance, field, regulatory, or support evidence |
