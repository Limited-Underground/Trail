...
| Latest increment | OT-134 permanently corrects Windows Host validation checkout drift without changing accepted bytes or hashes. The last green was OT-092 run 210; OT-093 run 211 was the first red with the older OTCBL0 signature; exact OTCAI failure began at OT-098 run 217. Run 262 proved the first OTCAI and OT-099 correction before exposing the same issue at OT-100. Deterministic LF, CRLF, and exact mixed-byte policy now covers all known raw-bound inputs; a 69-input audit and focused OT-100 6/6 pass. Run 263 then exposed the separate depth-one checkout omission at the later OT-093 historical gate; complete-history checkout and an early regression now preserve the required OT-092/OT-093 objects, and the complete Windows host matrix passes locally. No firmware, hardware, phone, product capability, score, or public website status changes. See [Decision 0073](docs/decisions/0073-freeze-windows-raw-evidence-checkout-bytes.md) and [OT-134 evidence](tests/hardware/OT-134-2026-08-24.md). |
