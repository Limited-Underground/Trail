...
| Latest increment | The Windows 95-style loader now preserves the same selected and keyboard-focused wrapped device, vertical scroll position, UI Automation item name, bundle availability, disabled Flash boundary, and zero-ready summary through a live in-window classic → deterministic high-contrast → classic resource transition. Pixel review found the high-contrast focus color initially duplicated the yellow selected outline; focus is now white while selection stays yellow. Physical key injection, Narrator, and actual Windows theme switching remain separate gates |
