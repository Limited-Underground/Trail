...
| Latest increment | OT-033 defines the first production-facing, brand-neutral Android/device seam: exact bounded `OTB0/v0` capability and `OTC0/v0` fragment codecs plus a one-controller session guard that requires encrypted-link, authenticated-bond, and separate application-authorization evidence. Fifteen focused groups and 100/100 repeats cover strict parsing, exclusive controller/session binding, monotonic non-wrapping session/request IDs, duplicate/no-reapply classification, stale and direction rejection, and redacted status. OT-034 separately adds a strictly build-only ESP-IDF v6.0.2 `esp32s3` Heltec V4 bench-candidate scaffold. Its fixed startup line and recurring heartbeat read no device-specific identifier; the application does not initialize, access, or bind radio, BLE, GNSS, storage, display, or power, and device writing is unauthorized. The native build/size gate, exact artifact hashes, 8.05-second incremental rerun, and complete 113-executable host/publication-safety gate pass. No device was accessed or flashed. The generic 2 MB/DIO/80 MHz image header is not the observed 16 MB/2 MB-PSRAM Heltec profile, and ESP-IDF runtime logs remain unreviewed. OT-033 is `done`, OT-034 remains `partial`, and V1 is 30% |
| Proven so far | A 113-executable C++ host matrix, including 15 BLE companion codec/session-admission groups, 59 C# loader document/identity/accessibility/production-window refresh/selection/failure-recovery/forward-and-reverse-keyboard/automation-peer/automation-scroll/automation-heading/high-DPI/resize/contrast-theme/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, three build-only Heltec target admission groups, and the focused simulator gate of 33 Core, 23 Windows bridge, 15 private USB-helper, 10 native-protocol, and 11 integrated WPF groups, all warning-free. A pinned ESP-IDF v6.0.2 `esp32s3` build also proves only the bounded generic OT-034 native candidate and exact artifacts; it does not prove the received Heltec profile or physical runtime. There is still no BLE stack, pairing workflow, Android app, production packet, native real radio, physical display, clean machine, installer, or support evidence. Two Heltec clients and one SenseCAP repeater separately completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests; one Wio candidate has partial USB/runtime/configuration plus local and source-free package loader-recognition evidence only |
