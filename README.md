...
| Latest increment | OT-044 adds a fixed-memory one-connection GATT session/indication lifecycle that reserves an exact response path before coordinator mutation. Congestion and guarded re-entry reject without mutation; wrong/stale completions preserve the exact pending response; actual submit failure, negative exact completion, timeout, unsubscribe, or security loss contain and block the session until exact disconnect. OT-045 wires an explicit Local test versus Bluetooth-device choice, Android 12+ Nearby Devices permission UX, and lifecycle-safe scan/select/connect/disconnect controls with no silent fallback. Both remain deliberately non-live: the target does not start or advertise NimBLE, and the Android production security authority is still deny-all. OT-034 remains `partial`, and V1 is 30% |
| Proven so far | A 117-executable C++ host matrix, including 15 BLE session-admission, 13 semantic-payload, 16 companion-request-coordinator, 15 GATT-session-lifecycle groups, and target-local boot self-checks, 59 C# loader scenarios, three build-only Heltec target admission groups, and the focused simulator gate of 33 Core, 23 Windows bridge, 15 private USB-helper, 10 native-protocol, and 11 integrated WPF groups, all warning-free. The Android foundation passes 59 JVM tests, warning-as-error lint, debug APK assembly, and manifest inspection. A pinned ESP-IDF v6.0.2 generic `esp32s3` build links the accepted companion codecs, coordinator, dormant GATT definition, and GATT lifecycle, but none has executed on hardware. There is still no started ESP-IDF NimBLE/controller/GATT service or advertising, approved pairing/application-authorization workflow, real device-authority adapter, production packet/radio target, physical display, clean-machine Android install, release signing/store package, or support evidence. Two Heltec clients and one SenseCAP repeater separately completed limited MeshCore-era bench evidence; one Wio candidate has partial USB/runtime/configuration and loader-recognition evidence only |
