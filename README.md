...
| Latest increment | A shown production window now passes a deterministic 1120×760 → 900×620 → 1120×760 resize transition. The stricter run found that reflow could leave the focused third card below the viewport and drop its focus; resize now re-brings the unchanged current selection into view after layout and restores its focus. The cards move from one row to a two-plus-one wrap and back while selection, public UI Automation name, bounded bundle availability, and disabled Flash state remain intact. Physical resize input, real monitor/DPI transitions, Narrator, installer lifecycle, and clean-machine acceptance remain separate gates |
