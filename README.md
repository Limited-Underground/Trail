...
| Latest increment | OT-118 admits strict five-of-eight host-only Monocypher API/configuration evidence for comparison measurement only. All three API/configuration registries are populated and counts advance to `3/3/0`; Monocypher remains structurally nonselectable, phase 0 still awaits the second-node exact profile, and every retained import/build anchor plus fresh execution authority remains open. No benchmark, device, selection, or score claim changes |
