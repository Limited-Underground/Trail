...
| Latest increment | OT-153 freezes the host-only executable and restoration bundle for the OT-152 Noise XK radio-cost contract: reproducible ESP32-S3 firmware, strict runner, simultaneous two-node coordinator, concrete fail-closed adapter, and exact Trail restoration. It creates no authority and performs no device or radio work; OT-154 requires fresh explicit non-reusable one-attempt authority. No selection, Phase 2 completion, readiness, or score claim changes; V1 remains exact 43.75%/displayed 44%. |
