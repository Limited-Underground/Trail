...
| Latest increment | OT-069 adds an inactive target-local owner for the exact candidate protected-NVS context. Ten strict lifecycle groups and a separate disabled-configuration check prove exact existing-config/open/cleanup ordering, ambiguity containment, and zero native I/O under current configuration. Target admission passes 9/9, and two pinned builds reproduce the same 470,928-byte BIN. The owner is build-compiled but not runtime-injected |
| Proven so far | The prior 123-executable C++ host matrix remains accepted; OT-065 through OT-069 add independently passing protected-store, trusted-authority composition, KV slot-media, target NVS-backend, and inactive context-owner suites. The Heltec target-only admission suite passes nine groups, and the inactive backend/context compile in the pinned ESP-IDF v6.0.2 target toolchain. `OT-DEV-001` still runs the 470,928-byte OT-064 app (SHA-256 `A7D8E672CF9169F1D1D4E86EEFF80399C47A145E7D64904C207DD5F1B23F359B`) with experimentally observed startup/status OLED and privacy-safe BLE advertising. Android passes 136 JVM tests across thirteen suites, clean lint, debug assembly, and manifest inspection. There is still no active protected partition, runtime storage/key/bond backend, independent physical rollback floor, GATT exchange, successful app/device authorization or Ready state, real LoRa/GNSS path, interactive display/input, release signing/store package, power/endurance, field, regulatory, or support evidence |
