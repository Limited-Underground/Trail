...
| Latest increment | OT-054 adds the fixed durable owner/tombstone record, protected-store contract, and private-reference/device-secret PRF boundary required for one-phone authorization to survive reboot. The generic target build-links and self-checks the composition but deliberately denies admission because protected NVS initialization, usable protected keys, a private bond store, and an independent rollback floor are not provisioned. OT-055 moves real Android BLE ownership into an explicit user-started, non-exported connected-device foreground service with `START_NOT_STICKY`; the Activity observes that owner while Local test remains separate, and no claim is automatically retried. The target service/controller/advertiser remain dormant and nothing was flashed, while the APK was not installed or exercised under Android, so this is not live pairing, authorization, service-lifecycle, or Ready evidence. OT-034 remains `partial`, and V1 is 30% |
| Proven so far | A 122-executable C++ host matrix, including 15 BLE session-admission, 13 semantic-payload, 16 companion-request-coordinator, 15 authorized GATT-session-lifecycle, 16 one-phone-authorization, 14 authorization-wire, 17 protected-persistence/private-binding, 20 restricted provisional-GATT, and 10 real-callback-adapter groups plus target-local boot self-checks; 59 C# loader scenarios; three build-only Heltec target admission groups; and the focused simulator gate of 33 Core, 23 Windows bridge, 15 private USB-helper, 10 native-protocol, and 11 integrated WPF groups, all warning-free. OT-054 also passes 100/100 persistence repeats, 100/100 target self-check repeats, and two reproducible pinned ESP-IDF builds. Android passes 124 JVM tests across twelve suites (protocol 3 + 10 + 6 + 10; application 8 + 15 + 17 + 11 + 2 + 21 + 1 + 20), warning-as-error lint, debug APK assembly, and manifest inspection; the latest 9,660,781-byte APK has SHA-256 `33174B72792E2AFC0D03AB52DFAC6613BAE48618BF268C3197D7E04105897722`. Two pinned ESP-IDF v6.0.2 generic `esp32s3` builds reproduce a 175,701-byte image and 175,824-byte BIN with SHA-256 `D39430096B7BEDD0F69D9ECCDE2424EDCD635C0BEA904EB2E4FCA3EEED307080`; the link map retains the persistence, target preflight/self-check, callback adapter, and accepted companion layers. This is build-linked-not-run evidence only. There is still no registered or live provisional NimBLE service, started controller or advertising, admitted protected target storage/key/bond backend, successful app/device authorization or Ready state, Android OS/device/install proof, release signing/store package, production packet/radio target, physical display, or support evidence. Two Heltec clients and one SenseCAP repeater separately completed limited MeshCore-era bench evidence; one Wio candidate has partial USB/runtime/configuration and loader-recognition evidence only |
