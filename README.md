...
| Latest increment | The Windows utility now compares an inspected bundle with the selected device through exact structured fields only. Runtime family labels never count as a match; all three connected radios remain unmatched because no authoritative received-unit profile exists. Even a future exact match remains separate from release admission and Flash permission |
| Proven so far | A 111-executable C++ host matrix, 48 C# loader document/identity/accessibility/refresh/selection/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests |
