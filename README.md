...
| Latest increment | OT-117 admits complete eight-of-eight host-only libsodium API/configuration evidence, including a bounded benchmark-only Noise XK composition adapter. Source/API-configuration/import counts advance to `3/2/0`, but Monocypher API/configuration, every retained import/build anchor, the second-node exact profile, and fresh execution authority remain open. No benchmark, device, selection, or score claim changes |
