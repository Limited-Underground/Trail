...
| Latest increment | OT-150 freezes the host-only, fixed-version mbedTLS/PSA candidate/control executable and resource bundle. Four fresh cache-disabled ESP-IDF v6.0.2 builds reproduce per side with zero compiler warnings; the admitted matched-resource result records linked flash 245,466 versus 145,834 bytes (`+99,632`) and static RAM 50,187 versus 49,035 bytes (`+1,152`). Fresh explicit authority is still required before hardware. No radio, selection, Phase 2 completion, readiness, or score change is claimed; V1 remains exact 43.75%/displayed 44%. |
