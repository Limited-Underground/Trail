...
| Latest increment | OT-048 freezes a fixed brand-neutral phone-authorization wire and response tracker: exact claim Start/Pending/terminal records, dedicated `OTC0` kinds, one opaque non-secret 128-bit device correlation, exact provisional generation/session/exchange/purpose binding, and exact-close-only connection release. OT-049 mirrors those bytes and tracker rules in Kotlin. Both remain default-disabled codec/tracker work: current `OTB0/v0` advertises no claim capability, the existing GATT path circularly requires application authorization before Protocol Info/session negotiation, and no provisional transport, phone, or device access exists. The generic target was rebuilt only because already-linked shared sources learned the three kinds and explicit normal-path rejection; the new authorization wire/tracker source remains host-only. OT-034 remains `partial`, and V1 is 30% |
| Proven so far | A 119-executable C++ host matrix, including 15 BLE session-admission, 13 semantic-payload, 16 companion-request-coordinator, 15 GATT-session-lifecycle, 16 one-phone-authorization, and 14 authorization-wire groups plus target-local boot self-checks, 59 C# loader scenarios, three build-only Heltec target admission groups, and the focused simulator gate of 33 Core, 23 Windows bridge, 15 private USB-helper, 10 native-protocol, and 11 integrated WPF groups, all warning-free. The Android foundation passes 77 JVM tests across eight suites, warning-as-error lint, debug APK assembly, and manifest inspection; the latest debug APK is 9,627,825 bytes with SHA-256 `967FCD7A032ECED63789378F5B3C0F6AC86D06CE9CF3B6B16205E7C49B8093A3`. Two pinned ESP-IDF v6.0.2 generic `esp32s3` rebuilds reproduce a 157,957-byte image and 158,080-byte BIN after already-linked protocol/semantic/coordinator sources gained authorization-kind recognition and normal-path rejection. The authorization wire/tracker source is absent from the link map, and no target code admits a provisional session. Nothing in OT-046/048/049 executed on hardware. There is still no negotiated authorization-claim capability, provisional GATT session, started ESP-IDF NimBLE/controller/GATT service or advertising, trusted BLE bond-store/persistence/physical-input adapter, production packet/radio target, physical display, clean-machine Android install, release signing/store package, or support evidence. Two Heltec clients and one SenseCAP repeater separately completed limited MeshCore-era bench evidence; one Wio candidate has partial USB/runtime/configuration and loader-recognition evidence only |
