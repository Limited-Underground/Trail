...
| Latest increment | The Windows 95-style utility now has deterministic rendered-layout evidence from its real production XAML at 1600×900 and the 900×620 minimum. Pixel review fixed a transparent content background and made device cards wrap instead of clipping; the third card remains reachable by vertical scrolling and disabled Flash labels remain readable. This does not yet prove Narrator, high-DPI, repeated-refresh, or clean-machine acceptance |
| Proven so far | A 111-executable C++ host matrix, 48 C# loader document/identity/accessibility/refresh/selection/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, deterministic desktop/minimum/scrolled loader renders, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests |
