...
| Latest increment | The Windows utility now requires one explicit current-device selection before local bundle inspection. Selection stores only the reduced in-memory card reference, is cleared by every refresh, invalidates any older bundle result, and never grants Flash permission. Exact-device matching, production signer trust, admission, and every writer remain blocked |
| Proven so far | A 111-executable C++ host matrix, 47 C# loader document/identity/accessibility/refresh/selection/snapshot-binding/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests |
