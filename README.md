...
| Latest increment | OT-142 identifies the failed OT-140-authorized attempt as a benchmark-only RFC 8032 seed transcription defect, corrects only that seed in a separate successor target, and adds a regression test against the real pinned Monocypher library. The touched device was restored exactly and both devices returned to Trail; no benchmark result was admitted. Two fresh builds match and the complete Windows host matrix passes. No hardware attempt is currently authorized, and V1 remains exact 43.75%/displayed 44%. |
