...
- **Archive backpressure no longer requires record loss:** the
  [bounded RAM outbox](docs/location/BREADCRUMB_ARCHIVE_OUTBOX_V0.md) holds 16
  exact `OTBA/v0` records in strict FIFO order, never overwrites a full queue,
  and retains the head on not-ready, remote rejection, or failure. Only an
  explicit `durable_ack` result permits exact-head removal; a post-ACK local
  mismatch latches upload closed. Ten groups plus 100/100 repeats pass. This is
  volatile host behavior—not persistent storage, an endpoint, authenticated
  transport, a server durability result, or lost-device recovery.
...
- **Offline maps now have a legal and technical gate:** the dated
  ...
  pass. A new
  [trusted runtime transition coordinator](docs/maps/OFFLINE_MAP_SELECTOR_TRUSTED_TRANSITION_COORDINATOR_V0.md)
  now derives both transition generations from the protected source, keeps
  ...
  for fresh-boot reconciliation. Eleven groups pass. A new
  [protected service-reseed coordinator](docs/maps/OFFLINE_MAP_SELECTOR_TRUSTED_RESEED_COORDINATOR_V0.md)
  now derives the reviewed floor from protected history before selector access,
  consumes the exact-bound permit, verified-clears and saves the replacement
  privately, advances and exactly reads back trust, and only then publishes the
  recovered map. Initial source failure leaves selector storage untouched;
  post-save trust uncertainty remains ambiguous-mapless for fresh-boot
  reconciliation. Twelve groups pass. A new
  [reset/replacement policy](docs/maps/OFFLINE_MAP_SELECTOR_RESET_REPLACEMENT_POLICY_V0.md)
  now keeps ordinary factory reset outside both map persistence domains,
  permits selector reseed only through the existing authorized path while
  protected history is intact, and refuses to reinterpret missing same-device
  history as first use. Same-device protected-source replacement routes only
  to future external recovery authority; independently established blank new
  hardware routes to fresh-domain commissioning, while retained-selector
  import is rejected. Ten groups pass. A separate
  [protected-domain authorizer](docs/maps/OFFLINE_MAP_SELECTOR_DOMAIN_AUTHORIZATION_V0.md)
  ...
  restart without a rollback or reapply. Fourteen groups pass. A new
  [active trust-domain boot coordinator](docs/maps/OFFLINE_MAP_SELECTOR_DOMAIN_BOOT_V0.md)
  then restores only an exact stable selector after matching the active domain
  record and protected domain/generation, rereads all three durable owners, and
  publishes only when they remain unchanged. It performs no write, repair, or
  protected mutation; thirteen groups pass. A new
  [domain-aware candidate entry](docs/maps/OFFLINE_MAP_SELECTOR_DOMAIN_CANDIDATE_V0.md)
  now persists a private trial selector, advances the exact protected domain,
  ...
  owners before publication. It now also restores canonical active checkpoints
  left by an interrupted runtime transition. Fourteen groups pass. A new
  [domain-aware runtime transition coordinator](docs/maps/OFFLINE_MAP_SELECTOR_DOMAIN_TRANSITION_V0.md)
  keeps volatile health progress behind exact three-owner rechecks and commits
  promotion, deadline fallback, valid fallback completion, and previous-package
  cleanup in selector-before-protected-before-domain order. Interrupted
  promotion and uncertain domain commit recover through the same boot boundary;
  invalid fallback evidence retains protected/domain history and requires
  service. Eleven groups pass. No protected target backend, concrete
  credential/continuity/entropy evidence, ESP-IDF target composition, physical
  package operation, or physical durability result exists.
  ...
  92-executable host matrix. It contains no ESP-IDF backend, partition table,
...
- **Replay checkpoints are now group-bound on disk:** fixed-size
  ...
  An [NVS-ready key/value adapter](docs/persistence/DUPLICATE_CHECKPOINT_KV_TARGET_ADAPTER_V0.md)
  now fixes `ot_state` / `ot_replay` / `ods_dup_a|b`, exact 704-byte values,
  explicit commit after write/erase, idempotent missing-key erase, and restart
  discovery after an applied-then-failed commit. Nine groups and 100/100
  focused repeats pass; protected ESP-IDF storage, target locking, physical
  atomicity/endurance, authentication, and rollback protection remain open.
...
- **OpenTrail validation:** [GitHub Actions](https://github.com/nbjelanovic/OpenTrail/actions/workflows/host-validation.yml)
  builds six verifier/planning/operator CLIs and runs all 92 C++ test
  ...
  admission, opt-in breadcrumb archive sessions and bounded archive outbox,
  loss-aware priority-to-delivery handoff, checked-time outbound
