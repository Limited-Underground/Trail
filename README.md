...
| Latest increment | OT-121 partially exercises the bounded Phase 2 authority: both anonymous experimental nodes pass the libsodium local-primitives slice at 7/7 operations with 700 data-cache-conditioned and 700 warm samples per node, then restore exactly to Trail. The receipt records `phase_two_complete=false` and `radio_used=false`; remaining candidate, Noise XK, radio, resource, and independent-admission gates stay open. No selection, support, compatibility, regulatory, production, or score claim changes |
