...
| Latest increment | OT-122 continues the bounded Phase 2 session: both anonymous experimental nodes pass all 8/8 libsodium operations, including complete benchmark-only Noise XK, with 800 data-cache-conditioned and 800 warm samples per node. Both report 0 bytes peak dynamic heap, 4,312 bytes maximum stack use, and 0 watchdog resets, then restore exactly to Trail. Other candidates, radio, matched linked-flash/static-RAM admission, Phase 3, and selection remain open; no support, compatibility, regulatory, production, or score claim changes |
