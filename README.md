...
| Latest increment | OT-101 remains partial. OT-101A adds target-neutral host-only adapters under `tests/host_support` from the existing power assessment and BLE runtime status into the accepted compact footer. Ten adapter groups and the existing twelve footer groups pass: valid battery percentages reconstruct their original sample time from the assessment time, the separate render time controls freshness, delayed samples expire, and regressive clock inputs fail closed. BLE maps only to `-`, `S`, `A`, `C`, `R`, or `E`; composition deliberately leaves `GPS:--` and the direction field blank. The adapters remain outside the firmware tree and add no target renderer/OLED, sensor, BLE transport, radio, or real-traffic binding. No firmware or target build, device access, flash, telemetry, traffic, physical-display, support, regulatory, or score claim was added |
