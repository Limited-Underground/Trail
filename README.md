...
| Latest increment | OT-030B replaces the first simulator's invented WPF workflow with the shared C++ `PortableUiShell` and fixed renderer-neutral plan, while a bounded Windows/Python bridge adds passive USB-candidate discovery, explicit selection/open, private one-client assignment, and deterministic teardown. OT-030C completes the bounded host/shared-model Inbox/Outbox/detail/fixed-template Compose/critical-ACK surface and native protocol v2 without enlarging the compact 24-byte `UiFrame`; message/text presentation lives in a shell-owned sidecar for the exact offered frame. The warning-free focused gate passes 33 Core, 23 Windows bridge, 15 private helper, 10 native-protocol, and 11 integrated WPF groups, including two native LCDs crossing one exact local-loopback template message. The Home, Compose, post-send, and inbound-detail renders also passed representative visual review, and the complete expanded 112-executable host/publication-safety gate exits 0. Local-loopback and USB-test evidence is explicitly not OpenTrail packet, radio, authenticated-peer, physical-LCD, or supported-hardware evidence. Parent OT-030 remains `partial`, V1 stays at 29%, and remote publication verification is pending. Decision 0007 retains future Android-companion and standalone-touchscreen tracks over this shared behavior without changing the standalone V1 |
| Proven so far | A 112-executable C++ host matrix, 59 C# loader document/identity/accessibility/production-window-refresh/selection/failure-recovery/forward-and-reverse-keyboard/automation-peer/automation-scroll/automation-heading/high-DPI/resize/contrast-theme/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, plus the focused simulator gate of 33 Core, 23 Windows bridge, 15 private USB-helper, 10 native-protocol, and 11 integrated WPF groups, all warning-free. Simulator evidence covers shared C++ frame/action authority, exact protocol-v2 parsing, passive USB discovery, explicit fail-closed open, bounded local-loopback messaging/critical acknowledgement, deterministic 100%/125%/150%/200% renders, and representative visual review; it does not prove a production packet, real radio, physical display, target firmware, clean machine, installer, or support. Two Heltec clients and one SenseCAP repeater separately completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests; one Wio candidate has partial USB/runtime/configuration plus local and source-free package loader-recognition evidence only |
