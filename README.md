...
| Latest increment | OT-065 adds a backend-neutral two-slot authorization-store coordinator and an exact inactive Heltec protected-storage candidate plan. Twelve strict host groups prove floor-first restore, inactive-slot preparation, exact readback, monotonic-floor advancement, reboot reconciliation, and fail-closed corrupt, stale, conflicting, and ambiguous states. The active partition table, sdkconfig, device image, GATT authority, and physical device remain unchanged |
| Proven so far | The prior 123-executable C++ host matrix remains accepted; OT-065 adds one independently passing 12-group protected-store executable and raises the Heltec target-only admission suite to eight groups. `OT-DEV-001` still runs the 470,928-byte OT-064 app (SHA-256 `A7D8E672CF9169F1D1D4E86EEFF80399C47A145E7D64904C207DD5F1B23F359B`) with experimentally observed startup/status OLED and privacy-safe BLE advertising. Android passes 136 JVM tests across thirteen suites, clean lint, debug assembly, and manifest inspection. There is still no admitted protected target storage/key/bond backend, independent physical rollback floor, GATT exchange, successful app/device authorization or Ready state, real LoRa/GNSS path, interactive display/input, release signing/store package, power/endurance, field, regulatory, or support evidence |
