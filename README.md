...
| Latest increment | OT-036 adds the first buildable native Android foundation: a pure Kotlin, brand-neutral `OTB0/v0` and `OTC0/v0` codec over the shared C++ golden bytes, plus a Jetpack Compose shell with explicit Disconnected, Selecting, Connecting, Connected, and Failed states over a deterministic fake transport. Six protocol and four application-state tests, warning-as-error lint, debug assembly, and an independent APK/manifest audit pass. OT-037 separately links the accepted C++ companion framing and semantic codecs into the build-only Heltec ESP32-S3 candidate and checks exact fixed vectors before its heartbeat path. The new image builds reproducibly but is explicitly `BUILD-LINKED-NOT-RUN` and `NOT-FLASHED`. OT-035, OT-036, and the bounded OT-037 integration are `done`; OT-034 remains `partial`, and V1 is 30% |
| Proven so far | A 114-executable C++ host matrix, including 15 BLE companion codec/session-admission and 13 semantic-payload groups, 59 C# loader scenarios, three build-only Heltec target admission groups, and the focused simulator gate of 33 Core, 23 Windows bridge, 15 private USB-helper, 10 native-protocol, and 11 integrated WPF groups, all warning-free. The separate Android foundation passes 10 JVM unit tests, warning-as-error lint, debug APK assembly, and manifest inspection. A pinned ESP-IDF v6.0.2 `esp32s3` build links the accepted companion codecs and produces a 145,657-byte generic candidate image with stable hashes; the self-check has not executed on hardware. There is still no live BLE adapter, approved pairing/application-authorization workflow, lifecycle-safe Android session owner, production packet/radio target, physical display, clean-machine Android install, release signing/store package, or support evidence. Two Heltec clients and one SenseCAP repeater separately completed limited MeshCore-era bench evidence; one Wio candidate has partial USB/runtime/configuration and loader-recognition evidence only |
