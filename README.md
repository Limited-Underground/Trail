...
| Latest increment | OT-020A adds a host-only lifecycle/recovery coordinator contract for a future supervised Wio GNSS pass. Fifteen synthetic injected-adapter groups and the complete host gate pass. The contract requires exact target/policy/GPS-off preconditions, durable recovery authority before enable, acknowledgement/readback, presence-only telemetry, verified off restoration and post-restore binding, expected-record journal updates, and settled same-boot/zero-pending/no-transmission counters; recovery-only mode cannot enable GPS. No live adapter/device I/O occurred, the entire live GNSS phase remains open, OT-020 stays `partial`/`experimented`, and the missing shipping/pre-write state remains explicit |
