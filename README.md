...
| Latest increment | The accessibility-hardened Windows 95-style loader now has a fresh verified source-free package. Its 464 manifest-bound files exclude source, firmware, writers/recovery tools, keys, and debug payload; both the builder and a separate clean extractor verified every hash and launched the exact self-contained `win-x64` executable. The package remains inspection-only, local, unsigned, and not a public release; installer lifecycle and clean-machine acceptance remain separate gates |
