...
| Latest increment | OT-143 freezes the exact OT-142 corrected-target six-file tuple, pinned build evidence, frozen transport/parser, fresh restoration-safe coordinator/private state, concrete adapter, and exact Trail restoration image into one immutable executable bundle. A fresh explicitly approved workspace-local non-reusable authority permits exactly one two-node application-only attempt and remains unexecuted. No hardware, result, radio, selection, Phase 2 completion, or score claim changes; V1 remains exact 43.75%/displayed 44%. |
