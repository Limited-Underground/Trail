...
| Latest increment | The production utility now exposes a real heading hierarchy: one level 1 utility title, level 2 inspection/bundle/safe-mode headings, and one level 3 public name for each current device. Initial, selected, and refreshed states pass both in-process and source-free external UI Automation checks. Reverse WPF focus traversal also passes with and without a selected device, skipping every disabled bundle/Flash action without creating an implicit selection. Narrator/Braille heading commands, physical Shift+Tab, installer lifecycle, and clean-machine acceptance remain separate gates |
| Proven so far | A 111-executable C++ host matrix, 58 C# loader document/identity/accessibility/production-window-refresh/selection/forward-and-reverse-keyboard/automation-peer/automation-scroll/automation-heading/high-DPI/resize/contrast-theme/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, deterministic classic, 125%/150%/200%, contrast, and in-window theme-transition loader renders plus shown resize-state acceptance, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests |
