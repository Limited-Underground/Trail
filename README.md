...
| Latest increment | A shown-window contrast matrix found that local button color values could override the shared disabled-state trigger. Production buttons now inherit the single semantic template, and every disabled instance is measured against its actual rendered surface in classic and deterministic high contrast across ready/unselected, ready/selected, and busy-refresh states. Live Windows contrast-theme switching, pixel antialiasing, Narrator/Braille UX, installer lifecycle, and clean-machine acceptance remain separate gates |
