...
| Latest increment | OT-137 records the single OT-136-authorized attempt as a fail-closed abort. Node A passed benchmark readback; two 512-byte reads observed 1,024 bytes, 11 complete opaque pre-READY records were counted within the cumulative complete-record allowance, and remaining partial pre-READY data crossed the 512-byte bound before any frame. Node A restored/readback/reset exactly to Trail, Node B was never benchmark-written after preflight and remained on Trail, and the owner confirmed both Trail logos. The authority is consumed and non-reusable. OT-137 tests pass 7/7 and the deterministic raw-byte audit passes 82/82. No benchmark result, radio, selection, Phase 2 completion, support, regulatory, production, end-to-end, or score claim is added. V1 remains exact 43.75%/displayed 44%. See [Decision 0076](docs/decisions/0076-record-ot137-ot136-execution-abort.md), [OT-137 evidence](tests/hardware/OT-137-2026-08-25.md), and the [sanitized abort receipt](tests/benchmarks/crypto/OT-137-OT005-MONOCYPHER-EXECUTION-ABORT-RECEIPT-V0.json). |
