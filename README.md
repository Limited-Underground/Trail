...
| Latest increment | OT-067 adds the exact protected key/value slot-media layer beneath the two-slot authorization store. Ten strict groups prove fixed `ot_auth` bindings, exact 32-byte values, missing-key handling, commit durability, reentry containment, rotation, reboot restore, and fail-closed malformed, rolled-back, prepared-ahead, and ambiguous media. The backend remains injected and the Heltec runtime remains unchanged |
| Proven so far | The prior 123-executable C++ host matrix remains accepted; OT-065, OT-066, and OT-067 add independently passing 12-group protected-store, 8-group trusted-authority composition, and 10-group KV slot-media executables, while the Heltec target-only admission suite remains green at eight groups. `OT-DEV-001` still runs the 470,928-byte OT-064 app (SHA-256 `A7D8E672CF9169F1D1D4E86EEFF80399C47A145E7D64904C207DD5F1B23F359B`) with experimentally observed startup/status OLED and privacy-safe BLE advertising. Android passes 136 JVM tests across thirteen suites, clean lint, debug assembly, and manifest inspection. There is still no admitted protected target storage/key/bond backend, independent physical rollback floor, GATT exchange, successful app/device authorization or Ready state, real LoRa/GNSS path, interactive display/input, release signing/store package, power/endurance, field, regulatory, or support evidence |
