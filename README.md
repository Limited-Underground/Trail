...
| Latest increment | OT-030A adds a laptop-only dual virtual-LCD simulator shell. Two independent .NET 8 WPF windows use separate bounded Client A/Client B state and two allowlisted in-memory simulated companions; 17 core and 10 WPF groups pass, including 100/100 core repeats, 100 serialized Connect/Reconnect races, and close-time draining of an in-flight service tick. One deterministic home render received visual review, and the complete local host gate exits 0. Visible copy separates queued, local-bridge-accepted, peer-simulator-observed, and peer-simulator-acknowledged states while denying radio or physical-LCD proof. There is no USB discovery/open, MeshCore adapter, real LoRa, physical display, installer, or support claim, so OT-030 remains `partial` and V1 stays at 29%. Decision 0007 records future Android-companion and self-contained-touchscreen tracks over shared repository contracts without changing the standalone V1 |
| Proven so far | A 111-executable C++ host matrix, 59 C# loader document/identity/accessibility/production-window-refresh/selection/failure-recovery/forward-and-reverse-keyboard/automation-peer/automation-scroll/automation-heading/high-DPI/resize/contrast-theme/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, 17 simulator-core and 10 simulator-WPF groups, simulator-core 100/100 repeats and 100 serialized Connect/Reconnect races, deterministic simulator 100%/125%/150%/200% bitmap checks plus one visually reviewed 100% home render, deterministic loader classic/scaled/contrast/theme renders and shown resize-state acceptance, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests; one Wio candidate has partial USB/runtime/configuration plus local and source-free package loader-recognition evidence only |
