...
| Latest increment | OT-114 admits the exact two-node US915 close-bench direct-radio result: 242/242 probe/data frames and 240/240 acknowledgements reconcile with zero loss, duplication, corruption, or unexpected traffic; exact 163- and 255-byte timeout bounds, 256-byte local rejection, signal/RTT metrics, and restart profile retention pass. Only `direct_radio_mtu_phy_region_unresolved` closes; readiness does not advance until a successor decision and executable benchmark plan are accepted |
| Proven so far | The final 216,112-byte diagnostic image passes the ESP-IDF 6.0.2 build and 9 focused source groups. Both V4.2 nodes used the same image and exact commanded 915 MHz/BW125/SF7/CR4-5/explicit-header/CRC/LDRO-off/2 dBm profile before and after restart. The run proves only the admitted close-bench region/MTU/full-PHY requirement; it does not prove EIRP, regulatory compliance, range, compatibility, support, secure LoRa, or production readiness. No score changes: Android remains 60%, V1 exact 43.75%/displayed 44%, the historical baseline exact 31.75%/displayed 32%, and V1.5 and V2 remain unmeasured |
