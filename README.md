...
| Latest increment | OT-140 accepts an immutable executable bundle around the exact OT-139 six-file quiet-target tuple, frozen OT-135 runner/parser boundary, fresh restoration-safe coordinator/private namespace, concrete adapter, and exact Trail restoration image. A fresh explicit authority permits exactly one two-node application-only attempt from this workspace, is non-reusable and consumed on success or abort, and remains unexecuted. Normal ROM logging remains unchanged and endpoint silence is not claimed. No hardware, benchmark result, radio, selection, Phase 2 completion, or score claim is added. V1 remains exact 43.75%/displayed 44%. See [Decision 0079](docs/decisions/0079-freeze-ot140-quiet-target-bundle-and-authority.md) and [OT-140 evidence](tests/hardware/OT-140-2026-08-25.md). |
