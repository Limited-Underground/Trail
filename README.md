...
| Latest increment | The Windows 95-style loader now has production-window keyboard and UI Automation acceptance. Tab follows Refresh → one current device → bounded bundle selection and skips disabled Flash; native arrows follow the wide/wrapped card geometry; explicit Home/End always reach the true first/last candidate, including the wrapped minimum layout. Actual list-item peers expose validated public summaries and blocker help, while changing status peers expose their current visible message. Physical key injection, Narrator, and live Windows theme switching remain separate gates |
| Proven so far | A 111-executable C++ host matrix, 53 C# loader document/identity/accessibility/production-window-refresh/selection/keyboard/automation-peer/high-DPI/contrast-theme/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, deterministic classic, 125%/150%/200%, and contrast loader renders, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests |
