...
| Latest increment | OT-130 freezes the exact OT-129 firmware, transport, parser, restoration-safe two-node coordinator, and Trail restoration image into one immutable bundle. Two fresh cache-disabled ESP-IDF 6.0.2 builds reproduced an identical six-file tuple, coordinator and authority tests each pass 11/11, and a fresh non-reusable one-attempt authority is accepted but not executed. No device, flash, benchmark, radio, selection, Phase 2 completion, or score claim is added; V1 remains exact 43.75%/displayed 44% |
