...
| Latest increment | OT-064 adds the first physically accepted target-local peripheral binding: a 128 x 64 Trail startup/status OLED on `OT-DEV-001`. One bounded factory-app-only write passed exact read-only verification; the owner observed the recognizable Trail logo followed by `BLE ADVERTISING`; boot self-checks, four USB heartbeats, and one privacy-safe Android exact-service scan passed. No connection, pairing, GATT exchange, LoRa, GNSS, full display UI, or unit-2 action occurred |
| Proven so far | A 123-executable C++ host matrix, 59 C# loader scenarios, the accepted OT-063 six-group read-only storage probe, three focused display-owner groups, seven target-only Heltec admission groups, and two reproducible pinned ESP-IDF v6.0.2 OLED builds pass. `OT-DEV-001` runs the 470,928-byte OT-064 app (SHA-256 `A7D8E672CF9169F1D1D4E86EEFF80399C47A145E7D64904C207DD5F1B23F359B`) with experimentally observed startup/status OLED and privacy-safe BLE advertising. Android passes 136 JVM tests across thirteen suites, clean lint, debug assembly, and manifest inspection. There is still no admitted protected target storage/key/bond backend, GATT exchange, successful app/device authorization or Ready state, real LoRa/GNSS path, interactive display/input, release signing/store package, power/endurance, field, regulatory, or support evidence |
