...
| Latest increment | Decision 0028 defers rollback-protected companion authorization beyond the current Heltec V1, and OT-085 build-tests a non-privileged fixed read-only BLE link-info characteristic plus a bounded `BLE CONNECTED` lifecycle without opening protected controls |
| Proven so far | The 148-executable native C++ host matrix passes; OT-085 adds a fixed privacy-safe 16-byte public BLE read and a host-tested 15-second single-link window with bounded termination, while every protected characteristic, authorization claim, and normal command remains closed. Two identical pinned ESP-IDF v6.0.2 builds and eleven Heltec target-admission groups pass; the new image has not been flashed or physically connected. OT-074 physically verified the exact installed partition table and blank protected-storage source region, OT-076 retained the exact installed application privately for recovery, and OT-077 froze a fail-closed recovery route. OT-083 and OT-084 reject both on-chip rollback-floor candidates, so secure ownership and protected control are deferred to future hardware rather than weakened on the current target. `OT-DEV-001` still runs the separately accepted OT-064 app with experimentally observed startup/status OLED and privacy-safe BLE advertising. Android passes 136 JVM tests across thirteen suites, clean lint, debug assembly, manifest inspection, bounded physical install/lifecycle/artwork observations, and exact-service discovery. Physical OT-085 flash/link/read/display acceptance, real LoRa/GNSS, complete-client recovery, release signing/store packaging, power/endurance, field, regulatory, and support evidence remain open |
