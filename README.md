...
| Latest increment | The production Windows 95-style window now passes three controlled read-only refresh/selection cycles. Every refresh clears the old card selection, disables bundle selection until a current card is chosen, restores Refresh, republishes all three cards, and replaces the stale “waiting for inspection” footer with accurate post-inspection copy. This is production-window state evidence, not real mouse/keyboard or live repeated-refresh acceptance |
| Proven so far | A 111-executable C++ host matrix, 49 C# loader document/identity/accessibility/production-window-refresh/selection/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, deterministic desktop/minimum/scrolled loader renders, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests |
