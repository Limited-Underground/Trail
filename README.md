...
| Latest increment | OT-046 adds a host-tested one-phone authorization authority: a trusted opaque bond identity, explicit 30-second physical claim/revoke/replace/reset windows, boot/session replay resistance, one active controller, and atomic verified persistence with fail-closed uncertainty. OT-047 adds the matching Android authorize-this-phone/replace-lost-phone UX with device-issued bounded tokens, exact result correlation, and uncertainty-safe timeout handling. Both remain non-live: the production Android claim adapter is disabled, firmware authorization is not target-linked, no phone/BLE target was accessed, and no device was written. OT-034 remains `partial`, and V1 is 30% |
| Proven so far | A 118-executable C++ host matrix, including 15 BLE session-admission, 13 semantic-payload, 16 companion-request-coordinator, 15 GATT-session-lifecycle, 16 one-phone-authorization groups, and target-local boot self-checks, 59 C# loader scenarios, three build-only Heltec target admission groups, and the focused simulator gate of 33 Core, 23 Windows bridge, 15 private USB-helper, 10 native-protocol, and 11 integrated WPF groups, all warning-free. The Android foundation passes 66 JVM tests, warning-as-error lint, debug APK assembly, and manifest inspection. A pinned ESP-IDF v6.0.2 generic `esp32s3` build links the accepted companion codecs, coordinator, dormant GATT definition, and GATT lifecycle, but the new authorization authority is host-only and none has executed on hardware. There is still no started ESP-IDF NimBLE/controller/GATT service or advertising, trusted BLE bond-store/persistence/physical-input adapter, production packet/radio target, physical display, clean-machine Android install, release signing/store package, or support evidence. Two Heltec clients and one SenseCAP repeater separately completed limited MeshCore-era bench evidence; one Wio candidate has partial USB/runtime/configuration and loader-recognition evidence only |
