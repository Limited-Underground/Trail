...
| Latest increment | A shown production window now passes an effective-wide → 900×620 minimum → effective-wide resize transition with geometry matched to the host's realized work-area capacity. On the local 1920×1080 display, the exact 1120×760 → 900×620 → 1120×760 run moves the cards from one row to a two-plus-one wrap and back. The stricter run found that reflow could leave the focused third card below the viewport and drop its focus; resize now re-brings the unchanged current selection into view after layout without reclaiming deliberate focus moves. Selection, public UI Automation name, bounded bundle availability, and disabled Flash state remain intact. Physical resize input, real monitor/DPI transitions, Narrator, installer lifecycle, and clean-machine acceptance remain separate gates |
| Proven so far | A 111-executable C++ host matrix, 55 C# loader document/identity/accessibility/production-window-refresh/selection/keyboard/automation-peer/high-DPI/resize/contrast-theme/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, deterministic classic, 125%/150%/200%, contrast, and in-window theme-transition loader renders plus shown resize-state acceptance, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests |
