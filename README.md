...
| Latest increment | OT-151 records the consumed, exactly restored OT-150 mbedTLS/PSA abort and corrects its deterministic benchmark-only X25519 negative-vector error in a separate host-only successor. The required `PSA_ERROR_INVALID_ARGUMENT` check and strict 1,015-frame success contract remain intact; only the nonportable error `output_length == 0` assertion is removed, and canonical early terminal failures now report immediately. No new authority, admitted benchmark result, radio, selection, Phase 2 completion, readiness, or score change is added; V1 remains exact 43.75%/displayed 44%. |
