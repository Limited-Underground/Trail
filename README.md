...
| Latest increment | OT-154 accepts the explicitly approved, host-only, non-reusable authority for exactly one later two-node Noise XK radio-cost attempt bound to the immutable OT-153 bundle. The authority remains unexecuted, is consumed on success or abort, grants no continuation, and changes no selection, Phase 2 completion, readiness, or score claim; V1 remains exact 43.75%/displayed 44%. |
| Proven so far | Both anonymous Heltec test units now run the identical OT-147 experimental image and visibly render live battery estimates and GNSS satellite counts in the compact footer. The battery percentage is approximate and uncalibrated; the GPS value does not prove a GNSS fix, position accuracy, or fix-loss behavior. The earlier OT-115 placeholder photo evidence, OT-114 direct-radio evidence, and OT-146 Monocypher comparison remain accepted as separate time-bounded results. LoRa activity-arrow binding, power endurance, supported hardware, production firmware, and field readiness remain unproven. Android remains 60%, V1 exact 43.75%/displayed 44%, the historical baseline exact 31.75%/displayed 32%, and V1.5 and V2 remain unmeasured |
...
[Decision 0090](docs/decisions/0090-accept-ot154-noise-xk-radio-one-attempt-authority.md), [OT-154 evidence](tests/hardware/OT-154-2026-08-27.md), and the [canonical authority](tests/benchmarks/crypto/OT-154-OT005-LIBSODIUM-NOISE-XK-RADIO-ONE-ATTEMPT-AUTHORITY-V0.json) accept the explicitly owner-approved `OT154NXRA0` authority for the unchanged OT-153 bundle. The 3,330-byte authority has raw SHA-256 `d792bd6814d03a196f74fdda5a675386e74b02e7e65335892a57f51a7dc5beb0` and permits exactly one later two-node application-only radio attempt from this workspace. It is consumed on success or abort, is non-reusable, grants no continuation, and remains unexecuted. OT-155 is the sole permitted execution step. No device, flash, reset, radio, phone, private journal, result, selection, Phase 2 completion, readiness, or score claim changes; no website update is required.
