...
| Latest increment | OT-085A physically accepts one bounded, non-privileged BLE link: the exact OT-085 image was installed and verified, one Android phone read the fixed public value, the OLED showed `BLE CONNECTED`, and disconnect returned the target to `BLE ADVERTISING` without opening protected controls |
| Proven so far | The 148-executable native C++ host matrix passes; OT-085 adds a fixed privacy-safe 16-byte public BLE read and a host-tested 15-second single-link window with bounded termination, while every protected characteristic, authorization claim, and normal command remains closed. OT-085A physically installed and verified that exact image on `OT-DEV-001`, accepted one exact Android 13 public read, observed `BLE CONNECTED`, and accepted the phone-disconnect return plus rediscovery in `BLE ADVERTISING`. Android passes 138 JVM tests across fourteen suites, clean lint, debug assembly, and compilation of the hardened test-only physical harness. OT-074's partition/source proof, OT-076's private OT-064 recovery input, and OT-077's fail-closed recovery route remain historical safeguards; OT-083 and OT-084 still defer secure ownership and protected control to future hardware. The firmware's automatic 15-second termination was not physically exercised. Real LoRa/GNSS, protected authorization, complete-client recovery, release signing/store packaging, power/endurance, field, regulatory, and support evidence remain open |
