...
| Latest increment | OT-108 freezes a versioned per-candidate API/configuration acceptance contract host-only. Each candidate binds to its own OT-107 sdkconfig; complete coverage is required for structural selection eligibility, while strict partial comparison evidence remains nonselectable. Evidence registries remain empty, counts stay `3/0/0`, two blockers remain, and readiness stays blocked |
| Proven so far | OT-108 proves only the corrected evidence/admission boundary; it accepts no API/configuration evidence and authorizes no execution or selection. OT-107 separately proves only the final per-candidate configuration. OT-106 proves deterministic build linkage of `BAT:--% GPS:-- BLE:<S/A/C/R/E>` plus a blank direction field, not live telemetry or a viewed display. No score changes: Android remains 60%, V1 exact 43.75%/displayed 44%, the historical baseline exact 31.75%/displayed 32%, and V1.5 and V2 remain unmeasured |
