...
| Latest increment | The C# and Python loaders now recognize the Wio USB/runtime family. The warning-free 59-group C# suite and three consecutive built-in production refreshes pass with the exact one-Heltec, one-SenseCAP, one-Wio public roster, all three runtime-identified and zero ready to flash. A failure-to-recovery gate found and fixed a collapsed error peer that retained stale assertive content. A replacement 464-file source-free ZIP passed independent hash/extraction/launch verification and three native external UI Automation selection/Refresh/live-event/heading cycles against that exact roster; a non-remediating Defender archive scan found no threats. An independent 96-DPI source-free minimum-window probe then invoked the real offscreen Wio-card `ScrollItem`, preserved the fail-closed state, found no inspection-error before or after, and left no owned process/temp residue. After a verifier-only post-refresh timing race was corrected with a complete settled-state wait, three consecutive hardened OT-019AI runs each passed the exact package and roster across three Refresh cycles, scrolling, no-error, privacy, and cleanup checks. The ZIP remains a local engineering artifact, not a public release |
| Proven so far | A 111-executable C++ host matrix, 59 C# loader document/identity/accessibility/production-window-refresh/selection/failure-recovery/forward-and-reverse-keyboard/automation-peer/automation-scroll/automation-heading/high-DPI/resize/contrast-theme/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, deterministic classic, 125%/150%/200%, contrast, and in-window theme-transition loader renders plus shown resize-state acceptance, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests; one Wio candidate has partial USB/runtime/configuration plus local and source-free package loader-recognition evidence only |
