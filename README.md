...
| Latest increment | OT-062 adds the owner-supplied Limited Underground Trail artwork byte-for-byte to the real Android entry surface with no artificial splash delay. The Android gate passes 136 JVM tests across thirteen suites, clean lint, manifest inspection, and an exact 12,236,702-byte APK; that APK was installed over the prior debug build on one physical Android 13/API 33 phone, cold-launched successfully, and the owner visually accepted the artwork and framing. A Home/reopen check also succeeded. This is Android entry-branding evidence only; OT-061 target/runtime boundaries and the historical 31% calculation are unchanged |
| Proven so far | A 123-executable C++ host matrix, 59 C# loader scenarios, five target-only Heltec admission groups, and one exact-profile ESP-IDF target remain accepted. OT-DEV-001 is experimentally flashed: the single authorized erase/write passed post-write verification, boot self-checks reached the runtime loop, at least two heartbeat records were observed, and one physical Android scan found exactly one compatible service candidate. Android passes 136 JVM tests across thirteen suites, clean lint, debug assembly, and manifest inspection; its 12,236,702-byte APK has SHA-256 `0E3A9C91E4AB68F0D6C45FB1D5A613CED7EE33154155AB2D0E76CE453F52918E`, packages the exact approved Trail artwork, and has physical API 33 launch/reopen plus owner visual acceptance. There is still no admitted protected target storage/key/bond backend, GATT exchange, successful app/device authorization or Ready state, real coordinate source, LoRa/GNSS/display runtime, release signing/store package, power/endurance, field, regulatory, or support evidence |
