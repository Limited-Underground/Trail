...
| Latest increment | OT-138 host-only testing reproduces the complete public OT-137 counter shape with fabricated bytes and classifies the contract conflict: the frozen runner charges already queued startup traffic and post-START preamble to the same 512-byte budget, so two immediately available reads can abort before the 250 ms START retry. The accepted build selects USB Serial/JTAG console plus INFO logging, while application suppression occurs later in `app_main`; the actual OT-137 bytes remain unconfirmed. The next successor is a separately reviewed console-isolated quiet-target build, not a larger or bypassed preamble budget. No hardware, authority, result, radio, selection, Phase 2 completion, or score claim is added. V1 remains exact 43.75%/displayed 44%. See [Decision 0077](docs/decisions/0077-classify-monocypher-boot-control-transport-conflict.md) and [OT-138 evidence](tests/hardware/OT-138-2026-08-25.md). |
