...
| Latest increment | Native Windows UI Automation now receives the source-free utility's real live-region event sequence across the process boundary. Three cycles delivered current bundle/selection events, then selection-cleared, inspection-busy, bundle-reset, final zero-ready, and selection-prompt events in the expected order while every Flash action stayed disabled. F5 from a focused card now returns focus to Refresh after either success or failure instead of leaving focus on a destroyed item. Physical input, Narrator, installer lifecycle, and clean-machine acceptance remain separate gates |
