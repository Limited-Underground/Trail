...
| Latest increment | OT-129 accepts the computer-only Monocypher START/READY correction. The isolated successor target waits for exact START, drains READY before the unchanged 1,014-frame stream, and the host retries START until READY, retains partial bytes, verifies endpoint lifecycle, bounds startup chatter, and exposes only privacy-safe failure categories. Focused adversarial/native/static validation and a pinned ESP-IDF 6.0.2 compile-only build pass. No device, flash, benchmark, radio, authority, selection, Phase 2 completion, or score claim is added |
