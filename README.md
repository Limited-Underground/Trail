...
| Latest increment | OT-128 records the OT-127 one-time attempt as aborted after Node A benchmark readback without a validated capture. Node A was restored, readback-verified, and reset to exact Trail; Node B was never benchmark-written. A later non-writing reset succeeded on both devices and both USB endpoints returned. After the immutable receipt was prepared, the owner visually confirmed both Trail displays on. Decision 0066 is consumed, no further Monocypher attempt is authorized, and no result, radio, selection, Phase 2 completion, support, compatibility, regulatory, production, or score claim is added |
