...
| Latest increment | OT-152 freezes the host-only Noise XK radio-cost measurement contract: exact 48/48/64-byte benchmark messages, 160 payload bytes and three frames per successful handshake, role reversal, theoretical airtime references, and one bounded whole-handshake restart per direction. It authorizes no device access or radio execution and admits no physical measurement, selection, Phase 2 completion, readiness, or score change; V1 remains exact 43.75%/displayed 44%. |
