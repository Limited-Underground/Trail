...
| Latest increment | OT-052 build-links the real ESP-IDF NimBLE callback adapter for protected `OTB0/v0.1`, exact registered handles and independently discovered Stream CCCD, fresh device-side link/bond/private-binding checks, claim-only provisional routing, immutable indication correlation, and promotion only after exact Accepted/Replaced confirmation. OT-053 wires the Android production composition in explicit Bluetooth mode: a successful protected Protocol Info read is device-enforced security evidence, bond state alone is not, and there is no fake or local fallback. The target service remains unregistered, its controller and advertiser never start, and nothing was flashed, so this is not live pairing, authorization, or Ready evidence. OT-034 remains `partial`, and V1 is 30% |
| Proven so far | A 121-executable C++ host matrix, including 15 BLE session-admission, 13 semantic-payload, 16 companion-request-coordinator, 15 authorized GATT-session-lifecycle, 16 one-phone-authorization, 14 authorization-wire, 20 restricted provisional-GATT, and 10 real-callback-adapter groups plus target-local boot self-checks; 59 C# loader scenarios; three build-only Heltec target admission groups; and the focused simulator gate of 33 Core, 23 Windows bridge, 15 private USB-helper, 10 native-protocol, and 11 integrated WPF groups, all warning-free. OT-052 also passes 100/100 adapter repeats, 100/100 target self-check repeats, and a pinned NimBLE teardown-order check. Android passes 101 JVM tests across ten suites (protocol 29; application 8 + 15 + 17 + 11 + 1 + 20), warning-as-error lint, debug APK assembly, and manifest inspection; the latest 9,644,209-byte APK has SHA-256 `BE385FEB8966210C4C09027388C3F560745F6A075B9CBB1ABF25DC0893C0033C`. Two pinned ESP-IDF v6.0.2 generic `esp32s3` builds reproduce a 170,313-byte image and 170,432-byte BIN with SHA-256 `22CAE43F7AEA9D980602C41E1ACEB49CA1174315EE87598D15E6717A27A1E4D4`; the link map retains the callback adapter and accepted companion layers. This is build-linked-not-run evidence only. There is still no registered or live provisional NimBLE service, started controller or advertising, trusted target bond-store/persistence/physical-input adapter, successful app/device authorization or Ready state, clean-machine Android install, release signing/store package, production packet/radio target, physical display, or support evidence. Two Heltec clients and one SenseCAP repeater separately completed limited MeshCore-era bench evidence; one Wio candidate has partial USB/runtime/configuration and loader-recognition evidence only |
