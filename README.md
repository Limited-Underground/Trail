...
| Latest increment | Native Windows UI Automation now accepts the fresh source-free utility across the process boundary. Stable privacy-safe element IDs expose the packaged Window/Button/List/ListItem contracts; all three cards provide summary, blocker help, selection/scroll patterns, and distinct 1-of-3 positions without USB/COM identifiers. Three external selection/Refresh cycles cleared stale selection, republished all three devices, restored zero-ready status, and kept every Flash action disabled. Physical input, Narrator, installer lifecycle, and clean-machine acceptance remain separate gates |
| Proven so far | A 111-executable C++ host matrix, 54 C# loader document/identity/accessibility/production-window-refresh/selection/keyboard/automation-peer/high-DPI/contrast-theme/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, deterministic classic, 125%/150%/200%, contrast, and in-window theme-transition loader renders, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests |
