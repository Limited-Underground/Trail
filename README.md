...
| Latest increment | OT-131 records the single executable Monocypher attempt as aborted after Node A benchmark readback when capture failed closed as `capture_failed` / `preamble_invalid`. Node A restored/readback/reset exactly to Trail, Node B was never benchmark-flashed, restoration completed, and the owner confirmed both Trail logos. The authority is consumed; no result, radio, selection, Phase 2 completion, or score claim is added; V1 remains exact 43.75%/displayed 44% |
