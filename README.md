...
| Latest increment | OT-066 joins the private bond-reference resolver, private session issuer, durable one-phone authority, and existing GATT authorization seams. Eight strict host groups prove exact caching, changed/stale denial, re-pair separation, physical-gated claim, reconnect, wrong-phone denial, replacement, release, persistence uncertainty, and private failure results. The Heltec runtime remains wired to denied authorities |
| Proven so far | The prior 123-executable C++ host matrix remains accepted; OT-065 and OT-066 add independently passing 12-group protected-store and 8-group trusted-authority composition executables, while the Heltec target-only admission suite remains green at eight groups. `OT-DEV-001` still runs the 470,928-byte OT-064 app (SHA-256 `A7D8E672CF9169F1D1D4E86EEFF80399C47A145E7D64904C207DD5F1B23F359B`) with experimentally observed startup/status OLED and privacy-safe BLE advertising. Android passes 136 JVM tests across thirteen suites, clean lint, debug assembly, and manifest inspection. There is still no admitted protected target storage/key/bond backend, independent physical rollback floor, GATT exchange, successful app/device authorization or Ready state, real LoRa/GNSS path, interactive display/input, release signing/store package, power/endurance, field, regulatory, or support evidence |
