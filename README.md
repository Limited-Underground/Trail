...
| Latest increment | The Windows 95-style loader now explicitly declares `PerMonitorV2` DPI awareness with a legacy per-monitor fallback and passes deterministic production-window rendering at 125%, 150%, and 200% scaling. Minimum-window controls, scroll access, selected-device status, bundle blockers, and disabled Flash labels remain readable. This is scaled-render evidence, not real monitor movement or alternate-theme acceptance |
| Proven so far | A 111-executable C++ host matrix, 50 C# loader document/identity/accessibility/production-window-refresh/selection/high-DPI/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, deterministic desktop/minimum/scrolled and 125%/150%/200% loader renders, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests |
