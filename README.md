...
| Latest increment | OT-061 selects `OT-DEV-001` as the sacrificial-first Heltec target, rehearses manual ROM entry/exit, rehashes and writes the frozen four-region OpenTrail image once after owner authorization, passes exact post-write verification, and boots through deterministic self-checks into a stable five-second USB heartbeat. The exact accepted Android APK then found one compatible OpenTrail BLE service advertisement without selecting, connecting, or pairing. `OT-DEV-002` remained disconnected and untouched. This is experimental target/advertising evidence—not authorization, Ready, LoRa, GNSS, display, support, or field proof. The historical phone-independent evidence calculation advances to 31%; current V1 Companion and V2 Integrated scores remain unmeasured |
| Proven so far | A 123-executable C++ host matrix, 59 C# loader scenarios, five target-only Heltec admission groups, and one exact-profile ESP-IDF target remain accepted. OT-DEV-001 is now experimentally flashed: the single authorized erase/write passed post-write verification, boot self-checks reached the runtime loop, at least two heartbeat records were observed, and one physical Android scan found exactly one compatible service candidate. Android passes 135 JVM tests across thirteen suites, clean lint, debug assembly, and manifest inspection; its 9,677,165-byte APK has SHA-256 `9CE206EEEAE2B13FC5C1092CEF41C226607FD3A9905A5797D4EBE31F3DC7F01C`. There is still no admitted protected target storage/key/bond backend, GATT exchange, successful app/device authorization or Ready state, real coordinate source, LoRa/GNSS/display runtime, release signing/store package, power/endurance, field, regulatory, or support evidence |
