...
| Latest increment | OT-020B adds the Windows host authority required below a future supervised Wio GNSS adapter. Fifteen authority groups cover a bounded no-overwrite DPAPI CurrentUser 32-byte key store, domain-separated target and session-scoped continuity tokens, a nonblocking fail-closed per-target mutex, and two reduced counter observations across a requested five-second quiet interval; the exact prior-window ceiling is 180 seconds inclusive. The separate OT-020A lifecycle suite remains at 15 groups, and the hardened full host gate exits 0 with publication safety. OT-020B has no discovery, serial, PnP, BLE, MeshCore, live-device, or mutable device-command surface; the aggregate gate's pre-existing read-only three-device loader check is separate, not OT-020B hardware evidence. The entire live GNSS phase remains open, OT-020 stays `partial`/`experimented`, the missing shipping/pre-write state remains explicit, and V1 stays at 29% |
