...
| Latest increment | OT-056 codes and build-links the one-connection NimBLE runtime owner: after deterministic boot checks it can initialize the host/controller, register the exact protected service, and advertise a privacy-oriented service-only payload. Because OT-054 protected storage/private-bond admission remains denied, every connection is immediately terminated and claim/normal commands remain closed. OT-057 adds renderer-neutral Android Group / Location presentation with real BLE coordinates unavailable and a separately labeled deterministic Local fake. OT-058 hardens the simulator’s native-session pump with bounded coalescing and raises the integrated WPF gate to 13/13 without weakening the five-second timeout. Nothing was flashed or run on a device; the APK was not installed, and no live pairing, location, authorization, or Ready proof exists. OT-034 remains `partial`, and V1 is 30% |
| Proven so far | A 123-executable C++ host matrix, including 13 bounded BLE-runtime-owner groups plus the accepted companion suites and target-local boot self-checks; 59 C# loader scenarios; three build-only Heltec target admission groups; and the focused simulator gate of 33 Core, 23 Windows bridge, 15 private USB-helper, 10 native-protocol, and 13 integrated WPF groups. OT-056 passes 13 owner groups at 100/100, target self-check 100/100, pinned NimBLE teardown/stop-order admission, and two reproducible ESP-IDF v6.0.2 builds. The 433,104-byte BIN has SHA-256 `8A25508B50B29FE2A09CF3390AE53473BBA0BF04F60AE9A6366B930D516FCE2A`; this is build-linked-not-run evidence only. Android passes 134 JVM tests across thirteen suites (protocol 29; application 105), warning-as-error lint, debug APK assembly, and manifest inspection; the 9,677,165-byte APK has SHA-256 `697D73A6E48F1850A2756FB0886A8201C653804FB5A2B9628DD26790C8EC65B1`. Full Test-Host exits 0. There is still no physical target runtime, admitted protected target storage/key/bond backend, successful app/device authorization or Ready state, real BLE coordinate source, phone GPS/map/tile path, Android OS/device/install proof, release signing/store package, production packet/radio target, physical display, or support evidence. Two Heltec clients and one SenseCAP repeater separately completed limited MeshCore-era bench evidence; one Wio candidate has partial USB/runtime/configuration and loader-recognition evidence only |
