...
| Latest increment | The real UI Automation `ScrollItem` provider now passes a functional minimum-window reachability check. Starting with the third wrapped card offscreen, direct provider invocation moves the production outer viewport and flips the peer to onscreen without selecting a device, stealing Refresh focus, changing public metadata, or enabling bundle selection or the footer Flash action. This is deterministic in-process provider evidence; Narrator/Braille UX, packaged out-of-process scrolling, physical wheel/touch/mouse input, installer lifecycle, and clean-machine acceptance remain separate gates |
| Proven so far | A 111-executable C++ host matrix, 56 C# loader document/identity/accessibility/production-window-refresh/selection/keyboard/automation-peer/automation-scroll/high-DPI/resize/contrast-theme/snapshot-binding/device-match/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, deterministic classic, 125%/150%/200%, contrast, and in-window theme-transition loader renders plus shown resize-state acceptance, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests |
