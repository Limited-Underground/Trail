...
| Latest increment | OT-144 finds before hardware that the accepted, unconsumed OT-143 authority cannot execute: its preparation/authority pin the corrected OT-142 application while its coordinator still pins the old OT-139 image. Decision 0080 and all OT-143 artifacts remain immutable history. A fresh host-only OT-144 runtime/preparation now binds the corrected image consistently; no device I/O occurred, and fresh explicit one-attempt authority is still required. No result, radio, selection, Phase 2 completion, or score claim changes; V1 remains exact 43.75%/displayed 44%. |
