...
| Latest increment | OT-156 accepts a host-only reset-aware successor for the consumed Noise XK radio path. Both restart acknowledgements now precede fresh bounded endpoint opens, both post-reboot contracts must pass before any radio verb, stale handles cannot cross the reset boundary, failures reduce to sixteen privacy-safe stages, and successful result/command bytes remain exact to OT-153. OT-157 must freeze a fresh bundle before any later authority. V1 remains exact 43.75%/displayed 44%. |
