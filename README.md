...
| Latest increment | OT-134 permanently corrects the Windows Host validation newline drift without changing any accepted artifact or digest. The last green was OT-092 run 210; OT-093 run 211 was the first red but failed the older OTCBL0 raw-manifest check, while the exact OTCAI signature began when OT-098 introduced that validator in run 217. Explicit Git attributes now preserve all six raw-bound OT-098/OT-099 files, focused tests pass 4/4 and 5/5, and the complete Windows host matrix passes. No firmware, hardware, phone, product capability, score, or public website status changes. See [Decision 0073](docs/decisions/0073-freeze-windows-raw-evidence-checkout-bytes.md) and [OT-134 evidence](tests/hardware/OT-134-2026-08-24.md). |
