...
| Latest increment | OT-115 installs the compact-footer OpenTrail bench firmware on both Heltec test units. A two-unit view shows both displays lit with the footer row; one clear close view confirms `BAT:--% GPS:-- BLE:A` with a blank traffic field. Content-aware redraw is host-tested; live battery, GNSS, and LoRa-activity bindings remain open, and no score changes |
