...
| Latest increment | The Windows utility now binds local bundle inspection to one current connected-device snapshot and discards stale/in-flight bundle results on refresh. A single failed read-only ROM-entry attempt on one Heltec also completed manual USB/runtime recovery with unchanged MeshCore/radio settings; exact low-level identity, automatic recovery, exact-device matching, and Flash remain blocked. See [the dated evidence](tests/hardware/OT-019O-2026-08-13.md) |
| Proven so far | A 111-executable C++ host matrix, 46 C# loader document/identity/accessibility/refresh/snapshot-binding/process-boundary/USB-runtime/hardware-profile/fixed-vector bundle-signature/packaged-inspection scenarios, cross-tool signature evidence, and Python evidence checks; two Heltec clients and one SenseCAP repeater have completed limited GNSS, transport, soak, burst, alert/acknowledgement, and bounded USB recovery tests |
