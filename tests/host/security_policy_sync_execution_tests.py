"""Actual OT201 orchestration/authority/observation/restoration; synthetic storage only."""
from pathlib import Path
import copy,json,sys,tempfile,unittest
from unittest.mock import patch
ROOT=Path(__file__).resolve().parents[2]
sys.path.insert(0,str(ROOT/'tools'))
import security_policy_sync_execution as x
...
class Tests(unittest.TestCase):
    def setUp(self):
        ...
        self.image=x.readback.ImageBinding(x.readback.SYNC_CONTRACT,'heltec_v4_security_sync_diag',x.sha(self.data['candidate']))
    ...
    def test_observation_failure_restoration_success(self):
        ...
        self.assertEqual(claim['schema'],'OT201-SYNC-OBSERVATION-CLAIM-1')
    ...
    def test_legacy_schema_and_authority_refused(self):
        for schema in ('OT188-GRANT-1','OT198-INPUT-EXECUTE-GRANT-1','OT196-STAGE-EXECUTE-GRANT-1'):
