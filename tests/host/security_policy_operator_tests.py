...
SPEC=importlib.util.spec_from_file_location("ot189_operator_tests_worker",ROOT/"tools/security_policy_operator.py")
...
class Tests(unittest.TestCase):
    ...
    def manifest_fixture(self):
        ...
        files={}
        names=["esptool.cfg","packages/esptool/__init__.py","packages/serial/__init__.py","python.exe","python314.dll","python314._pth","policy/security_policy_operator.py","policy/Invoke-SecurityPolicyOperator.ps1"]+["policy/security_policy_"+x+".py" for x in worker.POLICY]
        for name in names:
            raw=b"[esptool]\n" if name=="esptool.cfg" else worker.PTH if name=="python314._pth" else (ROOT/"tools/Invoke-SecurityPolicyOperator.ps1").read_bytes() if name=="policy/Invoke-SecurityPolicyOperator.ps1" else b"synthetic pinned fixture"
        ...
        m={"schema":"OT189-RUNTIME-1","root":str(capsule),"worktree":str(worktree),"files":files,"versions":{"python":"3.14.6","esptool":"5.3.1","pyserial":"3.5"},"powershell":{"path":str(host),"bytes":host.stat().st_size,"sha256":worker.digest(host.read_bytes())}}
    ...
    def test_operator_request_bound_files_and_runtime(self):
        ...
        req={"schema":"OT189-OPERATOR-1","runtime_sha256":h,"operation":"recover","package_path":str(package),"package_sha256":worker.digest(b"{}"),"grant_path":str(grant),"grant_sha256":worker.digest(b"{}"),"origin_attempt":"a"*32}
    ...
    def test_powershell_rejects_poison_before_native_startup(self):
        ...
        script=ROOT/"tools/Invoke-SecurityPolicyOperator.ps1"
        def invoke(manifest):
    ...
    def test_recover_dispatch_without_candidate_and_source_mismatch(self):
        ...
        req={"schema":"OT189-OPERATOR-1","runtime_sha256":h,"operation":"recover","package_path":str(package_path),"package_sha256":worker.digest(package_path.read_bytes()),"grant_path":str(grant_path),"grant_sha256":worker.digest(b"{}"),"origin_attempt":"a"*32}
        ...
        modules={"security_policy_bundle":SimpleNamespace(),"security_policy_execution":execution,"security_policy_hardware":hardware}
        with patch.object(worker.importlib,"import_module",side_effect=lambda n:modules[n]),patch.object(worker,"make_transport",return_value="transport"),patch.object(worker,"audit_loaded_modules",return_value=True):
