...
spec = importlib.util.spec_from_file_location("runtime_bundle_tested", ROOT / "tools/security_policy_sync_runtime_bundle.py")
...
class BundleTests(unittest.TestCase):
    ...
    def test_old_manifest_schema_refused(self):
        manifest=self.build()
        self.assertEqual(manifest['schema'],'OT201-SYNC-RUNTIME-1')
    ...
    def test_sync_required_file_cannot_be_removed_from_manifest(self):
        manifest=self.build()
        name='policy/security_policy_sync_execution.py'
    ...
    def test_complete_old_and_new_policy_closure(self):
        ...
        expected=tuple('security_policy_'+name+'.py' for name in (
            ...
            'sync_readback','sync_observation','sync_bundle','sync_execution','sync_operator',
            'sync_backup_operator','sync_runtime_bundle','sync_restore_observation'))+(
            'Invoke-SecurityPolicyOperator.ps1','Invoke-SecurityPolicySyncOperator.ps1')
        self.assertEqual(22,len(expected));self.assertEqual(set(expected),set(bundle.POLICY))
