...
spec=importlib.util.spec_from_file_location('bundle',ROOT/'tools/security_policy_deadline_bundle.py')
b=importlib.util.module_from_spec(spec);spec.loader.exec_module(b)

class BundleTests(unittest.TestCase):
    def test_new_image_contract_is_exact_and_immutable(self):
        image=b.image_binding(self.package)
        self.assertEqual(image.contract,'OT200-SYNC-INPUT-RECORD-1')
        self.assertEqual(image.target,'heltec_v4_invitation_eval')
        self.assertEqual(image.sha256,b.CANDIDATE_SHA)
        with self.assertRaises(Exception): image.sha256='0'*64

    def test_predecessor_and_wrong_image_contract_refused(self):
        for field,value in [('image_target','heltec_v4_security_input_diag'),
                            ('diagnostic_record_contract','OT198-INPUT-RECORD-1'),
                            ('diagnostic_keys',['stage']),('input_detail_write',False),
                            ('pre_restore_nvs_observation',1)]:
            changed=copy.deepcopy(self.package);changed['scope'][field]=value
            with self.assertRaises(b.BundleError): b.verify(self.root,changed)
            with self.assertRaises(b.BundleError): b.image_binding(changed)
        changed=copy.deepcopy(self.package);changed['schema']='OT198-INPUT-BUNDLE-1'
        with self.assertRaises(b.BundleError): b.verify(self.root,changed)

    def test_host_deadline_contract_and_predecessor_schema_are_required(self):
        self.assertEqual('OT212-ENDPOINT-DEADLINE-1',self.package['scope']['host_deadline_contract'])
        for value in (None,True,'OT210-INVITATION-RUNTIME-1','OT212-ENDPOINT-DEADLINE-2'):
            changed=copy.deepcopy(self.package);changed['scope']['host_deadline_contract']=value
            with self.assertRaises(b.BundleError): b.verify(self.root,changed)
            with self.assertRaises(b.BundleError): b.image_binding(changed)
        changed=copy.deepcopy(self.package);changed['scope'].pop('host_deadline_contract')
        with self.assertRaises(b.BundleError): b.verify(self.root,changed)
        changed=copy.deepcopy(self.package);changed['schema']='OT210-INVITATION-BUNDLE-1'
        with self.assertRaises(b.BundleError): b.verify(self.root,changed)

    def test_changed_candidate_identity_rejected_before_projection(self):
        for field,value in [('sha256','0'*64),('bytes',438912)]:
            changed=copy.deepcopy(self.package);changed['candidate'][field]=value
            with self.assertRaises(b.BundleError): b.image_binding(changed)

    def test_frozen_decoder_cannot_be_reblessed(self):
        for name in ('tools/security_policy_invitation_readback.py','tools/security_policy_invitation_observation.py',
                     'tools/security_policy_input_readback.py','tools/security_policy_capture.py',
                     'tools/security_policy_deadline_endpoint.py','tools/security_policy_deadline_boundary.py'):
            with self.subTest(name=name):
                source=self.root/name;before=source.read_bytes();source.write_bytes(b'changed decoder')
                with self.assertRaisesRegex(b.BundleError,'frozen_source_changed'):
                    b.freeze(self.root,self.candidate,tuple(self.roles),runtime_sha256='a'*64)
    ...
    def setUp(self):
        ...
        pins={}
        for i in range(b.BUILD_SOURCE_COUNT):
        ...
        for index,role in enumerate('AB'):
            ...
            nvs=self.root/'.private'/f'{role}-nvs.bin';nvs.write_bytes(b'\xff'*12288)
            ...
            self.roles.append({'role':role,'private_route':f'COM{91+index}',
                'private_identity':f'0123456789a{index}','application':str(app),'nvs':str(nvs),'protected':protected})
        self.addCleanup(patch.stopall)
        patch.multiple(b,FROZEN_SOURCE_PINS={name:b.sha((self.root/name).read_bytes()) for name in b.FROZEN_SOURCE_PINS},CANDIDATE_SHA=b.sha(self.candidate.read_bytes()),REPORT_SHA=b.sha(report.read_bytes()),
        ...
        self.package=b.freeze(self.root,self.candidate,tuple(self.roles),runtime_sha256="a"*64)

    def test_all_reserved_original_namespaces_refused(self):
        from security_policy_invitation_readback_tests import namespace_fixture
        path=Path(self.roles[0]['nvs'])
        for name in sorted(b.readback.FRESH_NAMESPACES):
            with self.subTest(namespace=name):
                path.write_bytes(namespace_fixture(name))
                with self.assertRaises(b.BundleError):
                    b.freeze(self.root,self.candidate,tuple(self.roles),runtime_sha256='a'*64)

    def test_frozen_receipt_operator_still_refuses_new_candidate(self):
        import security_policy_receipt_bundle as frozen
        with self.assertRaises(frozen.BundleError): frozen.image_binding(self.package)
        changed=copy.deepcopy(self.package)
        changed['schema']=frozen.SCHEMA
        changed['scope']=copy.deepcopy(frozen.SCOPE)
        with self.assertRaises(frozen.BundleError): frozen.image_binding(changed)

    def test_original_namespace_and_corruption_refused(self):
        import security_policy_input_readback_tests as fixture
        path=Path(self.package['roles'][0]['nvs']['path'])
        for raw in (fixture.diagnostic(),b'X'*12288):
            path.write_bytes(raw)
            with self.assertRaisesRegex(b.BundleError,'original_diagnostic_not_fresh'):
                b.freeze(self.root,self.candidate,tuple(self.roles),runtime_sha256='a'*64)

    def test_runtime_and_legacy_schema_refused(self):
        for key,value in [('runtime_sha256',''),('runtime_sha256',True),('schema','OT188-SECURITY-POLICY-BUNDLE-1')]:
            changed=copy.deepcopy(self.package);changed[key]=value
            with self.assertRaises(b.BundleError):b.verify(self.root,changed)

    def test_frozen_source_cannot_rebless(self):
        name=next(iter(b.FROZEN_SOURCE_PINS))
    ...
    def test_roundtrip_exact_bytes_and_digest(self):
        ...
        for index,role in enumerate(loaded['roles']):
            ...
            self.assertEqual(role['nvs'],b'\xff'*12288)
    ...
    def test_recovery_requires_build_and_original_provenance(self):
        self.candidate.unlink()
        self.assertIsNone(b.verify(self.root,self.package,True)['candidate'])
        (self.root/b.REPORT).unlink()
        with self.assertRaises(b.BundleError):b.verify(self.root,self.package,True)
    ...
    def test_role_order_and_missing_side(self):
        for roles in (tuple(reversed(self.roles)),self.roles[:1]):
            with self.assertRaises(b.BundleError):b.freeze(self.root,self.candidate,roles,runtime_sha256="a"*64)
    ...
    def test_duplicate_routes_casefold_and_normalized_identity(self):
        for name,value in [('private_route','com91'),('private_identity','01-23-45-67-89-a0')]:
            ...
            with self.assertRaisesRegex(b.BundleError,'roles_not_unique'):b.freeze(self.root,self.candidate,tuple(roles),runtime_sha256="a"*64)

    def test_duplicate_custody_rejected(self):
        for name in ('application','nvs'):
            roles=copy.deepcopy(self.roles);roles[1][name]=roles[0][name]
            with self.assertRaisesRegex(b.BundleError,'custody_not_unique'):b.freeze(self.root,self.candidate,tuple(roles),runtime_sha256="a"*64)
    ...
    def test_traversal_outside_and_relative_paths(self):
        ...
        for path in paths:
            ...
            with self.assertRaises(b.BundleError):b.freeze(self.root,self.candidate,tuple(roles),runtime_sha256="a"*64)
