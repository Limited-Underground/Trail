...
import security_policy_stage_readback_tests as stage_fixtures
...
TARGET=ROOT/'firmware/targets/heltec_v4_security_policy_diag/main'
POLICY=ROOT/'firmware/targets/heltec_v4_security_policy_eval/main'
SCENARIOS=('success','preexisting','nvs_fail','create_fail','set_fail','readback_fail','readback_corrupt','commit_applied_fail','install_fail','begin_fail','no_input','partial','invalid','input_fault','start_fail','sodium_fail','evaluate_fail','namespace_fail','stop_fail','entropy_fault','send_fail')
...
def run():
    ...
    with tempfile.TemporaryDirectory(prefix='policy-lifecycle-',dir=ROOT/'build') as d:
        ...
        contract=json.loads((ROOT/'firmware/components/security_diagnostics/stage_record_v1.json').read_bytes())
        def check_result(result,scenario):
            ...
            for text in lines[1:]:
                ...
                nvs=stage_fixtures.fixture([
                    stage_fixtures.entry(0,1,'ot195diag',b'\x01'),
                    stage_fixtures.entry(1,8,'stage',raw),
                ])
                decoded=stage_fixtures.decoder.decode(nvs)
                if (decoded['stage']!=contract['stages'][str(raw[4])] or
                        decoded['error']!=contract['errors'][str(raw[5])]):
                    raise RuntimeError('stage_decoder_crosslayer_mismatch')
        ...
        print(f'PASS {len(SCENARIOS)+4} actual stage lifecycle groups')
