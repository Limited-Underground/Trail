...
class Tests(unittest.TestCase):
    def test_exact_input_and_anchors_only(self):
        ...
        generated = source.console.contained.generate(raw)
        for bad in (generated.replace(b"void handle_send(", b"void changed_send("),
                    generated + b"\nvoid handle_send(char* const* tokens) {\n}\n"):
            with patch.object(source.console.contained, "generate", return_value=bad):
    ...
    def test_install_and_lock_boundaries(self):
        ...
        main = source.console.function(text, 'extern "C" void app_main() {')
        ...
        cli = source.console.function(text, "void cli_task(void*) {")
    ...
    def test_only_receipt_output_changes(self):
        raw = FROZEN.read_bytes()
        before = source.console.generate(raw).decode()
        expected = before.replace("ESP_LOGI(", "OT_RECEIPT_I(").replace("ESP_LOGW(", "OT_RECEIPT_W(")
        expected = expected.replace("            std::fflush(stdout);\n", "")
        text = source.generate(raw).decode()
        # Protocol record text stays exact; only readiness admission/order changes.
        import re
        self.assertEqual(re.findall(r'OT_RECEIPT_[IW]\(.*?\);', text, re.S),
                         re.findall(r'OT_RECEIPT_[IW]\(.*?\);', expected, re.S))
        ready = source.console.function(text, "void handle_ready(const char* challenge) {")
        self.assertLess(ready.index("!idle || !counters_zero"), ready.index("ot_console_begin_session"))
        cli = source.console.function(text, "void cli_task(void*) {")
        self.assertLess(cli.index("!ot_console_session_started()"), cli.index("handle_prepare(tokens.data())"))
        self.assertNotIn("ESP_LOG", text)
        self.assertNotIn("fflush", text)

    def test_cli_refuses_frozen_overwrite(self):
        result = subprocess.run([sys.executable, str(ROOT / "tools/noise_xk_receipt_firmware_source.py"),
    ...
    def test_actual_generated_handlers(self):
        text = source.generate(FROZEN.read_bytes()).decode()
        functions = "\n".join(source.console.function(text, sig) for sig in (
...
template<class... A> void log(const char*,const char* fmt,A... args){
 opentrail::diagnostics::BoundedReceiptFormatter formatter;
 auto r=formatter.format({nullptr,[](void*) noexcept -> std::uint64_t {return 100;}}, 20000,
                         'I', 1, "test", fmt, args...);
 CHECK(r.status==opentrail::diagnostics::ReceiptFormatStatus::complete);logs.emplace_back(formatter.data());
 if(!fault.empty() && logs.back().find(fault)!=std::string::npos)healthy=false;
}
#define OT_RECEIPT_I log
...
'''
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            built=subprocess.run([compiler,"-std=c++17","-Wall","-Wextra","-Werror","-DNDEBUG","-I",str(ROOT/"firmware/components/diagnostics/include"),str(cpp),"-o",str(exe)],capture_output=True)
