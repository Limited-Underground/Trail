...
import security_policy_stage_runtime_bundle as runtime
...
class IsolatedTests(unittest.TestCase):
    ...
    def launch(self,mode,request,*,poison=False):
        ...
        return subprocess.run([str(self.pwsh),'-NoProfile','-NonInteractive','-File',str(self.capsule/'policy/Invoke-SecurityPolicyStageOperator.ps1'),
    ...
    def stage_request(self):
        ...
        self.save(f'ot196-stage-grant-{attempt}.used',{'package_sha256':package_sha,'grant':grant_ref['sha256'],'runtime_sha256':self.runtime_sha})
        op=self.save('stage-op.json',{'schema':'OT196-STAGE-OPERATOR-1','runtime_sha256':self.runtime_sha,'operation':'recover',
        ...
        return {'schema':'OT196-STAGE-ROM-1','runtime_sha256':self.runtime_sha,'operator_request':op,
    ...
    def backup_request(self):
        ...
        op=self.save('backup-op.json',{'schema':'OT196-STAGE-BACKUP-OPERATOR-1','runtime_sha256':self.runtime_sha,'operation':'capture',
        ...
        return {'schema':'OT196-STAGE-BACKUP-ROM-1','runtime_sha256':self.runtime_sha,'operator_request':op,
    ...
    def test_stage_rom_actual_isolation_and_stale_authority(self):
        req=self.stage_request();self.reached(self.launch('StageRom',req,poison=True))
        self.refused(self.launch('StageRom',req))  # Exact child replay is consumed.
        req['argv'][-1]='erase-flash';self.refused(self.launch('StageRom',req))
        req['argv'][-1]='read-mac';req['schema']='OT189-ROM-1';self.refused(self.launch('StageRom',req))
        req['schema']='OT196-STAGE-ROM-1';(self.private/('ot196-stage-grant-'+'b'*32+'.used')).write_bytes(b'{}')
        self.refused(self.launch('StageRom',req))
    def test_backup_rom_actual_isolation_and_stale_phase(self):
        req=self.backup_request();self.reached(self.launch('StageBackupRom',req,poison=True))
        req['argv'][-1]='erase-flash';self.refused(self.launch('StageBackupRom',req))
        req['argv'][-1]='read-mac';req['schema']='OT190-ROM-1';self.refused(self.launch('StageBackupRom',req))
        req['schema']='OT196-STAGE-BACKUP-ROM-1';path=self.private/('security-policy-backup-'+'d'*32+'-journal.jsonl')
        with path.open('ab') as out:out.write(encoded({'seq':2,'event':'read_intent','role':'B'})+b'\n')
        self.refused(self.launch('StageBackupRom',req))
