...
spec = importlib.util.spec_from_file_location("runtime_bundle_tested", ROOT / "tools/security_policy_deadline_runtime_bundle.py")
...
class BundleTests(unittest.TestCase):
    ...
    def test_old_manifest_schema_refused(self):
        manifest=self.build()
        self.assertEqual(manifest['schema'],'OT212-DEADLINE-RUNTIME-1')
        for schema in ('OT189-RUNTIME-1','OT196-STAGE-RUNTIME-1','OT198-INPUT-RUNTIME-1','OT201-SYNC-RUNTIME-1','OT203-RECEIPT-RUNTIME-1','OT210-INVITATION-RUNTIME-1'):
            with self.subTest(schema=schema):
                with self.assertRaises(bundle.RuntimeBundleError):
                    bundle.verify(self.destination,{**manifest,'schema':schema})

    def test_sync_required_file_cannot_be_removed_from_manifest(self):
        manifest=self.build()
        name='policy/security_policy_deadline_execution.py'
    ...
    def test_complete_old_and_new_policy_closure(self):
        manifest=self.build()
        expected=tuple('security_policy_'+name+'.py' for name in (
            'bundle','hardware','endpoint','execution','capture','operator','backup',
            'backup_operator','diagnostics','runtime_bundle','input_readback','input_timing',
            'invitation_readback','invitation_observation','deadline_bundle','deadline_execution','deadline_operator',
            'deadline_backup_operator','deadline_runtime_bundle','deadline_restore_observation','receipt_boundary','deadline_endpoint','deadline_boundary'))+(
            'Invoke-SecurityPolicyOperator.ps1','Invoke-SecurityPolicyDeadlineOperator.ps1')
        self.assertEqual(25,len(expected));self.assertEqual(set(expected),set(bundle.POLICY))
        self.assertEqual({'policy/'+name for name in expected},
                         {name for name in manifest['files'] if name.startswith('policy/')})
        for name in expected:
            with self.subTest(name=name):
                path=self.destination/'policy'/name;raw=path.read_bytes()
                changed=copy.deepcopy(manifest);changed['files'].pop('policy/'+name);path.unlink()
                with self.assertRaises(bundle.RuntimeBundleError):bundle.verify(self.destination,changed)
                path.write_bytes(raw)
