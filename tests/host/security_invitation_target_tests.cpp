// Actual OT208 target headers and app_main, with SDK-only seams and real crypto.
#include <algorithm>
#include <array>
#include <cstdint>
#include <cstdlib>
...
#include <vector>
#include "nvs.h"
#include "invitation_evaluation.hpp"
#include "opentrail/security_input_record.hpp"
using namespace opentrail;
namespace tested_target=opentrail::target::heltec_v4_invitation_eval;
...
bool no_policy_writes(){for(const auto&e:events)if(e.name.rfind("ot187_",0)==0&&(e.operation=="set"||e.operation=="erase"||e.operation=="commit"))return false;return true;}
...
esp_err_t nvs_set_blob(nvs_handle_t h,const char* key,const void* data,std::size_t size){
 auto&handle=mock::handle(h);mock::event("set",handle.name,key);CHECK(size==64);
...
using Backend=tested_target::NvsInvitationBackend;using Error=Backend::Error;
...
const std::array<std::string,7> names{"ot208_boot","ot208_ia","ot208_ib","ot187_ta","ot187_tb","ot187_ra","ot187_rb"};
static void clean_exit(){CHECK(mock::handles.empty());CHECK(mock::all_cleared());}
static std::uint64_t boot_generation(){const auto&blobs=mock::durable.at("ot208_boot");std::uint64_t highest=0;
...
int main(){unsigned groups=0;
 ...
 for(unsigned which=0;which<3;++which){mock::reset();{
  ...
  const auto calls=mock::events.size();CHECK(backend.write_blob("nvs",N,K,bytes.data(),64)==Error::invalid_argument);CHECK(backend.erase_key(P,N,"other")==Error::invalid_argument);CHECK(backend.commit(P,"ot208_boot")==Error::invalid_argument);CHECK(mock::events.size()==calls);}
 ...
 for(const auto& fault:{"query_error","query_short","data_error","data_short"}){mock::reset();mock::durable["ot208_ia"][K]=mock::Bytes(64,7);{
 ...
 CHECK(mock::first("set","ot208_boot")<mock::first("rng"));
 CHECK(mock::first("set","ot208_ia")<mock::first("set","ot187_ta"));CHECK(mock::first("set","ot208_ib")<mock::first("set","ot187_ta"));
 CHECK(mock::count("open")==7);for(std::size_t i=0;i<7;++i)CHECK(baseline[i].operation=="open");
 const auto old_generation=boot_generation();const auto old_ta=mock::policy("ot187_ta"),old_tb=mock::policy("ot187_tb"),old_ra=mock::policy("ot187_ra"),old_rb=mock::policy("ot187_rb");groups++;
 ...
 CHECK(mock::policy("ot187_ta")==old_ta&&mock::policy("ot187_tb")==old_tb&&mock::policy("ot187_ra")==old_ra&&mock::policy("ot187_rb")==old_rb);groups++;
 ...
 {mock::reset();app_main();clean_exit();CHECK(mock::count("send")==1&&mock::receipt.find(" pass\n")!=std::string::npos);CHECK(mock::receipt.rfind("SEC_BEGIN1 ",0)==0);
  ...
  CHECK(mock::first("console_begin")<mock::first("entropy_start"));CHECK(mock::first("entropy_start")<mock::first("sodium_init"));CHECK(mock::first("sodium_init")<mock::first("open","ot208_boot"));CHECK(mock::first("entropy_stop")<mock::first("send"));
  ...
  const auto before=mock::durable;mock::reset(true);app_main();clean_exit();CHECK(mock::durable==before);CHECK(mock::count("console_install")==0&&mock::count("open","ot208_boot")==0&&mock::count("entropy_start")==0&&mock::count("send")==0);groups+=2;
 }
 for(unsigned kind=0;kind<9;++kind){mock::reset();if(kind==0)mock::fail_init=true;if(kind==1)mock::durable["ot198diag"]["owner"]={1};if(kind==2)mock::fail_install=true;if(kind==3)mock::console_good=false;if(kind==4)mock::fail_begin=true;if(kind==5)mock::fail_start=true;if(kind==6)mock::fail_sodium=true;if(kind==7)mock::fail_stop=true;if(kind==8)mock::random_fail_nth=1;
  ...
  if(kind==5||kind==6){CHECK(mock::count("open","ot208_boot")==0);}
  if(kind==5){CHECK(mock::count("sodium_init")==0);}groups++;
 }
 std::cout<<"PASS "<<groups<<" actual invitation target groups\n";
