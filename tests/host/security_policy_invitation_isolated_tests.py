...
import security_policy_invitation_runtime_bundle as runtime
...
class IsolatedTests(unittest.TestCase):
    ...
    def launch(self,mode,request=None,*,poison=False,manifest=None,manifest_sha=None):
        ...
        argv=[str(self.pwsh),'-NoProfile','-NonInteractive','-File',str(self.capsule/'policy/Invoke-SecurityPolicyInvitationOperator.ps1'),
    ...
    def stage_request(self):
        ...
        self.save(f'ot210-invitation-grant-{attempt}.used',{'package_sha256':package_sha,'grant':grant_ref['sha256'],'runtime_sha256':self.runtime_sha})
        op=self.save('stage-op.json',{'schema':'OT210-INVITATION-OPERATOR-1','runtime_sha256':self.runtime_sha,'operation':'recover',
        ...
        return {'schema':'OT210-INVITATION-ROM-1','runtime_sha256':self.runtime_sha,'operator_request':op,
    ...
    def backup_request(self):
        ...
        op=self.save('backup-op.json',{'schema':'OT210-INVITATION-BACKUP-OPERATOR-1','runtime_sha256':self.runtime_sha,'operation':'capture',
        ...
        return {'schema':'OT210-INVITATION-BACKUP-ROM-1','runtime_sha256':self.runtime_sha,'operator_request':op,
    ...
    def test_stage_rom_actual_isolation_and_stale_authority(self):
        req=self.stage_request();self.reached(self.launch('InvitationRom',req,poison=True))
        self.refused(self.launch('InvitationRom',req))  # Exact child replay is consumed.
        req['argv'][-1]='erase-flash';self.refused(self.launch('InvitationRom',req))
        req['argv'][-1]='read-mac';req['schema']='OT201-SYNC-ROM-1';self.refused(self.launch('InvitationRom',req))
        req['schema']='OT210-INVITATION-ROM-1';execution.used_path(self.work,self.recovery_attempt).write_bytes(b'{}')
        self.refused(self.launch('InvitationRom',req))
    def test_backup_rom_actual_isolation_and_stale_phase(self):
        req=self.backup_request();self.reached(self.launch('InvitationBackupRom',req,poison=True))
        req['argv'][-1]='erase-flash';self.refused(self.launch('InvitationBackupRom',req))
        req['argv'][-1]='read-mac';req['schema']='OT201-SYNC-BACKUP-ROM-1';self.refused(self.launch('InvitationBackupRom',req))
        req['schema']='OT210-INVITATION-BACKUP-ROM-1';path=self.private/('security-policy-backup-'+self.backup_attempt+'-journal.jsonl')
        with path.open('ab') as out:out.write(encoded({'seq':2,'event':'read_intent','role':'B'})+b'\n')
        self.refused(self.launch('InvitationBackupRom',req))
    ...
    def test_old_runtime_manifest_is_refused_by_actual_launcher(self):
        legacy=copy.deepcopy(self.manifest);legacy['schema']='OT201-SYNC-RUNTIME-1'
    ...
    def test_wrong_runtime_and_old_operator_envelope_never_reach_boundary(self):
        ...
        self.refused(self.launch('InvitationRom',changed))
        envelope=json.loads(Path(req['operator_request']['path']).read_bytes())
        envelope['schema']='OT201-SYNC-OPERATOR-1'
    ...
    def test_raw_execute_envelope_requires_typed_backup_handoff(self):
        ...
        self.refused(self.launch('InvitationOperator',envelope))
    ...
    def test_old_recovery_grant_is_not_new_sync_authority(self):
        ...
        grant=json.loads(Path(envelope['grant_path']).read_bytes());grant['schema']='OT201-SYNC-RECOVER-GRANT-1'
