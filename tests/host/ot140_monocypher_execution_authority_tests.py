#!/usr/bin/env python3
"""Adversarial host tests for the OT-140 executable bundle authority."""
...
TOOL_PATH = ROOT / "tools" / "ot140_monocypher_execution_authority.py"
SPEC = importlib.util.spec_from_file_location("ot140_authority", TOOL_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("OT-140 authority tool unavailable")
...
class Fixture:
    def __init__(self, directory: str) -> None:
        ...
        self.ot136_authority_payload = b"ot136-consumed-authority-v0\n"
        self.ot137_abort_payload = b"ot137-abort-v0\n"
        self.ot139_decision_payload = b"ot139-decision-v0\n"
        self.ot139_evidence_payload = b"ot139-evidence-v0\n"
        self.ot139_build_evidence_payload = b"ot139-build-evidence-v0\n"
        ...
        self._write(
            self.root / contract.OT136_AUTHORITY_RELATIVE,
            self.ot136_authority_payload,
        )
        self._write(
            self.root / contract.OT137_ABORT_RELATIVE,
            self.ot137_abort_payload,
        )
        self._write(
            self.root / contract.OT139_DECISION_RELATIVE,
            self.ot139_decision_payload,
        )
        self._write(
            self.root / contract.OT139_EVIDENCE_RELATIVE,
            self.ot139_evidence_payload,
        )
        self._write(
            self.root / contract.OT139_BUILD_EVIDENCE_RELATIVE,
            self.ot139_build_evidence_payload,
        )
    ...
    def patches(self):
        return (
            ...
            mock.patch.object(
                contract,
                "OT136_AUTHORITY_RAW_SHA256",
                sha256(self.ot136_authority_payload),
            ),
            mock.patch.object(
                contract,
                "OT137_ABORT_RAW_SHA256",
                sha256(self.ot137_abort_payload),
            ),
            mock.patch.object(
                contract,
                "OT139_DECISION_RAW_SHA256",
                sha256(self.ot139_decision_payload),
            ),
            mock.patch.object(
                contract,
                "OT139_EVIDENCE_RAW_SHA256",
                sha256(self.ot139_evidence_payload),
            ),
            mock.patch.object(
                contract,
                "OT139_BUILD_EVIDENCE_RAW_SHA256",
                sha256(self.ot139_build_evidence_payload),
            ),
            mock.patch.object(
                contract,
                "OT139_ARTIFACT_TUPLE",
                tuple(
...
class ContractTests(unittest.TestCase):
    ...
    def test_01_current_ot136_ot139_and_runtime_bindings_match(self) -> None:
        for _, relative, digest in contract.FIXED_RUNTIME_BINDINGS:
            self.assertEqual(sha256((ROOT / relative).read_bytes()), digest)
        for relative, digest in (
            (contract.OT136_AUTHORITY_RELATIVE, contract.OT136_AUTHORITY_RAW_SHA256),
            (contract.OT137_ABORT_RELATIVE, contract.OT137_ABORT_RAW_SHA256),
            (contract.OT139_DECISION_RELATIVE, contract.OT139_DECISION_RAW_SHA256),
            (contract.OT139_EVIDENCE_RELATIVE, contract.OT139_EVIDENCE_RAW_SHA256),
            (
                contract.OT139_BUILD_EVIDENCE_RELATIVE,
                contract.OT139_BUILD_EVIDENCE_RAW_SHA256,
            ),
        ):
            self.assertEqual(sha256((ROOT / relative).read_bytes()), digest)

    def test_02_preparation_binds_adapter_and_consumed_ot136_ot139_successor(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ot140-authority-test-") as directory:
            ...
            self.assertTrue(supersession["ot136_authority_consumed"])
            self.assertFalse(supersession["ot136_authority_reusable"])
            self.assertFalse(supersession["replacement_attempt_inherited"])
            self.assertEqual(value["quiet_target"]["source_input_count"], 14)
            self.assertEqual(len(value["quiet_target"]["artifact_tuple"]), 6)
            self.assertTrue(
                value["quiet_target"]["only_application_bin_is_writable"]
            )
    ...
    def test_10_authority_tool_has_no_hardware_surface(self) -> None:
        ...
        self.assertIn(
            'ADAPTER_RELATIVE = "tools/ot140_monocypher_hardware_adapter.py"',
            source,
        )
