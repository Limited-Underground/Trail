#!/usr/bin/env python3
"""Adversarial host tests for the OT-136 executable bundle authority."""
...
TOOL_PATH = ROOT / "tools" / "ot136_monocypher_execution_authority.py"
SPEC = importlib.util.spec_from_file_location("ot136_authority", TOOL_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("OT-136 authority tool unavailable")
...
class Fixture:
    def __init__(self, directory: str) -> None:
        ...
        self.ot133_authority_payload = b"ot133-consumed-authority-v0\n"
        self.ot133_abort_payload = b"ot133-abort-v0\n"
        self.ot135_decision_payload = b"ot135-decision-v0\n"
        self.ot135_evidence_payload = b"ot135-evidence-v0\n"
        ...
        self._write(
            self.root / contract.OT133_AUTHORITY_RELATIVE,
            self.ot133_authority_payload,
        )
        self._write(
            self.root / contract.OT133_ABORT_RELATIVE,
            self.ot133_abort_payload,
        )
        self._write(
            self.root / contract.OT135_DECISION_RELATIVE,
            self.ot135_decision_payload,
        )
        self._write(
            self.root / contract.OT135_EVIDENCE_RELATIVE,
            self.ot135_evidence_payload,
        )
    ...
    def patches(self):
        return (
            ...
            mock.patch.object(
                ...
                sha256(self.ot133_authority_payload),
            ),
            mock.patch.object(
                contract,
                "OT133_ABORT_RAW_SHA256",
                sha256(self.ot133_abort_payload),
            ),
            mock.patch.object(
                contract,
                "OT135_DECISION_RAW_SHA256",
                sha256(self.ot135_decision_payload),
            ),
            mock.patch.object(
                contract,
                "OT135_EVIDENCE_RAW_SHA256",
                sha256(self.ot135_evidence_payload),
            ),
...
class ContractTests(unittest.TestCase):
    ...
    def test_01_current_ot133_ot135_and_runtime_bindings_match(self) -> None:
        for _, relative, digest in contract.FIXED_RUNTIME_BINDINGS:
            self.assertEqual(sha256((ROOT / relative).read_bytes()), digest)
        for relative, digest in (
            (contract.OT133_AUTHORITY_RELATIVE, contract.OT133_AUTHORITY_RAW_SHA256),
            (contract.OT133_ABORT_RELATIVE, contract.OT133_ABORT_RAW_SHA256),
            (contract.OT135_DECISION_RELATIVE, contract.OT135_DECISION_RAW_SHA256),
            (contract.OT135_EVIDENCE_RELATIVE, contract.OT135_EVIDENCE_RAW_SHA256),
        ):
            self.assertEqual(sha256((ROOT / relative).read_bytes()), digest)

    def test_02_preparation_binds_adapter_and_consumed_ot133_ot135_successor(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ot136-authority-test-") as directory:
            ...
            self.assertTrue(supersession["ot133_authority_consumed"])
            self.assertFalse(supersession["ot133_authority_reusable"])
