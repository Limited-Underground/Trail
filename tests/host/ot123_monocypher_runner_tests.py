#!/usr/bin/env python3
"""Adversarial, hardware-free tests for the OT-123 Monocypher comparison coordinator."""
...
SPEC = importlib.util.spec_from_file_location(
    "ot123_monocypher_runner", TOOLS / "ot123_monocypher_runner.py"
)
...
class Fixture:
    def __init__(self, directory: str) -> None:
        ...
        self.journal = self.private / "ot123-monocypher-recovery-journal.json"
...
class Ot123MonocypherRunnerTests(unittest.TestCase):
    def run_fixture(
    ...
    def test_01_exact_authority_validates_against_all_pinned_parents(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix="ot123-monocypher-parent-test-"
        ) as directory:
    ...
    def test_02_happy_path_is_app_only_two_node_and_restores(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ot123-monocypher-runner-test-") as directory:
            fixture, transport = Fixture(directory), FakeTransport()
            receipt = self.run_fixture(fixture, transport)
            self.assertEqual(receipt["result"], "two_node_monocypher_comparison_passed_and_restored")
    ...
    def test_02a_installed_readback_fails_before_any_write(self) -> None:
        for failure in (
            ...
            with self.subTest(failure=failure):
                with tempfile.TemporaryDirectory(
                    prefix="ot123-monocypher-runner-test-"
                ) as directory:
    ...
    def test_04_restore_failure_is_publicly_safe_and_recoverable(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ot123-monocypher-runner-test-") as directory:
            ...
            self.assertEqual(receipt["result"], "monocypher_comparison_execution_aborted")
            self.assertEqual(receipt["comparison_boundary"], runner.COMPARISON_BOUNDARY)
            ...
            with self.subTest(failure_type=failure_type.__name__):
                with tempfile.TemporaryDirectory(
                    ...
                    benchmark = runner.Image(
                        ...
                        BENCHMARK_PAYLOAD,
                        hashlib.sha256(BENCHMARK_PAYLOAD).hexdigest(),
                    )
                    ...
                    receipt = json.loads(fixture.receipt.read_text(encoding="ascii"))
                    self.assertEqual(receipt["result"], "recovery_failed")
                    self.assertFalse(receipt["restoration_complete"])
                    self.assertEqual(
    ...
    def test_10_source_has_no_radio_or_broad_flash_operation(self) -> None:
        source = (TOOLS / "ot123_monocypher_runner.py").read_text(encoding="utf-8")
        for forbidden in (
        ...
        self.assertIn('ROOT / ".private" / "ot123-monocypher-recovery-journal.json"', source)
    ...
    def test_19_capture_failure_closes_and_sanitizes(self) -> None:
        ...
        with (
            ...
            with self.assertRaisesRegex(
                runner.RunnerError, "^serial capture (?:failed|incomplete)$"
            ) as caught:
