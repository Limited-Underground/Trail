...
SPEC=importlib.util.spec_from_file_location("ot196_stage_operator_tests_worker",ROOT/"tools/security_policy_input_operator.py")
...
class Tests(unittest.TestCase):
    ...
    def manifest_fixture(self):
        ...
        files={}
        names=["policy/Invoke-SecurityPolicyOperator.ps1","esptool.cfg","packages/esptool/__init__.py","packages/serial/__init__.py","python.exe","python314.dll","python314._pth","policy/security_policy_input_operator.py","policy/Invoke-SecurityPolicyInputOperator.ps1"]+["policy/security_policy_"+x+".py" for x in worker.POLICY]
        for name in names:
            raw=b"[esptool]\n" if name=="esptool.cfg" else worker.PTH if name=="python314._pth" else (ROOT/"tools/Invoke-SecurityPolicyInputOperator.ps1").read_bytes() if name=="policy/Invoke-SecurityPolicyInputOperator.ps1" else b"synthetic pinned fixture"
        ...
        m={"schema":"OT198-INPUT-RUNTIME-1","root":str(capsule),"worktree":str(worktree),"files":files,"versions":{"python":"3.14.6","esptool":"5.3.1","pyserial":"3.5"},"powershell":{"path":str(host),"bytes":host.stat().st_size,"sha256":worker.digest(host.read_bytes())}}
    ...
    def test_operator_request_bound_files_and_runtime(self):
        ...
        req={"schema":"OT198-INPUT-OPERATOR-1","runtime_sha256":h,"operation":"recover","package_path":str(package),"package_sha256":worker.digest(b"{}"),"grant_path":str(grant),"grant_sha256":worker.digest(b"{}"),"origin_attempt":"a"*32}
    ...
    def test_powershell_rejects_poison_before_native_startup(self):
        ...
        script=ROOT/"tools/Invoke-SecurityPolicyInputOperator.ps1"
        def invoke(manifest):
    ...
    def test_recover_dispatch_without_candidate_and_source_mismatch(self):
        ...
        for launcher in ('Invoke-SecurityPolicyInputOperator.ps1','Invoke-SecurityPolicyOperator.ps1'):
            m['files']['policy/'+launcher]=m['files']['policy/Invoke-SecurityPolicyInputOperator.ps1']
        ...
        req={"schema":"OT198-INPUT-OPERATOR-1","runtime_sha256":h,"operation":"recover","package_path":str(package_path),"package_sha256":worker.digest(package_path.read_bytes()),"grant_path":str(grant_path),"grant_sha256":worker.digest(b"{}"),"origin_attempt":"a"*32}
        ...
        modules={"security_policy_input_bundle":SimpleNamespace(),"security_policy_input_execution":execution,"security_policy_hardware":hardware}
    ...
    def test_child_command_uses_verified_powershell(self):
        ...
        args=worker.child_args(manifest,path,sha,"InputOperator",request,"b"*64)
        self.assertEqual(str(self.root/"pwsh.exe"),str(args[0]))
        self.assertIn("-NoProfile",args);self.assertIn("-NonInteractive",args)
        self.assertEqual(str(capsule/"policy/Invoke-SecurityPolicyInputOperator.ps1"),str(args[args.index("-File")+1]))
        self.assertEqual(str(path),str(args[args.index("-Manifest")+1]))
        self.assertEqual(sha,args[args.index("-ManifestSha256")+1])
        self.assertEqual("InputOperator",args[args.index("-Mode")+1])
