...
MODULE_PATH = ROOT / "tools" / "ot150_mbedtls_psa_hardware_adapter.py"
SPEC = importlib.util.spec_from_file_location("ot150_hardware_adapter_tests", MODULE_PATH)
...
PORT_B = "PRIVATE-B"
BINDING_FIELDS = {
    "benchmark_name": "ot150-mbedtls-psa-v0.bin",
    "benchmark_bytes": 245_584,
    ...
    "protocol_start": adapter.coordinator.protocol.START.decode("ascii"),
    "protocol_ready": adapter.coordinator.protocol.READY.decode("ascii"),
...
class AdapterTests(unittest.TestCase):
    ...
    def _run_main(self, arguments, *, factory=FakeBackend):
        ...
        with (
            mock.patch.object(
                ...
                return_value=dict(BINDING_FIELDS),
            ),
    ...
    def test_01_authority_gate_returns_exact_nonreusable_grant(self) -> None:
        gate = adapter.ExecutionAuthorityGate(
            ...
            BINDING,
            Path("C:/private/ot150-mbedtls-psa-v0.bin"),
    ...
    def test_02_authority_gate_rejects_binding_before_validator(self) -> None:
        gate = adapter.ExecutionAuthorityGate(Path("C:/x"), BINDING, None, Path("C:/y"))
        with mock.patch.object(
    ...
    def test_05_execute_requires_visual_confirmation_before_bundle_or_backend(self) -> None:
        factory = mock.Mock()
        with mock.patch.object(
            adapter.authority_contract, "load_preparation_binding"
        ) as load_binding:
            code, stdout, stderr = self._run_main(
                [
                    "--port-a", PORT_A,
                    "--port-b", PORT_B,
                    "--benchmark-app", "C:/private/ot150-mbedtls-psa-v0.bin",
        ...
        self.assertEqual(stderr, "ERROR: OT-150 hardware operation failed\n")
    ...
    def test_06_preflight_cli_uses_one_backend_and_never_executes(self) -> None:
        ...
        self.assertEqual(stdout, "OK: OT-150 preflight reset completed on both nodes\n")
        self.assertEqual(len(created), 1)
        self.assertIs(preflight.call_args.args[1], created[0])
        execute.assert_not_called()

    def test_07_execute_cli_passes_preparation_binding_and_same_backend(self) -> None:
        created: list[FakeBackend] = []
        ...
        self.assertEqual((code, stderr), (0, ""))
        self.assertEqual(stdout, "OK: OT-150 two-node attempt completed and restored\n")
        self.assertEqual(len(created), 1)
    ...
    def test_08_recovery_needs_no_benchmark_or_visual_flag(self) -> None:
        ...
        self.assertEqual((code, stderr), (0, ""))
        self.assertEqual(stdout, "OK: OT-150 recovery restored all touched nodes\n")
        ...
        with mock.patch.object(
            ...
            code, stdout, stderr = self._run_main(
                [
                    "--port-a", private,
                    "--port-b", PORT_B,
                    "--benchmark-app", f"C:/{private}/ot150-mbedtls-psa-v0.bin",
    ...
    def test_13_duplicate_endpoint_aliases_fail_before_bundle_or_backend(self) -> None:
        factory = mock.Mock()
        code, stdout, stderr = self._run_main(
            [
                "--port-a", "COM7",
    ...
    def test_14_adapter_has_no_broad_flash_or_private_output_surface(self) -> None:
        ...
        self.assertNotIn("radio", " ".join(["write-flash", "read-flash"]))
