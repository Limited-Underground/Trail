#!/usr/bin/env python3
"""Focused host-only tests for the OT-130 restoration-safe coordinator."""
...
MODULE_PATH = ROOT / "tools" / "ot130_monocypher_coordinator.py"
SPEC = importlib.util.spec_from_file_location("ot130_coordinator_tests_module", MODULE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("unable to load OT-130 coordinator")
coordinator = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = coordinator
SPEC.loader.exec_module(coordinator)


BENCHMARK_PAYLOAD = b"ot130 benchmark fixture"
...
class Fixture:
    ...
    def __enter__(self):
        ...
        self.stack.enter_context(mock.patch.object(
            ...
            self.private / "ot130-monocypher-execution-journal.json",
        ))
        self.stack.enter_context(mock.patch.object(
            coordinator, "EXECUTION_RECEIPT_PATH",
            self.private / "ot130-monocypher-execution-receipt.json",
        ))
        self.stack.enter_context(mock.patch.object(
            coordinator, "RECOVERY_RECEIPT_PATH",
            self.private / "ot130-monocypher-recovery-receipt.json",
        ))
        self.stack.enter_context(mock.patch.object(
...
class Ot130CoordinatorTests(unittest.TestCase):
