#!/usr/bin/env python3
"""Focused host-only tests for the OT-143 restoration-safe coordinator."""
...
MODULE_PATH = ROOT / "tools" / "ot143_monocypher_coordinator.py"
SPEC = importlib.util.spec_from_file_location("ot143_coordinator_tests_module", MODULE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("unable to load OT-143 coordinator")
coordinator = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = coordinator
SPEC.loader.exec_module(coordinator)


BENCHMARK_PAYLOAD = b"ot143 benchmark fixture"
...
class Fixture:
    ...
    def __enter__(self):
        ...
        self.stack.enter_context(mock.patch.object(
            ...
            self.private / "ot143-monocypher-execution-journal.json",
        ))
        self.stack.enter_context(mock.patch.object(
            coordinator, "EXECUTION_RECEIPT_PATH",
            self.private / "ot143-monocypher-execution-receipt.json",
        ))
        self.stack.enter_context(mock.patch.object(
            coordinator, "RECOVERY_RECEIPT_PATH",
            self.private / "ot143-monocypher-recovery-receipt.json",
        ))
        self.stack.enter_context(mock.patch.object(
...
class Ot143CoordinatorTests(unittest.TestCase):
