...
spec = importlib.util.spec_from_file_location("runtime_bundle_tested", ROOT / "tools/security_policy_receipt_runtime_bundle.py")
...
class BundleTests(unittest.TestCase):
    ...
    def test_old_manifest_schema_refused(self):
        manifest=self.build()
        self.assertEqual(manifest['schema'],'OT203-RECEIPT-RUNTIME-1')
    ...
    def test_sync_required_file_cannot_be_removed_from_manifest(self):
        manifest=self.build()
        name='policy/security_policy_receipt_execution.py'
    ...
    def test_complete_old_and_new_policy_closure(self):
        ...
        expected=tuple('security_policy_'+name+'.py' for name in (
            ...
            'receipt_readback','receipt_observation','receipt_bundle','receipt_execution','receipt_operator',
            'receipt_backup_operator','receipt_runtime_bundle','receipt_restore_observation','receipt_boundary'))+(
            'Invoke-SecurityPolicyOperator.ps1','Invoke-SecurityPolicyReceiptOperator.ps1')
