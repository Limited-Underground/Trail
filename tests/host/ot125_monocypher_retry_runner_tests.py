#!/usr/bin/env python3
"""Adversarial, hardware-free tests for the OT-125 Monocypher comparison coordinator."""
...
SPEC = importlib.util.spec_from_file_location(
    "ot125_monocypher_retry_runner", TOOLS / "ot125_monocypher_retry_runner.py"
)
...
class Fixture:
    def __init__(self, directory: str) -> None:
        ...
        self.journal = self.private / "ot125-monocypher-corrective-retry-journal.json"
        self.receipt = self.private / "ot125-monocypher-corrective-retry-execution-receipt.json"
        self.recovery_receipt = self.private / "ot125-monocypher-corrective-retry-recovery-receipt.json"
...
class Ot125MonocypherRunnerTests(unittest.TestCase):
    def run_fixture(
    ...
    def test_01_exact_authority_validates_against_all_pinned_parents(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix="ot125-monocypher-parent-test-"
        ) as directory:
    ...
    def test_02_happy_path_is_app_only_two_node_and_restores(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ot125-monocypher-runner-test-") as directory:
    ...
    def test_02a_installed_readback_fails_before_any_write(self) -> None:
        for failure in (
    ...
    def test_10_source_has_no_radio_or_broad_flash_operation(self) -> None:
        source = (TOOLS / "ot125_monocypher_retry_runner.py").read_text(encoding="utf-8")
        for forbidden in (
        ...
        self.assertIn('ROOT / ".private" / "ot125-monocypher-corrective-retry-journal.json"', source)
    ...
    def test_22_repeated_empty_reads_get_one_fresh_cycle(self) -> None:
        frame = runner.frame_contract.PREFIX + b'{"record_kind":"header"}'
        ...
        endpoints = [
            Endpoint([b""] * runner.CAPTURE_EMPTY_READ_LIMIT),
            Endpoint([frame + b"\n"]),
        ]
        constructed = 0
    ...
    def test_25_two_empty_read_cycles_fail_fast_and_closed(self) -> None:
        class Endpoint:
            def __init__(self) -> None:
                self.port: str | None = None
                self.dtr = False
        ...
        self.assertEqual(
            [endpoint.reads for endpoint in endpoints],
            [runner.CAPTURE_EMPTY_READ_LIMIT, runner.CAPTURE_EMPTY_READ_LIMIT],
        )
        self.assertTrue(all(endpoint.closed for endpoint in endpoints))
