...
import pair_confirmation_trial as pair
...
class PairTests(unittest.TestCase):
    def setUp(self):
        ...
        self.request = {'schema': 'OT-PAIR-REQUEST-1', 'runtime_sha256': 'c'*64, 'bridge_sha256': 'd'*64, 'case': 'nonradio-confirm-and-restore', 'roles': {r: {'device_binding': r.lower()*64, 'candidate': pair.base.descriptor(self.candidates[r]), 'protected': protected, 'original_prefix': pair.base.descriptor(self.backends[r].original[pair.SPANS['application']][:589824])} for r in pair.ROLES}}
        self.bridge = Bridge(self.log)
    def grant(self, operation='execute', attempt='1'*32, origin=None):
        g = {'schema': 'OT-PAIR-GRANT-1', 'attempt': attempt, 'request_sha256': pair.sha(pair.canonical(self.request)), 'runtime_sha256': self.request['runtime_sha256'], 'operation': operation, 'origin_attempt': origin, 'attempt_count': 1, 'actions': pair.ACTIONS if operation == 'execute' else pair.RECOVERY_ACTIONS, 'issued_utc': 100, 'expires_utc': 200}
    ...
    def test_success_both_captured_before_first_write_and_close_before_restore(self):
        result = self.execute(); self.assertEqual(result['observation'], 'local_confirmed')
    ...
    def test_original_tamper_holds_custody_but_restores_other_role(self):
        self.backends['A'].fail_restore = True
        with self.assertRaises(pair.Error): self.execute()
        original = self.root/'.private'/('pair-confirmation-'+'1'*32+'-A-application.bin')
    ...
    def test_caller_mapping_mutation_cannot_replace_admitted_pair(self):
        ...
        state = pair.decode((self.root/'.private'/('pair-confirmation-'+'1'*32+'.json')).read_bytes())
    ...
    def test_radio_and_usb_grants_are_not_interchangeable(self):
        for case in ('nonradio-confirm-and-restore', 'radio-confirm-and-restore'):
            ...
            value['actions'] = pair.actions_for({'case': 'radio-confirm-and-restore' if case.startswith('nonradio') else 'nonradio-confirm-and-restore'}, 'execute')
    ...
    def test_radio_request_uses_same_restoration_scope(self):
        self.request['case'] = 'radio-confirm-and-restore'
        raw,_ = self.grant(); value = pair.decode(raw)
