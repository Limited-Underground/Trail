"""Actual OT-210 custody/worker/ROM child/Endpoint; synthetic physical boundary.
...
import security_policy_invitation_operator as op
import security_policy_invitation_backup_operator as backupop
import security_policy_invitation_bundle as bundle
import security_policy_invitation_execution as execution
...
CANDIDATE = None
OUTPUT = ROOT/'.private/ot210-operator'
...
class Tests(unittest.TestCase):
 ...
 def handle(self,route):
  ...
  class Handle:
   ...
   def write(h,value):
    ...
    return len(value)
   def read(h,size):
    t.now+=max(.001,min(h.timeout,.25));result=h.reply[:size];h.reply=h.reply[size:];return result
   def close(h):
 ...
 def dispatch(self,args,**kwargs):
  ...
  if self.fail_preflight and mode=='InvitationRom' and child['argv'][11]=='read-flash':
  ...
  try:
   with contextlib.redirect_stdout(output):
    (op.run_rom if mode=='InvitationRom' else backupop.run_rom)(self.ctx,p,h)
   if self.replay and child['argv'][11]=='run':
    copied,hh=self.store('copied-reset.json',child)
    with self.assertRaises(Exception):(op.run_rom if mode=='InvitationRom' else backupop.run_rom)(self.ctx,copied,hh)
 ...
 def backup_request(self,operation='capture',origin=None):
  ...
  return self.store(operation+'-request.json',{'schema':'OT210-INVITATION-BACKUP-OPERATOR-1','runtime_sha256':self.ctx['sha'],'operation':operation,'request_path':str(rp),'request_sha256':rh,'grant_path':str(gp),'grant_sha256':gh,'origin_attempt':origin,'execution_request':None})
 def prepare(self):
  ...
  ep,eh=self.store('stage-request.json',{'schema':'OT210-INVITATION-OPERATOR-1','runtime_sha256':self.ctx['sha'],'operation':'execute','package_path':str(pp),'package_sha256':ph,'grant_path':str(gp),'grant_sha256':gh,'origin_attempt':None})
  rp,rh=self.store('backup-request.json',self.data)
  return self.store('handoff.json',{'schema':'OT210-INVITATION-BACKUP-OPERATOR-1','runtime_sha256':self.ctx['sha'],'operation':'handoff','request_path':str(rp),'request_sha256':rh,'grant_path':None,'grant_sha256':None,'origin_attempt':'1'*32,'execution_request':{'path':str(ep),'sha256':eh}})
 ...
 def test_actual_handoff_child_endpoint_restore_both(self):
  ...
  for row in result['roles']:
   ...
   capture=self.private/f"ot210-invitation-{row['role']}-{'2'*32}.bin"
 ...
 def test_observation_failure_preserves_restore(self):
  ...
  for role in 'AB':
   ...
   self.assertFalse((self.private/f"ot210-invitation-{role}-{'2'*32}.bin").exists())
 ...
 def test_ambiguous_close_blocks_all_later_rom_and_capture(self):
  ...
  self.assertFalse(list(self.private.glob('ot210-invitation-A-*.bin')))
