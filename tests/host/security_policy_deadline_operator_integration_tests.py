"""Actual OT-212 custody/worker/ROM child/Endpoint; synthetic physical boundary.
...
import security_policy_deadline_operator as op
import security_policy_deadline_backup_operator as backupop
import security_policy_deadline_bundle as bundle
import security_policy_deadline_execution as execution
...
CANDIDATE = None
OUTPUT = ROOT/'.private/ot212-deadline'
...
class Tests(unittest.TestCase):
 def setUp(self):
  ...
  self.wrong_begin=False;self.omit_begin=False;self.trailing_receipt=False
  self.deadline_crossing=None;self.crossings=[];self.raw_reads_after_deadline=[]
  ...
  def create(ctx,bindings,transport):
   ...
   actual_factory=b.endpoint_factory
   def factory(handle,*,guard,monotonic):
    def admitted():
     valid=guard()
     if self.deadline_crossing and handle.armed:
      handle.final_guards+=1
      if handle.final_guards==self.deadline_crossing:
       self.now=handle.deadline;self.crossings.append((handle.role,self.deadline_crossing))
     return valid
    endpoint=actual_factory(handle,guard=admitted,monotonic=monotonic)
    actual_run=endpoint.run_once
    def run(challenge,deadline):
     handle.deadline=deadline
     return actual_run(challenge,deadline)
    endpoint.run_once=run
    return endpoint
   b.endpoint_factory=factory
 ...
 def handle(self,route):
  ...
  class Handle:
   ...
   def write(h,value):
    t.trace.append(('RUN',role));challenge=value.strip().split()[-1]
    begin=b'' if t.omit_begin else b'SEC_BEGIN1 '+(b'0'*32 if t.wrong_begin else challenge)+b'\r\n'
    h.reply=b'' if t.empty else b'O'*64+begin+b'SEC_EVAL1 ot187-policy-v0 '+challenge+b' pass\r\n'+(b'extra' if t.trailing_receipt else b'')
    return len(value)
   def read(h,size):
    if t.deadline_crossing and t.now>=h.deadline:t.raw_reads_after_deadline.append(role)
    ended=t.now+max(.001,min(h.timeout,.25))
    if t.deadline_crossing and ended>=h.deadline and not h.armed:
     t.now=h.deadline-0.000001;h.armed=True;h.final_guards=0
    else:t.now=ended
    result=h.reply[:size];h.reply=h.reply[size:];return result
   def close(h):
    t.trace.append(('close',role))
    if t.close_ambiguous:raise RuntimeError('synthetic close ambiguity')
    h.is_open=False
  handle=Handle();handle.role=role;handle.armed=False;handle.final_guards=0;handle.deadline=None
  return handle
 ...
 def dispatch(self,args,**kwargs):
  ...
  if self.fail_preflight and mode=='DeadlineRom' and child['argv'][11]=='read-flash':
  ...
  try:
   with contextlib.redirect_stdout(output):
    (op.run_rom if mode=='DeadlineRom' else backupop.run_rom)(self.ctx,p,h)
   if self.replay and child['argv'][11]=='run':
    copied,hh=self.store('copied-reset.json',child)
    with self.assertRaises(Exception):(op.run_rom if mode=='DeadlineRom' else backupop.run_rom)(self.ctx,copied,hh)
 ...
 def backup_request(self,operation='capture',origin=None):
  ...
  return self.store(operation+'-request.json',{'schema':'OT212-DEADLINE-BACKUP-OPERATOR-1','runtime_sha256':self.ctx['sha'],'operation':operation,'request_path':str(rp),'request_sha256':rh,'grant_path':str(gp),'grant_sha256':gh,'origin_attempt':origin,'execution_request':None})
 def prepare(self):
  ...
  ep,eh=self.store('stage-request.json',{'schema':'OT212-DEADLINE-OPERATOR-1','runtime_sha256':self.ctx['sha'],'operation':'execute','package_path':str(pp),'package_sha256':ph,'grant_path':str(gp),'grant_sha256':gh,'origin_attempt':None})
  rp,rh=self.store('backup-request.json',self.data)
  return self.store('handoff.json',{'schema':'OT212-DEADLINE-BACKUP-OPERATOR-1','runtime_sha256':self.ctx['sha'],'operation':'handoff','request_path':str(rp),'request_sha256':rh,'grant_path':None,'grant_sha256':None,'origin_attempt':'1'*32,'execution_request':{'path':str(ep),'sha256':eh}})
 def test_reserved_original_namespace_stops_before_candidate_write(self):
  from security_policy_invitation_readback_tests import namespace_fixture
  self.doCleanups()
  for name in sorted(readback.FRESH_NAMESPACES):
   fixture=Tests('test_actual_handoff_child_endpoint_restore_both');fixture.setUp()
   try:
    original=namespace_fixture(name)
    fixture.flash['A',53248]=original;fixture.original['A',53248]=original
    with self.subTest(namespace=name):
     with self.assertRaises(Exception): fixture.prepare()
     self.assertEqual(fixture.flash,fixture.original)
     self.assertFalse(any(event[0] in ('write-flash','open','RUN') for event in fixture.trace),fixture.trace)
     self.assertFalse(execution.used_path(fixture.root,'2'*32).exists())
 ...
 def test_actual_handoff_child_endpoint_restore_both(self):
  ...
  self.assertEqual(result['receipt_boundary']['schema'],'OT203-RECEIPT-BOUNDARY-1')
  self.assertTrue(result['receipt_boundary']['available'])
  for boundary in result['receipt_boundary']['roles']:
   self.assertTrue(boundary['available']);self.assertTrue(boundary['begin_observed'])
   self.assertEqual(boundary['preamble_discarded_bytes'],64)
   self.assertEqual(boundary['wire_read_bytes'],174)
   self.assertEqual(boundary['max_wire_read_bytes'],129)
  ...
  for row in result['roles']:
   ...
   capture=self.private/f"ot212-deadline-{row['role']}-{'2'*32}.bin"
 ...
 def test_wrong_begin_restores_without_opening_b(self):
  self.wrong_begin=True;p,h=self.prepare();result=backupop.run_operator(self.ctx,p,h)
  self.assertEqual(result['status'],'evaluation_failed',self.child_errors)
  self.assertNotIn(('open','B'),self.trace);self.assertEqual(self.flash,self.original)
  self.assertEqual(result['receipt_boundary']['roles'][0]['error'],'begin_invalid')
  self.assertFalse(result['receipt_boundary']['roles'][0]['begin_observed'])
 def test_both_pre_read_deadline_crossings_accept_and_restore_before_b(self):
  self.doCleanups()
  # At the last raw-read return, guard 1 is post-read admission. Guards 2 and 3
  # are the two endpoint pre-read checks after capture samples its loop clock.
  for crossing in (2,3):
   fixture=Tests('test_actual_handoff_child_endpoint_restore_both');fixture.setUp()
   try:
    fixture.deadline_crossing=crossing
    p,h=fixture.prepare();result=backupop.run_operator(fixture.ctx,p,h)
    with self.subTest(pre_read_check=crossing-1):
     self.assertEqual(result['status'],'pass',fixture.child_errors)
     self.assertEqual(fixture.crossings,[('A',crossing),('B',crossing)])
     self.assertEqual(fixture.raw_reads_after_deadline,[])
     self.assertEqual(fixture.flash,fixture.original)
     self.assertEqual(fixture.observation_reads,['A','B'])
     self.assertLess(fixture.trace.index(('original_reset','A')),fixture.trace.index(('open','B')))
     for row in result['receipt_diagnostics']['roles']:
      self.assertTrue(row['matching_receipt_observed']);self.assertTrue(row['receipt_accepted'])
      self.assertEqual((row['inner_endpoint_error'],row['capture_error']),('none','none'))
      self.assertEqual(row['read_bytes'],65)
      # Observation starts after the execute-owned deadline epoch; 29999 ms is
      # valid. The injected crossings prove deadline arrival independently.
      self.assertGreater(row['elapsed_ms'],0);self.assertLessEqual(row['elapsed_ms'],30000)
     self.assertTrue(all(row['begin_observed'] for row in result['receipt_boundary']['roles']))
     self.assertFalse((fixture.private/'security-policy-active.lock').exists())
   finally:fixture.doCleanups()
 def test_old_receipt_without_begin_never_passes(self):
  self.omit_begin=True;p,h=self.prepare();result=backupop.run_operator(self.ctx,p,h)
  self.assertEqual(result['status'],'evaluation_failed',self.child_errors)
  self.assertNotIn(('open','B'),self.trace);self.assertEqual(self.flash,self.original)
  self.assertFalse(result['receipt_boundary']['roles'][0]['begin_observed'])
 def test_post_begin_extra_output_still_refused_and_restored(self):
  self.trailing_receipt=True;p,h=self.prepare();result=backupop.run_operator(self.ctx,p,h)
  self.assertEqual(result['status'],'evaluation_failed',self.child_errors)
  self.assertNotIn(('open','B'),self.trace);self.assertEqual(self.flash,self.original)
  self.assertTrue(result['receipt_boundary']['roles'][0]['begin_observed'])
  self.assertFalse(result['receipt_diagnostics']['roles'][0]['receipt_accepted'])
 ...
 def test_observation_failure_preserves_restore(self):
  ...
  for role in 'AB':
   ...
   self.assertFalse((self.private/f"ot212-deadline-{role}-{'2'*32}.bin").exists())
 ...
 def test_ambiguous_close_blocks_all_later_rom_and_capture(self):
  ...
  self.assertFalse(list(self.private.glob('ot212-deadline-A-*.bin')))
