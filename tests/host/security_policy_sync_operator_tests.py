...
SPEC=importlib.util.spec_from_file_location("ot201_sync_operator_tests_worker",ROOT/"tools/security_policy_sync_operator.py")
...
class Tests(unittest.TestCase):
    ...
    def manifest_fixture(self):
        ...
        files={}
        names=["policy/Invoke-SecurityPolicyOperator.ps1","esptool.cfg","packages/esptool/__init__.py","packages/serial/__init__.py","python.exe","python314.dll","python314._pth","policy/security_policy_sync_operator.py","policy/Invoke-SecurityPolicySyncOperator.ps1"]+["policy/security_policy_"+x+".py" for x in worker.POLICY]
        for name in names:
            raw=b"[esptool]\n" if name=="esptool.cfg" else worker.PTH if name=="python314._pth" else (ROOT/"tools/Invoke-SecurityPolicySyncOperator.ps1").read_bytes() if name=="policy/Invoke-SecurityPolicySyncOperator.ps1" else b"synthetic pinned fixture"
        ...
        m={"schema":"OT201-SYNC-RUNTIME-1","root":str(capsule),"worktree":str(worktree),"files":files,"versions":{"python":"3.14.6","esptool":"5.3.1","pyserial":"3.5"},"powershell":{"path":str(host),"bytes":host.stat().st_size,"sha256":worker.digest(host.read_bytes())}}
    ...
    def test_previous_runtime_schemas_refused(self):
        ...
        for schema in ('OT189-RUNTIME-1','OT196-STAGE-RUNTIME-1','OT198-INPUT-RUNTIME-1'):
    ...
    def test_operator_request_bound_files_and_runtime(self):
        ...
        req={"schema":"OT201-SYNC-OPERATOR-1","runtime_sha256":h,"operation":"recover","package_path":str(package),"package_sha256":worker.digest(b"{}"),"grant_path":str(grant),"grant_sha256":worker.digest(b"{}"),"origin_attempt":"a"*32}
    ...
    def test_powershell_rejects_poison_before_native_startup(self):
        ...
        source.write_text('#include <cstdio>\n#include <cstdlib>\n#include <cstring>\nint main(){const char*cfg=std::getenv("ESPTOOL_CFGFILE");const char*expected=std::getenv("OT201_EXPECTED_CFG");if(!cfg||!expected||std::strcmp(cfg,expected)!=0||std::getenv("ESPTOOL_OPEN_PORT_ATTEMPTS")||std::getenv("PYTHONOPTIMIZE")||std::getenv("PYTHONPATH")||std::getenv("PYTHONHOME"))return 6;const char*p=std::getenv("OT201_SENTINEL_PATH");if(!p)return 4;FILE*f=std::fopen(p,"wb");if(!f)return 5;std::fputs("executed",f);std::fclose(f);return 0;}')
        ...
        env=dict(os.environ);env["OT201_SENTINEL_PATH"]=str(marker);env["OT201_EXPECTED_CFG"]=str(root/"esptool.cfg");env["ESPTOOL_CFGFILE"]=str(self.root/"poison.cfg");env["ESPTOOL_OPEN_PORT_ATTEMPTS"]="999"
        env.update(PYTHONOPTIMIZE='2',PYTHONPATH=str(self.root/'poison'),PYTHONHOME=str(self.root/'poison'))
        script=ROOT/"tools/Invoke-SecurityPolicySyncOperator.ps1"
        def invoke(manifest):
    ...
    def test_recover_dispatch_without_candidate_and_source_mismatch(self):
        ...
        for launcher in ('Invoke-SecurityPolicySyncOperator.ps1','Invoke-SecurityPolicyOperator.ps1'):
            m['files']['policy/'+launcher]=m['files']['policy/Invoke-SecurityPolicySyncOperator.ps1']
        ...
        req={"schema":"OT201-SYNC-OPERATOR-1","runtime_sha256":h,"operation":"recover","package_path":str(package_path),"package_sha256":worker.digest(package_path.read_bytes()),"grant_path":str(grant_path),"grant_sha256":worker.digest(b"{}"),"origin_attempt":"a"*32}
        ...
        modules={"security_policy_sync_bundle":SimpleNamespace(),"security_policy_sync_execution":execution,"security_policy_hardware":hardware}
