...
import security_policy_receipt_runtime_bundle as runtime
...
class IsolatedTests(unittest.TestCase):
    ...
    def launch(self,mode,request=None,*,poison=False,manifest=None,manifest_sha=None):
        ...
        argv=[str(self.pwsh),'-NoProfile','-NonInteractive','-File',str(self.capsule/'policy/Invoke-SecurityPolicyReceiptOperator.ps1'),
    ...
    def stage_request(self):
        ...
        self.save(f'ot203-receipt-grant-{attempt}.used',{'package_sha256':package_sha,'grant':grant_ref['sha256'],'runtime_sha256':self.runtime_sha})
        op=self.save('stage-op.json',{'schema':'OT203-RECEIPT-OPERATOR-1','runtime_sha256':self.runtime_sha,'operation':'recover',
        ...
        return {'schema':'OT203-RECEIPT-ROM-1','runtime_sha256':self.runtime_sha,'operator_request':op,
    ...
    def backup_request(self):
        ...
        op=self.save('backup-op.json',{'schema':'OT203-RECEIPT-BACKUP-OPERATOR-1','runtime_sha256':self.runtime_sha,'operation':'capture',
        ...
        return {'schema':'OT203-RECEIPT-BACKUP-ROM-1','runtime_sha256':self.runtime_sha,'operator_request':op,
    ...
    def test_stage_rom_actual_isolation_and_stale_authority(self):
        req=self.stage_request();self.reached(self.launch('ReceiptRom',req,poison=True))
        self.refused(self.launch('ReceiptRom',req))  # Exact child replay is consumed.
        req['argv'][-1]='erase-flash';self.refused(self.launch('ReceiptRom',req))
        req['argv'][-1]='read-mac';req['schema']='OT201-SYNC-ROM-1';self.refused(self.launch('ReceiptRom',req))
        req['schema']='OT203-RECEIPT-ROM-1';execution.used_path(self.work,self.recovery_attempt).write_bytes(b'{}')
        self.refused(self.launch('ReceiptRom',req))
    def test_backup_rom_actual_isolation_and_stale_phase(self):
        req=self.backup_request();self.reached(self.launch('ReceiptBackupRom',req,poison=True))
        req['argv'][-1]='erase-flash';self.refused(self.launch('ReceiptBackupRom',req))
        req['argv'][-1]='read-mac';req['schema']='OT201-SYNC-BACKUP-ROM-1';self.refused(self.launch('ReceiptBackupRom',req))
        req['schema']='OT203-RECEIPT-BACKUP-ROM-1';path=self.private/('security-policy-backup-'+self.backup_attempt+'-journal.jsonl')
        with path.open('ab') as out:out.write(encoded({'seq':2,'event':'read_intent','role':'B'})+b'\n')
        self.refused(self.launch('ReceiptBackupRom',req))
    ...
    def test_wrong_runtime_and_old_operator_envelope_never_reach_boundary(self):
        ...
        self.refused(self.launch('ReceiptRom',changed))
    ...
    def test_raw_execute_envelope_requires_typed_backup_handoff(self):
        ...
        self.refused(self.launch('ReceiptOperator',envelope))
