"""OT216 actual protected BLE backend/crypto proof, with optional pinned scalar reuse.
...
def run(output, reuse=None):
    ...
    result = {'schema': 'OT216-CONFIRMATION-BACKEND-HOST-1', 'result': 'failed', 'hardware': False}
    try:
        ...
        frozen = load(ROOT / 'tests/host/security_policy_invitation_lifecycle_tests.py', 'ot216_commands')
        commands = frozen.Commands(output)
        module = load(ROOT / 'tools/noise_xk_independent_interop.py', 'ot216_crypto')
        ...
        cpp = ROOT / 'tests/host/security_confirmation_ble_backend_tests.cpp'
        target = ROOT / 'firmware/targets/heltec_v4_bench/main'
        stubs = ROOT / 'tests/host/security_confirmation_ble_stubs'
        sdk_stubs = ROOT / 'tests/host/security_invitation_target_stubs'
        header = target / 'confirmation_evaluation_backend.hpp'
        for include in (stubs, sdk_stubs, target, ROOT / 'firmware/components/companion/include', ROOT / 'firmware/components/time/include'):
            flags += ['-I', str(include)]
        extra_cpp = [target / name for name in ('confirmation_evaluation_backend.cpp', 'confirmation_nonowning_entropy.cpp', 'heltec_v4_secure_random.cpp')]
        extra_cpp += [ROOT / name for name in ('firmware/components/security/src/serialized_secure_random.cpp', 'firmware/components/persistence/src/persistent_storage_kv.cpp', 'firmware/components/companion/src/companion_device_name_owner.cpp')]
        ...
        sources += extra_cpp + list(stubs.glob('*.h')) + [target / 'confirmation_nonowning_entropy.hpp', target / 'confirmation_nvs_backend.hpp', ROOT / 'firmware/components/companion/include/opentrail/companion_confirmation_codec.hpp', ROOT / 'firmware/components/companion/include/opentrail/companion_configuration_dispatcher.hpp', ROOT / 'firmware/components/security_evaluation/include/opentrail/evaluation_confirmation_owner.hpp']
        ...
        for index, relative in enumerate(frozen.COMMON):
            ...
            commands.run([CXX, '-std=c++17', '-fno-exceptions', '-fno-rtti', '-Werror', *flags,
                         ...
                         env=env, check=True)
            objects.append(obj)
        for index, source in enumerate(extra_cpp):
            obj = output / f'backend-{index}.o'
            commands.run([CXX, '-std=c++17', '-fno-exceptions', '-fno-rtti', '-Werror', *flags, '-MMD', '-MF', output / f'backend-{index}.d', '-c', source, '-o', obj], env=env, check=True)
            objects.append(obj)
        exe = output / 'confirmation-ble-backend-tests.exe'
        commands.run([CXX, '-std=c++17', '-fno-exceptions', '-fno-rtti', '-Werror', *flags,
                      '-Wl,--gc-sections', '-MMD', '-MF', output / 'confirmation-ble-backend-tests.d',
                      '-Wl,--wrap=sodium_init', '-Wl,--wrap=ot_noise_xk_split', '-Wl,--wrap=crypto_aead_chacha20poly1305_ietf_encrypt', '-Wl,--wrap=crypto_aead_chacha20poly1305_ietf_decrypt', cpp, *objects, '-o', exe], env=env, check=True)
        completed = commands.run([exe], env=env, check=True, timeout=60)
        need(completed.stdout.startswith('PASS ') and ' actual protected confirmation backend groups' in completed.stdout,
             'test_report_missing')
