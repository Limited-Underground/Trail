#!/usr/bin/env python3
"""Adversarial host tests for the OT-133 executable bundle authority."""
...
TOOL_PATH = ROOT / "tools" / "ot133_monocypher_execution_authority.py"
SPEC = importlib.util.spec_from_file_location("ot133_authority", TOOL_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("OT-133 authority tool unavailable")
...
class Fixture:
    def __init__(self, directory: str) -> None:
        ...
        self.ot131_authority_payload = b"ot131-consumed-authority-v0\n"
        self.ot131_abort_payload = b"ot131-abort-v0\n"
        self.ot132_decision_payload = b"ot132-decision-v0\n"
        self.ot132_evidence_payload = b"ot132-evidence-v0\n"
        ...
        self._write(
            self.root / contract.OT131_AUTHORITY_RELATIVE,
            self.ot131_authority_payload,
        )
        self._write(
            self.root / contract.OT131_ABORT_RELATIVE,
            self.ot131_abort_payload,
        )
        self._write(
            self.root / contract.OT132_DECISION_RELATIVE,
            self.ot132_decision_payload,
        )
        self._write(
            self.root / contract.OT132_EVIDENCE_RELATIVE,
            self.ot132_evidence_payload,
        )
    ...
    def patches(self):
        return (
            ...
            mock.patch.object(
                ...
                sha256(self.ot131_authority_payload),
            ),
            mock.patch.object(
                contract,
                "OT131_ABORT_RAW_SHA256",
                sha256(self.ot131_abort_payload),
            ),
            mock.patch.object(
                contract,
                "OT132_DECISION_RAW_SHA256",
                sha256(self.ot132_decision_payload),
            ),
            mock.patch.object(
                contract,
                "OT132_EVIDENCE_RAW_SHA256",
                sha256(self.ot132_evidence_payload),
            ),
...
class ContractTests(unittest.TestCase):
    ...
    def test_01_current_ot131_ot132_and_runtime_bindings_match(self) -> None:
        for _, relative, digest in contract.FIXED_RUNTIME_BINDINGS:
            self.assertEqual(sha256((ROOT / relative).read_bytes()), digest)
        for relative, digest in (
            (contract.OT131_AUTHORITY_RELATIVE, contract.OT131_AUTHORITY_RAW_SHA256),
            (contract.OT131_ABORT_RELATIVE, contract.OT131_ABORT_RAW_SHA256),
            (contract.OT132_DECISION_RELATIVE, contract.OT132_DECISION_RAW_SHA256),
            (contract.OT132_EVIDENCE_RELATIVE, contract.OT132_EVIDENCE_RAW_SHA256),
        ):
            self.assertEqual(sha256((ROOT / relative).read_bytes()), digest)

    def test_02_preparation_binds_adapter_and_consumed_ot131_ot132_successor(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ot133-authority-test-") as directory:
            ...
            self.assertTrue(supersession["ot131_authority_consumed"])
            self.assertFalse(supersession["ot131_authority_reusable"])
    ...
    def test_10_authority_tool_has_no_hardware_surface(self) -> None:
        ...
        self.assertIn("tools/ot132_monocypher_protocol_runner.py", source)
