...
import security_policy_input_operator as op
import security_policy_input_backup_operator as backupop
...
class Tests(unittest.TestCase):
 ...
 def dispatch(self,args,**kwargs):
  ...
  if self.fail_preflight and mode=='InputRom' and child['argv'][11]=='read-flash':
  ...
  try:
   with contextlib.redirect_stdout(output):
    (op.run_rom if mode=='InputRom' else backupop.run_rom)(self.ctx,p,h)
   if self.replay and child['argv'][11]=='run':
    copied,hh=self.store('copied-reset.json',child)
    with self.assertRaises(Exception):(op.run_rom if mode=='InputRom' else backupop.run_rom)(self.ctx,copied,hh)
 ...
 def backup_request(self,operation='capture',origin=None):
  ...
  return self.store(operation+'-request.json',{'schema':'OT198-INPUT-BACKUP-OPERATOR-1','runtime_sha256':self.ctx['sha'],'operation':operation,'request_path':str(rp),'request_sha256':rh,'grant_path':str(gp),'grant_sha256':gh,'origin_attempt':origin,'execution_request':None})
 def prepare(self):
  ...
  ep,eh=self.store('stage-request.json',{'schema':'OT198-INPUT-OPERATOR-1','runtime_sha256':self.ctx['sha'],'operation':'execute','package_path':str(pp),'package_sha256':ph,'grant_path':str(gp),'grant_sha256':gh,'origin_attempt':None})
  rp,rh=self.store('backup-request.json',self.data)
  return self.store('handoff.json',{'schema':'OT198-INPUT-BACKUP-OPERATOR-1','runtime_sha256':self.ctx['sha'],'operation':'handoff','request_path':str(rp),'request_sha256':rh,'grant_path':None,'grant_sha256':None,'origin_attempt':'1'*32,'execution_request':{'path':str(ep),'sha256':eh}})
