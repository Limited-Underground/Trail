...
import security_policy_invitation_restore_observation as stage
...
class Tests(unittest.TestCase):
    def setUp(self):
        ...
        self.image = stage.readback.ImageBinding(stage.readback.SYNC_CONTRACT,
            "heltec_v4_invitation_eval", "b" * 64)
        self.observation = stage.Observation(self.row["nvs"], "A", expected_image=self.image)

    def test_predecessor_image_binding_is_not_receipt_image_custody(self):
        before = list(self.backend.trace)
        with self.assertRaises(Exception):
            stage.Observation(self.row["nvs"], "A", expected_image=sync_fixtures.SYNC_IMAGE)
    ...
    def test_capture_before_actual_full_restoration(self):
        ...
        captures = list((self.root / ".private").glob("ot210-invitation-*.bin"))
    ...
    def test_existing_capture_is_not_overwritten(self):
        path = self.root / ".private" / f"ot210-invitation-A-{'a' * 32}.bin"
    ...
    def test_close_failure_blocks_all_rom_and_disk_capture(self):
        ...
        self.assertFalse(list((self.root / ".private").glob("ot210-invitation-*.bin")))
