...
esp_err_t nvs_open(const char* name,int mode,nvs_handle_t* out){
 if(std::string(name)!="ot198diag")return ot194_nvs_open(name,mode,out);
...
int main(int argc,char** argv){
 ...
 if(scenario=="invalid_prefix")input[0]='X';
 if(scenario=="invalid_hex")input[input.size()-2]='G';
 if(scenario=="buffer_limit")input=std::string(96,'x');
 ...
 else if(scenario=="no_input"||scenario=="partial"||scenario=="invalid"||scenario=="input_fault"||scenario=="actual_input_fault"||scenario=="poll_cap"||scenario=="late_first"||scenario=="invalid_prefix"||scenario=="invalid_hex"||scenario=="buffer_limit")CHECK(last==Stage::input_result&&error==((scenario=="no_input"||scenario=="late_first")?Error::idle_timeout:scenario=="poll_cap"?Error::loop_limit:scenario=="invalid_prefix"?Error::invalid_prefix:scenario=="invalid_hex"?Error::invalid_hex:scenario=="buffer_limit"?Error::buffer_limit:scenario=="partial"?Error::assembly_timeout:scenario=="invalid"?Error::invalid_length:Error::console_fault)&&wire.empty());
 ...
 if(scenario=="success"||scenario=="actual_success"||scenario=="last_first")CHECK(wire=="SEC_EVAL1 ot187-policy-v0 "+challenge+" pass"+(scenario=="actual_success"?"\r\n":"\n")&&std::find(events.begin(),events.end(),"nvs_write")!=events.end());
