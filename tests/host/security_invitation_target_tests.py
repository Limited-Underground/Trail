"""OT-208 real target body/SDK-boundary proof; no device or network access.
...
TARGET = ROOT / 'firmware/targets/heltec_v4_invitation_eval/main'
...
def run(output, reuse):
    ...
    result = {'schema': 'OT208-INVITATION-TARGET-HOST-1', 'result': 'failed', 'hardware': False}
    try:
        ...
        frozen = load(ROOT / 'tests/host/security_policy_invitation_lifecycle_tests.py', 'ot208_commands')
        commands = frozen.Commands(output)
        module = load(ROOT / 'tools/noise_xk_independent_interop.py', 'ot208_crypto')
        ...
        sources = [Path(__file__), REPORT, *[path for path in STUBS.rglob('*') if path.is_file()],
                   TARGET / 'app_main.cpp', TARGET / 'nvs_invitation_backend.hpp', TARGET / 'invitation_evaluation.hpp',
                   ROOT / 'tests/host/security_invitation_target_tests.cpp']
        ...
        for index, source in enumerate([*[ROOT / path for path in common], TARGET / 'app_main.cpp',
            ...
            commands.run([CXX, '-std=c++17', '-fno-exceptions', '-fno-rtti', '-Werror', *flags,
                         ...
                         env=env, check=True)
            objects.append(obj)
        exe = output / 'invitation-target-tests.exe'
        commands.run([CXX, '-Wl,--gc-sections', '-Wl,--wrap=sodium_init',
                      '-Wl,--wrap=sodium_memzero', *objects, '-o', exe], env=env, check=True)
        completed = commands.run([exe], env=env, check=True, timeout=60)
        need(completed.stdout.startswith('PASS ') and ' actual invitation target groups' in completed.stdout,
             'test_report_missing')
