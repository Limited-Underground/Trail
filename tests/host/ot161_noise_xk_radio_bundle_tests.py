#!/usr/bin/env python3
"""Focused adversarial tests for the host-only OT-161 bundle."""
...
import ot161_noise_xk_radio_bundle as bundle  # noqa: E402
...
class Tests(unittest.TestCase):
    ...
    def test_02_preparation_is_deterministic_frozen_and_grants_no_authority(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ot161-bundle-") as directory:
            ...
            self.assertEqual(value["schema"], "OT161NXBP0")
    ...
    def test_10_source_has_no_execution_persistence_or_authority_surface(self) -> None:
        source_path = ROOT / "tools/ot161_noise_xk_radio_bundle.py"
    ...
    def test_11_record_is_minimal_corrected_runtime_substitution(self) -> None:
        record = bundle.decode_canonical(
            (ROOT / bundle.PREPARATION_RELATIVE).read_bytes()
        )
        predecessor = bundle.decode_canonical(
            (
                ROOT
                / "tests/benchmarks/crypto/"
                "OT-157-OT005-LIBSODIUM-NOISE-XK-RADIO-"
                "EXECUTABLE-BUNDLE-PREPARATION-V0.json"
            ).read_bytes()
        )
        self.assertEqual(set(record), set(predecessor))
        for key in (
            "artifact_kind", "status", "recorded_date", "version",
            "execution_contract", "firmware", "images", "privacy",
            "authority", "claims",
        ):
            self.assertEqual(record[key], predecessor[key])
        self.assertEqual(
            set(record["bindings"]), {"ot153_preparation", "runtime"}
        )
        self.assertEqual(
            record["bindings"]["ot153_preparation"],
            predecessor["bindings"]["ot153_preparation"],
        )
        self.assertEqual(
            record["bindings"]["runtime"],
            {
                ...
                "hardware_adapter": {
                    "path": "tools/ot160_noise_xk_radio_hardware_adapter.py",
                    "bytes": 3952,
                    "raw_sha256": (
