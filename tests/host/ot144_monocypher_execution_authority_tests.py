#!/usr/bin/env python3
"""Adversarial host tests for the OT-144 executable bundle authority."""
...
TOOL_PATH = ROOT / "tools" / "ot144_monocypher_execution_authority.py"
SPEC = importlib.util.spec_from_file_location("ot144_authority", TOOL_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("OT-144 authority tool unavailable")
...
class Fixture:
    def __init__(self, directory: str) -> None:
        ...
        self.ot143_authority_payload = b"ot143-unconsumed-unexecutable-authority-v0\n"
        self.ot142_target_source_payload = b"ot142-corrected-target-v0\n"
        self.ot142_real_library_test_payload = b"ot142-real-library-test-v0\n"
        self.ot142_target_test_payload = b"ot142-target-test-v0\n"
        self.ot143_build_evidence_payload = b"ot143-build-evidence-v0\n"
        ...
        self._write(
            self.root / contract.OT143_AUTHORITY_RELATIVE,
            self.ot143_authority_payload,
        )
        self._write(
            self.root / contract.OT142_TARGET_SOURCE_RELATIVE,
            self.ot142_target_source_payload,
        )
        self._write(
            self.root / contract.OT142_REAL_LIBRARY_TEST_RELATIVE,
            self.ot142_real_library_test_payload,
        )
        self._write(
            self.root / contract.OT142_TARGET_TEST_RELATIVE,
            self.ot142_target_test_payload,
        )
        self._write(
            self.root / contract.OT143_BUILD_EVIDENCE_RELATIVE,
            self.ot143_build_evidence_payload,
        )
        self.artifact_payloads = (
            ("application_bin", contract.BENCHMARK_NAME, self.benchmark_payload),
            ("application_elf", "fixture.elf", b"elf-v0"),
            ("linker_map", "fixture.map", b"map-v0"),
            ("bootloader_bin", "bootloader.bin", b"bootloader-v0"),
            ("partition_table_bin", "partition-table.bin", b"partition-v0"),
            ("generated_sdkconfig", "sdkconfig", b"sdkconfig-v0"),
        )
        for _, name, payload in self.artifact_payloads:
            self._write(self.root / name, payload)
    ...
    def patches(self):
        return (
            ...
            mock.patch.object(
                contract,
                "OT143_AUTHORITY_RAW_SHA256",
                sha256(self.ot143_authority_payload),
            ),
            mock.patch.object(
                contract,
                "OT142_TARGET_SOURCE_RAW_SHA256",
                sha256(self.ot142_target_source_payload),
            ),
            mock.patch.object(
                contract,
                "OT142_REAL_LIBRARY_TEST_RAW_SHA256",
                sha256(self.ot142_real_library_test_payload),
            ),
            mock.patch.object(
                contract,
                "OT142_TARGET_TEST_RAW_SHA256",
                sha256(self.ot142_target_test_payload),
            ),
            mock.patch.object(
                contract,
                "OT143_BUILD_EVIDENCE_RAW_SHA256",
                sha256(self.ot143_build_evidence_payload),
            ),
            mock.patch.object(
                contract,
                "OT142_ARTIFACT_TUPLE",
                tuple(
                    (role, name, len(payload), sha256(payload))
                    for role, name, payload in self.artifact_payloads
                ),
            ),
...
class ContractTests(unittest.TestCase):
    ...
    def test_01_current_ot143_ot142_and_runtime_bindings_match(self) -> None:
        for _, relative, digest in contract.FIXED_RUNTIME_BINDINGS:
            self.assertEqual(sha256((ROOT / relative).read_bytes()), digest)
        source = (ROOT / contract.OT142_TARGET_SOURCE_RELATIVE).read_bytes()
        self.assertEqual(
            sha256(source.replace(b"\r\n", b"\n")),
            contract.OT142_TARGET_SOURCE_RAW_SHA256,
        )
        for relative, digest in (
            (contract.OT143_AUTHORITY_RELATIVE, contract.OT143_AUTHORITY_RAW_SHA256),
            (
                contract.OT142_REAL_LIBRARY_TEST_RELATIVE,
                contract.OT142_REAL_LIBRARY_TEST_RAW_SHA256,
            ),
            (contract.OT142_TARGET_TEST_RELATIVE, contract.OT142_TARGET_TEST_RAW_SHA256),
            (
                contract.OT143_BUILD_EVIDENCE_RELATIVE,
                contract.OT143_BUILD_EVIDENCE_RAW_SHA256,
            ),
        ):
            self.assertEqual(sha256((ROOT / relative).read_bytes()), digest)

    def test_02_preparation_supersedes_unconsumed_unexecutable_ot143(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ot144-authority-test-") as directory:
            ...
            self.assertFalse(supersession["ot143_authority_consumed"])
            self.assertFalse(supersession["ot143_authority_executable"])
            self.assertFalse(supersession["ot143_authority_reusable"])
            self.assertFalse(supersession["replacement_attempt_inherited"])
            self.assertFalse(supersession["ot143_hardware_io_occurred"])
            self.assertEqual(
                supersession["reason"],
                "coordinator_binding_mismatch_detected_before_device_io",
            )
            self.assertEqual(
                supersession["ot143_terminal_boundary"],
                "unconsumed_unexecutable_before_device_io",
            )
            accepted = value["accepted_corrected_target"]
            self.assertEqual(accepted["accepted_commit"], contract.OT142_ACCEPTED_COMMIT)
            self.assertEqual(
                accepted["build_evidence"]["raw_sha256"],
                sha256(fixture.ot143_build_evidence_payload),
            )
            self.assertEqual(value["corrected_target"]["source_input_count"], 14)
            self.assertEqual(len(value["corrected_target"]["artifact_tuple"]), 6)
            self.assertTrue(
                value["corrected_target"]["only_application_bin_is_writable"]
            )
            self.assertFalse(value["claims"]["execution_authorized"])
            self.assertEqual(
                value["status"], "executable_bundle_prepared_authority_pending"
            )
            source_path = fixture.root / contract.OT142_TARGET_SOURCE_RELATIVE
            source_path.write_bytes(
                fixture.ot142_target_source_payload.replace(b"\n", b"\r\n")
            )
            self.assertEqual(
    ...
    def test_04_exact_images_and_adapter_identity_are_required(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ot144-authority-test-") as directory:
            ...
            with self.assertRaisesRegex(
                contract.ContractError, "artifact tuple digest"
            ):
                contract.build_preparation(
                    fixture.benchmark, fixture.restore, fixture.adapter
                )
    ...
    def test_10_authority_tool_has_no_hardware_surface(self) -> None:
        ...
        self.assertIn(
            'ADAPTER_RELATIVE = "tools/ot144_monocypher_hardware_adapter.py"',
            source,
        )
        self.assertNotIn("tools/ot129_monocypher_protocol_runner.py", source)
        self.assertIn("tools/ot135_monocypher_protocol_runner.py", source)
