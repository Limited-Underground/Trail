...
SPEC=importlib.util.spec_from_file_location("ot212_deadline_operator_tests_worker",ROOT/"tools/security_policy_deadline_operator.py")
...
class Tests(unittest.TestCase):
    ...
    def manifest_fixture(self):
        ...
        files={}
        names=["policy/Invoke-SecurityPolicyOperator.ps1","esptool.cfg","packages/esptool/__init__.py","packages/serial/__init__.py","python.exe","python314.dll","python314._pth","policy/security_policy_deadline_operator.py","policy/Invoke-SecurityPolicyDeadlineOperator.ps1"]+["policy/security_policy_"+x+".py" for x in worker.POLICY]
        for name in names:
            raw=b"[esptool]\n" if name=="esptool.cfg" else worker.PTH if name=="python314._pth" else (ROOT/"tools/Invoke-SecurityPolicyDeadlineOperator.ps1").read_bytes() if name=="policy/Invoke-SecurityPolicyDeadlineOperator.ps1" else b"synthetic pinned fixture"
        ...
        m={"schema":"OT212-DEADLINE-RUNTIME-1","root":str(capsule),"worktree":str(worktree),"files":files,"versions":{"python":"3.14.6","esptool":"5.3.1","pyserial":"3.5"},"powershell":{"path":str(host),"bytes":host.stat().st_size,"sha256":worker.digest(host.read_bytes())}}
        path,h=self.write("manifest.json",encoded(m))
        return m,path,h
    def test_valid_manifest(self):
        m,p,h=self.manifest_fixture();self.assertEqual(m,worker.verify_manifest(p,h))
    def test_factory_selects_successor_without_mutating_frozen_endpoint(self):
        import security_policy_hardware as hardware
        import security_policy_deadline_endpoint as deadline
        import security_policy_endpoint as frozen_endpoint
        import security_policy_receipt_boundary as boundary
        from security_policy_receipt_boundary_tests import Handle,BEGIN,RECEIPT,C
        bindings=tuple(hardware.RoleBinding(role,'fixture-'+role,'0123456789a'+str(index))
                       for index,role in enumerate('AB'))
        backend=worker.make_backend({},bindings,object())
        backend._opening_role='A'
        handle=Handle([BEGIN,RECEIPT])
        endpoint=backend.endpoint_factory(handle,guard=lambda:True,monotonic=lambda:handle.now)
        self.assertIs(type(endpoint.endpoint.endpoint),deadline.Endpoint)
        self.assertEqual(RECEIPT.replace(b'\r\n',b'\n'),endpoint.run_once(C,30.0))
        self.assertTrue(endpoint.close())
        ctx={'manifest':{'files':{'policy/security_policy_'+name+'.py':{} for name in worker.POLICY}}}
        result=worker.with_diagnostics(ctx,backend,{'status':'fixture'})
        self.assertTrue(result['receipt_diagnostics']['roles'][0]['receipt_accepted'])
        self.assertTrue(result['receipt_boundary']['roles'][0]['begin_observed'])
        old=boundary.ReceiptBoundaryEndpoint(Handle([]),role='A',monotonic=lambda:0)
        self.assertIs(type(old.endpoint.endpoint),frozen_endpoint.Endpoint)
    def test_previous_runtime_schemas_refused(self):
        m,p,h=self.manifest_fixture()
        for schema in ('OT189-RUNTIME-1','OT196-STAGE-RUNTIME-1','OT198-INPUT-RUNTIME-1','OT201-SYNC-RUNTIME-1','OT203-RECEIPT-RUNTIME-1','OT210-INVITATION-RUNTIME-1'):
            with self.subTest(schema=schema):
                p.write_bytes(encoded({**m,'schema':schema}))
                with self.assertRaises(worker.OperatorError):
                    worker.verify_manifest(p,worker.digest(p.read_bytes()))
    ...
    def test_operator_request_bound_files_and_runtime(self):
        ...
        req={"schema":"OT212-DEADLINE-OPERATOR-1","runtime_sha256":h,"operation":"recover","package_path":str(package),"package_sha256":worker.digest(b"{}"),"grant_path":str(grant),"grant_sha256":worker.digest(b"{}"),"origin_attempt":"a"*32}
        ...
        for key,value in [("schema","OT210-INVITATION-OPERATOR-1"),("schema","OT198-INPUT-OPERATOR-1"),("schema","OT196-STAGE-OPERATOR-1"),("runtime_sha256","0"*64),("origin_attempt",None),("extra",True)]:
    ...
    def test_powershell_rejects_poison_before_native_startup(self):
        ...
        source.write_text('#include <cstdio>\n#include <cstdlib>\n#include <cstring>\nint main(){const char*cfg=std::getenv("ESPTOOL_CFGFILE");const char*expected=std::getenv("OT212_EXPECTED_CFG");if(!cfg||!expected||std::strcmp(cfg,expected)!=0||std::getenv("ESPTOOL_OPEN_PORT_ATTEMPTS")||std::getenv("PYTHONOPTIMIZE")||std::getenv("PYTHONPATH")||std::getenv("PYTHONHOME"))return 6;const char*p=std::getenv("OT212_SENTINEL_PATH");if(!p)return 4;FILE*f=std::fopen(p,"wb");if(!f)return 5;std::fputs("executed",f);std::fclose(f);return 0;}')
        ...
        env=dict(os.environ);env["OT212_SENTINEL_PATH"]=str(marker);env["OT212_EXPECTED_CFG"]=str(root/"esptool.cfg");env["ESPTOOL_CFGFILE"]=str(self.root/"poison.cfg");env["ESPTOOL_OPEN_PORT_ATTEMPTS"]="999"
        env.update(PYTHONOPTIMIZE='2',PYTHONPATH=str(self.root/'poison'),PYTHONHOME=str(self.root/'poison'))
        script=ROOT/"tools/Invoke-SecurityPolicyDeadlineOperator.ps1"
        def invoke(manifest):
        ...
        for schema in ('OT189-RUNTIME-1','OT196-STAGE-RUNTIME-1','OT198-INPUT-RUNTIME-1','OT201-SYNC-RUNTIME-1','OT203-RECEIPT-RUNTIME-1','OT210-INVITATION-RUNTIME-1'):
            self.assertNotEqual(0,invoke({**m,'schema':schema}).returncode)
    ...
    def test_recover_dispatch_without_candidate_and_source_mismatch(self):
        ...
        for launcher in ('Invoke-SecurityPolicyDeadlineOperator.ps1','Invoke-SecurityPolicyOperator.ps1'):
            m['files']['policy/'+launcher]=m['files']['policy/Invoke-SecurityPolicyDeadlineOperator.ps1']
        ...
        req={"schema":"OT212-DEADLINE-OPERATOR-1","runtime_sha256":h,"operation":"recover","package_path":str(package_path),"package_sha256":worker.digest(package_path.read_bytes()),"grant_path":str(grant_path),"grant_sha256":worker.digest(b"{}"),"origin_attempt":"a"*32}
        ...
        modules={"security_policy_deadline_bundle":SimpleNamespace(),"security_policy_deadline_execution":execution,"security_policy_hardware":hardware}
        with patch.object(worker.importlib,"import_module",side_effect=lambda n:modules[n]),patch.object(worker,"make_transport",return_value="transport"),patch.object(worker,"make_backend",side_effect=hardware.Backend),patch.object(worker,"with_diagnostics",side_effect=lambda c,b,r:r),patch.object(worker,"audit_loaded_modules",return_value=True):
            ...
            req['operation']='execute';req['origin_attempt']=None;request.write_bytes(encoded(req))
            with self.assertRaises(worker.OperatorError):
                worker.run_operator(ctx,request,worker.digest(request.read_bytes()))
            self.assertEqual(1,hardware.Backend.call_count)
            req['operation']='recover';req['origin_attempt']='a'*32
    ...
    def test_child_command_uses_verified_powershell(self):
        ...
        args=worker.child_args(manifest,path,sha,"DeadlineOperator",request,"b"*64)
        self.assertEqual(str(self.root/"pwsh.exe"),str(args[0]))
        self.assertIn("-NoProfile",args);self.assertIn("-NonInteractive",args)
        self.assertEqual(str(capsule/"policy/Invoke-SecurityPolicyDeadlineOperator.ps1"),str(args[args.index("-File")+1]))
        self.assertEqual(str(path),str(args[args.index("-Manifest")+1]))
        self.assertEqual(sha,args[args.index("-ManifestSha256")+1])
        self.assertEqual("DeadlineOperator",args[args.index("-Mode")+1])
        self.assertEqual(str(request),str(args[args.index("-Request")+1]))
        self.assertEqual("b"*64,args[args.index("-RequestSha256")+1])
        for mode in ('InputOperator','InputRom','InputBackup','InputBackupRom','Operator','Rom','InvitationOperator','InvitationRom','InvitationBackup','InvitationBackupRom'):
            with self.subTest(mode=mode),self.assertRaises(worker.OperatorError):
                worker.child_args(manifest,path,sha,mode,request,'b'*64)
