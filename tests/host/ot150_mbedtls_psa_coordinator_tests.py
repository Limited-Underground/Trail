#!/usr/bin/env python3
"""Focused host-only tests for the OT-150 restoration-safe coordinator."""
...
MODULE_PATH = ROOT / "tools" / "ot150_mbedtls_psa_coordinator.py"
SPEC = importlib.util.spec_from_file_location("ot150_coordinator_tests_module", MODULE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("unable to load OT-150 mbedTLS/PSA coordinator")
coordinator = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = coordinator
SPEC.loader.exec_module(coordinator)


BENCHMARK_NAME = "ot150_mbedtls_psa_bench.bin"
BENCHMARK_PAYLOAD = b"ot150 mbedtls psa benchmark fixture"
...
class Fixture:
    def __init__(self) -> None:
        ...
        self.binding = coordinator.ExecutionBinding(
            ...
            protocol_start=coordinator.protocol.START.decode("ascii"),
            protocol_ready=coordinator.protocol.READY.decode("ascii"),
    ...
    def __enter__(self):
        ...
        self.stack.enter_context(mock.patch.object(
            ...
            self.private / "ot150-mbedtls-psa-execution-journal.json",
        ))
        self.stack.enter_context(mock.patch.object(
            coordinator, "EXECUTION_RECEIPT_PATH",
            self.private / "ot150-mbedtls-psa-execution-receipt.json",
        ))
        self.stack.enter_context(mock.patch.object(
            coordinator, "RECOVERY_RECEIPT_PATH",
            self.private / "ot150-mbedtls-psa-recovery-receipt.json",
        ))
        self.stack.enter_context(mock.patch.object(
...
def diagnostics(frame_count: int = 1015):
...
def capture_result(label: str):
    return coordinator.protocol.CaptureResult(
        parsed={"candidate_id": "esp_idf_mbedtls_psa", "node_fixture": label},
        diagnostics=diagnostics(),
    )


class Ot150CoordinatorTests(unittest.TestCase):
    ...
    def test_01_success_requires_two_captures_and_restores_each_node(self) -> None:
        with Fixture() as fixture:
            ...
            self.assertEqual(receipt["result"], "two_node_mbedtls_psa_passed_and_restored")
    ...
    def test_11_private_values_and_broad_write_surfaces_are_structurally_absent(self) -> None:
        ...
        self.assertIn("ot150_mbedtls_psa_protocol_runner.py", source)
        ...
        duplicate_config = coordinator.RunConfig(
            ...
            binding=coordinator.ExecutionBinding(
                benchmark_name=BENCHMARK_NAME,
                benchmark_bytes=len(BENCHMARK_PAYLOAD),
                benchmark_sha256=hashlib.sha256(BENCHMARK_PAYLOAD).hexdigest(),
                restore_name=coordinator.RESTORE_NAME,
                restore_bytes=coordinator.RESTORE_BYTES,
                restore_sha256=coordinator.RESTORE_SHA256,
                application_offset=coordinator.APPLICATION_OFFSET,
                baud=coordinator.BAUD,
                protocol_start=coordinator.protocol.START.decode("ascii"),
                protocol_ready=coordinator.protocol.READY.decode("ascii"),
            ...
            benchmark_path=Path("C:/placeholder/ot150_mbedtls_psa_bench.bin"),
