#!/usr/bin/env python3
"""Adversarial host tests for the OT-143 executable bundle authority."""
...
TOOL_PATH = ROOT / "tools" / "ot143_monocypher_execution_authority.py"
SPEC = importlib.util.spec_from_file_location("ot143_authority", TOOL_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("OT-143 authority tool unavailable")
...
class Fixture:
    def __init__(self, directory: str) -> None:
        ...
        self.ot140_authority_payload = b"ot140-consumed-authority-v0\n"
        ...
        self._write(
            self.root / contract.OT140_AUTHORITY_RELATIVE,
            self.ot140_authority_payload,
        )
        self._write(
    ...
    def patches(self):
        return (
            ...
            mock.patch.object(
                ...
                sha256(self.ot140_authority_payload),
            ),
            mock.patch.object(
                contract,
...
class ContractTests(unittest.TestCase):
    ...
    def test_01_current_ot140_ot142_and_runtime_bindings_match(self) -> None:
        ...
        for relative, digest in (
            (contract.OT140_AUTHORITY_RELATIVE, contract.OT140_AUTHORITY_RAW_SHA256),
            (
    ...
    def test_02_preparation_binds_corrected_target_and_consumed_ot140(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ot143-authority-test-") as directory:
            ...
            self.assertTrue(supersession["ot140_authority_consumed"])
            self.assertFalse(supersession["ot140_authority_reusable"])
            self.assertFalse(supersession["replacement_attempt_inherited"])
            self.assertEqual(
                supersession["ot140_terminal_boundary"],
                "fail_closed_exact_restoration_no_result",
            )
