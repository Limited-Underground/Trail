"""Actual OT203 orchestration/authority/observation/restoration; synthetic storage only."""
from pathlib import Path
import copy,json,sys,tempfile,unittest
from unittest.mock import patch
ROOT=Path(__file__).resolve().parents[2]
sys.path.insert(0,str(ROOT/'tools'))
import security_policy_receipt_execution as x
...
class Tests(unittest.TestCase):
    def setUp(self):
        ...
        self.image=x.readback.ImageBinding(x.readback.SYNC_CONTRACT,'heltec_v4_security_receipt_sync',x.sha(self.data['candidate']))
    ...
    def test_observation_failure_restoration_success(self):
        ...
        self.assertEqual(claim['schema'],'OT203-RECEIPT-OBSERVATION-CLAIM-1')
