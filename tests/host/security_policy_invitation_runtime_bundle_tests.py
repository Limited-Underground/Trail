...
spec = importlib.util.spec_from_file_location("runtime_bundle_tested", ROOT / "tools/security_policy_invitation_runtime_bundle.py")
...
class BundleTests(unittest.TestCase):
    ...
    def test_old_manifest_schema_refused(self):
        manifest=self.build()
        self.assertEqual(manifest['schema'],'OT210-INVITATION-RUNTIME-1')
    ...
    def test_sync_required_file_cannot_be_removed_from_manifest(self):
        manifest=self.build()
        name='policy/security_policy_invitation_execution.py'
    ...
    def test_complete_old_and_new_policy_closure(self):
        ...
        expected=tuple('security_policy_'+name+'.py' for name in (
            ...
            'invitation_readback','invitation_observation','invitation_bundle','invitation_execution','invitation_operator',
            'invitation_backup_operator','invitation_runtime_bundle','invitation_restore_observation','receipt_boundary'))+(
            'Invoke-SecurityPolicyOperator.ps1','Invoke-SecurityPolicyInvitationOperator.ps1')
        self.assertEqual(23,len(expected));self.assertEqual(set(expected),set(bundle.POLICY))
