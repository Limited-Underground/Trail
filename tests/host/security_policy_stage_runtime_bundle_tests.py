...
spec = importlib.util.spec_from_file_location("runtime_bundle_tested", ROOT / "tools/security_policy_stage_runtime_bundle.py")
...
class BundleTests(unittest.TestCase):
    ...
    def test_old_manifest_schema_refused(self):
        manifest=self.build()
        self.assertEqual(manifest['schema'],'OT196-STAGE-RUNTIME-1')
    ...
    def test_stage_required_file_cannot_be_removed_from_manifest(self):
        manifest=self.build()
        name='policy/security_policy_stage_execution.py'
        (self.destination/name).unlink();manifest['files'].pop(name)
        with self.assertRaises(bundle.RuntimeBundleError):bundle.verify(self.destination,manifest)

    def test_complete_old_and_new_policy_closure(self):
        manifest=self.build()
        for name in ('security_policy_execution.py','security_policy_stage_execution.py',
                     'security_policy_stage_readback.py','security_policy_stage_observation.py',
                     'security_policy_stage_runtime_bundle.py','Invoke-SecurityPolicyStageOperator.ps1'):
