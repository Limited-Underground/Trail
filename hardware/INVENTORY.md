...
| Protected-storage source-read attempt | One owner-authorized, one-use no-stub/no-reset ROM attempt returned only `DENY-READ-FAILURE`; it was not repeated, retained no raw bytes or private binding details, and its ephemeral executor was deleted. Manual RST returned the Trail logo and `BLE ADVERTISING`; the partition-table and blank-region source proof remains absent | [OT-073 evidence](../tests/hardware/OT-073-2026-08-17.md) |
