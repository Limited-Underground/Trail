...
| --- | --- | --- |
| Inventory state | Experimentally flashed OpenTrail target; exact profile, boot/self-check/USB heartbeat, service advertising, one fixed public BLE read, and target-local Trail startup/link-status OLED passed. Exact SKU/RF front end, installed antenna, full pinout, protected authorization, LoRa/GNSS, interactive UI/input, recovery-after-loss, and support remain unresolved | [OT-061](../tests/hardware/OT-061-2026-08-16.md), [OT-064](../tests/hardware/OT-064-2026-08-17.md), and [OT-085A](../tests/hardware/OT-085A-2026-08-19.md) evidence |
| Evidence role | Experimented OpenTrail bench target with physically accepted startup/status OLED and one bounded public BLE link/read lifecycle; not validated or supported hardware | OT-061 full-image, OT-064 app-only, and OT-085A app-only write authorizations are consumed; no standing write, recovery, unit-2, or support authority |
...
| Current bounded runtime | Deterministic boot self-check and heartbeat retained; one Android 13 phone read the exact fixed 16-byte public value, the owner observed `BLE CONNECTED`, and phone disconnect returned the target to `BLE ADVERTISING` before same-endpoint rediscovery | [OT-085A evidence](../tests/hardware/OT-085A-2026-08-19.md); no name, address, pairing, protected authorization, or private binding retained |
