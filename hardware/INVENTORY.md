...
| Current installed application | Experimental OpenTrail `heltec_v4_bench`; 500,944-byte application, SHA-256 `F2A58414F82EED585BA90BF7671B06C8EBFAA82E8B23F6AC2093DE95143B4E0E` | OT-147 application-only writes, independent exact readbacks, and hard resets succeeded on both anonymous bench nodes |
| Current bounded runtime | The owner visually confirmed live `BAT` and `GPS` values after both hard resets. Battery percentage is an approximate voltage-derived indication; GPS exposes only the current satellite count; BLE retains its existing live state. The traffic field remains blank because LoRa RX/TX activity is not yet bound | [OT-147 evidence](../tests/hardware/OT-147-2026-08-26.md); [OT-115](../tests/hardware/OT-115-2026-08-21.md) remains historical placeholder-layout evidence and [OT-114](../tests/hardware/OT-114-2026-08-21.md) remains historical direct-radio evidence for a replaced diagnostic image |
