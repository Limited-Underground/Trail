...
Complete the remaining OT-003A physical/regulatory inventory and OT-005 cryptographic gates in parallel. Next specify and host-test OT-017's normalized OpenGauge critical-alert input, including schema versioning, validation, trust/authentication boundary, rate/stale/duplicate behavior, and fixtures. OT-009's remaining hardware work is a direct SX1262/OpenTrail binding and later field measurements; the software-forced MeshCore path is now proven. Security and regulatory constraints must remain inputs before any public packet v1 is declared.
